...
public class Central extends Jugador {
    ...
    public Central(int idx, PlayerDetail detalle, Position posRef) {
        super(idx, detalle, posRef);
        this.setTipo(Jugador.TIPO_CENTRAL);
    }

    public Central(int idx, PlayerDetail detalle) {
        super(idx, detalle);
        this.setTipo(Jugador.TIPO_CENTRAL);
    }
    ...
    public Command irAPosicionDestino() {
        ...
        //        }
            ...
            boolean isLateral = Math.abs(this.getPosicionActual().getX()) > Math.abs(Constants.posteDerArcoSup.getX() * 2);
            if (this.estaBalonEnCampoRival() && isDentroArea && !isLateral) {
                ...
                double x = (this.getPosicionBalon().getX() * (Constants.ANCHO_CAMPO_JUEGO / 8)) / (Constants.ANCHO_CAMPO_JUEGO / 2);
    ...
    private Command disparar(Position posPortero) {
        ...
        // Portero a la izda
        if (posPortero.distance(Constants.posteDerArcoSup) > posPortero.distance(Constants.posteIzqArcoSup)) {
            Position posDisparo = Position.medium(Constants.centroArcoSup, Constants.posteDerArcoSup);
            // Rematador a la izda
            if (this.getPosicionActual().getX() <= 0) {
