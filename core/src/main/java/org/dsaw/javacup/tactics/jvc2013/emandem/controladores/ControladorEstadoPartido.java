...
public class ControladorEstadoPartido {
  ...
  private Map<String, List<Integer>> jugadoresPorCuadrantes = new HashMap<>(0);
  private Map<Integer, String> jugadoresConCuadrante = new HashMap<>();
  private List<Integer> jugadoresPuedenRecuperar = new ArrayList<>();
  private List<Integer> jugadoresPuedenRematar = new ArrayList<>();
  private List<Integer> jugadoresRestantes = new ArrayList<>();
  private Map<Integer, CirculoMarcacion>
      marcasDeJugadores =
      new HashMap<>();
  private Map<Integer, List<Integer>> jugadoresMarcados = new HashMap<>(0);
  ...
  public void analizarYCargarInformacion(final GameSituations situacionPartido) {
    ...
    for (int numJugador = 0; numJugador < situacionPartido.myPlayers().length; numJugador++) {
      if (numJugador > JUGADOR.PORTERO.numero()) {
        CUADRANTE_CANCHA
            cuadrante =
            CuadrantesCancha.getInstance()
                .getCuadrantePorPosicion(situacionPartido.myPlayers()[numJugador]);
    ...
    for (int numJugador = 0; numJugador < situacionPartido.myPlayers().length; numJugador++) {
      ...
      if (!this.marcasDeJugadores.containsKey(numJugador)) {
        this.marcasDeJugadores.put(numJugador,
                                   new CirculoMarcacion(situacionPartido.myPlayers()[numJugador],
                                                        ControladorUtils.DISTANCIA_MARCAJE));
      }
      this.marcasDeJugadores.get(numJugador)
          .setJugadorPosicionX(situacionPartido.myPlayers()[numJugador].getX());
      this.marcasDeJugadores.get(numJugador)
          .setJugadorPosicionY(situacionPartido.myPlayers()[numJugador].getY());
    }

    // Cataloga a los jugadores con marca
    for (int numJugador = 0; numJugador < situacionPartido.myPlayers().length; numJugador++) {
      if (numJugador > JUGADOR.PORTERO.numero()) {
        for (int numJugadorEnemigo = 0; numJugadorEnemigo < situacionPartido.rivalPlayers().length;
             numJugadorEnemigo++) {
          if (this.marcasDeJugadores.get(numJugador)
              .estoyMarcado(situacionPartido.rivalPlayers()[numJugadorEnemigo])) {
