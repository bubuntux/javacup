...
public class Tactica_Mou_Team {
    ...
    public List<Command> generarTactica(GameSituations sp) {
        ...
        LinkedList<Command> listaComandos = new LinkedList<Command>();
    ...
    {
        if (this.sp.isStarts() && this.sp.ballPosition().getY() == -47 && Math.abs(this.sp.ballPosition().getX()) == 9.16) {
        ...
        if (this.sp.ballPosition().getY() > 25) {
            ...
            for (int i = 1; i < 10; i++) {
                if (this.buscarNumEnArray(this.sp.canKick(), i, 0, 100) != -1) { //el defensa puede rematar
                    angulo = this.calcularAnguloVerticalLlegarJusto(dist, this.sp.myPlayersDetail()[i].getPower());
    ...
    private void moverJugadoresEnFuncionBalon(GameSituations sp, List<Command> comandos) {
        ...
        Position posicionesActuales[] = {
            new Position(this.control(0 + coenfAdX, -10, 10), this.control(42 + coenfAdY, 31, 42)), //delantero,
            new Position(this.control(-8 + coenfAdX, -25, 5), this.control(-26 + coenfAdY, -37, -20)), //central izq
            new Position(this.control(0 + coenfAdX, -20, 20), this.control(-26 + coenfAdY, -41, 0)), // central
            new Position(this.control(8 + coenfAdX, -5, 25), this.control(-26 + coenfAdY, -37, -20)), //central derecho
            new Position(this.control(12 + coenfAdX, -5, 25), this.control(0 + coenfAdY, -24, 19)), //  pivote derecha
            new Position(this.control(-12 + coenfAdX, -25, 5), this.control(0 + coenfAdY, -24, 19)), //  pivote izquierda
            new Position(this.control(0 + coenfAdX, -20, 20), this.control(0 + coenfAdY, -27, 15)), //  pivote
            new Position(this.control(12 + coenfAdX, 0, 20), this.control(27 + coenfAdY, 0, 42)), //  extremo derecho
            new Position(this.control(-12 + coenfAdX, -20, 0), this.control(27 + coenfAdY, 0, 42)), //  extremo izquierdo
            new Position(this.control(0 + coenfAdX, -15, 15), this.control(20 + coenfAdY, -10, 34)), // media punta
            new Position(this.control(sp.ballPosition().getX(), -(Constants.LARGO_ARCO / 2) + 1.3, (Constants.LARGO_ARCO / 2) - 1.3), -52)}; //  PORTERO
    ...
    private LinkedList<Command> moverPortero(LinkedList<Command> listaComandos) {
        if (this.sp.rivalGoals() > this.golesContrario) {
            ...
            if (this.posAnteriorBalon.distance(this.posAnteriorPortero) > Constants.DISTANCIA_CONTROL_BALON_PORTERO) {
        ...
        if (this.sp.isStarts() && this.sp.ballPosition().getY() == -47 && Math.abs(this.sp.ballPosition().getX()) == 9.16) {
            listaComandos.add(new CommandMoveTo(10, new Position(this.sp.ballPosition().getX(), this.sp.ballPosition().getY())));
        } else if (this.sp.ballPosition().getY() > -22.5) { //ballPosition lejos
            listaComandos.add(new CommandMoveTo(10, new Position(this.control(this.sp.ballPosition().getX(), -(Constants.LARGO_ARCO / 2) + 1.5, (Constants.LARGO_ARCO / 2) - 1.5), -48)));
    ...
    private int iteracionesTardar(int fuerzaRemate, int remateJugador, double gradosVert, double distancia) {
        int it;
    ...
    private int itBalonCercaGol() {
        ...
        for (int it = 1; it < 82 && iteracion == -1; it++) {
            if (Math.abs(this.sp.getTrajectory(it)[0]) < ((Constants.LARGO_ARCO + 1) / 2) && this.sp.getTrajectory(it)[1] <= -52.5) { //esto seria gol por lo que me tengo que quedar con la iteration anterior
                iteracion = it - 1;
            }
        }
        return iteracion;
    ...
    private int itInteresaSalirPortero() {
        ...
        for (i = 1; i < 100; i++) {
            ...
            yoLlego = posBalon.distance(posPortero) <= ((velPortero * i) + Constants.DISTANCIA_CONTROL_BALON_PORTERO);
...
//System.out.println(i);
    ...
    private LinkedList<Command> irAPorBalon(LinkedList<Command> listaComandos) {
        ...
        if (primerMomentoChute[0] != 0) {
            posFuturaBalon = new Position(this.sp.getTrajectory(primerMomentoChute[0])[0], this.sp.getTrajectory(primerMomentoChute[0])[1]);
            ...
            if (elLlegaAntes) {
                ...
                if (this.llegaRivalATiempoChutar(posFuturaBalon, primerMomentoChute[0])) {
                    if (posFuturaBalon.distance(new Position(0, 52.5)) < 18 || posFuturaBalon.distance(new Position(0, -52.5)) < 18) {
    ...
    private boolean llegaRivalATiempoChutar(Position pos, int it) {
        ...
        for (int i = 0; !llega && i < 11; i++) {
            ...
            if (this.sp.rivalPlayersDetail()[i].isGoalKeeper()) {
                if (this.sp.getTrajectory(it)[2] <= Constants.ALTO_ARCO && this.sp.rivalPlayers()[i].distance(pos) <= ((Constants.getVelocidad(this.sp.rivalPlayersDetail()[i].getSpeed()) * it) + Constants.DISTANCIA_CONTROL_BALON_PORTERO)) {
                    llega = true;
                }

            } else {
                if (this.sp.getTrajectory(it)[2] <= Constants.ALTURA_CONTROL_BALON && this.sp.rivalPlayers()[i].distance(pos) <= ((Constants.getVelocidad(this.sp.rivalPlayersDetail()[i].getSpeed()) * it) + Constants.DISTANCIA_CONTROL_BALON)) {
                    llega = true;
                }
            }
        }
    ...
    private boolean interceptariaContrarioRegate(PlayerDetail jug, Position posJug, Position posDestino) {
        ...
        for (int it = 0; it < 150; it++) {
            ...
            if (posNueva.isInsideGameField(0)) {
                if (it > 10 && posJug.distance(posNueva) <= (Constants.getVelocidad(jug.getSpeed()) * it) + Constants.DISTANCIA_CONTROL_BALON) {
                    yoLlego = true;
                ...
                for (int i = 0; !elLlega && i < 11; i++) {
                    ...
                    if (this.sp.rivalPlayersDetail()[i].isGoalKeeper()) {
                        if (this.sp.rivalPlayers()[i].distance(posNueva) <= ((Constants.getVelocidad(this.sp.rivalPlayersDetail()[i].getSpeed()) * it) + Constants.DISTANCIA_CONTROL_BALON_PORTERO)) {
                            elLlega = true;
                        }

                    } else {
                        if (this.sp.rivalPlayers()[i].distance(posNueva) <= ((Constants.getVelocidad(this.sp.rivalPlayersDetail()[i].getSpeed()) * it) + Constants.DISTANCIA_CONTROL_BALON)) {
                            elLlega = true;
                        }
                    }
                }
    ...
    private int[] jugadoresMiosMasCercanos(Position pos, int cuantosBuscar, double minX, double maxX, double minY, double maxY, double distMax, int... jugsNoIncluir) {
        ...
        for (int jug = 0; jug < 11 && cont < cuantosBuscar; jug++) {
            if (this.buscarNumEnArray(jugsNoIncluir, jug, 0, 100) == -1) {
                ...
                if (posJug.getX() > minX && posJug.getX() < maxX && posJug.getY() > minY && posJug.getY() < maxY && pos.distance(posJug) < distMax) {
                    misJugs[cont] = jug;
                    cont++;
                }
            }
    ...
    private int jugMasLibre(int... misJugs) {
        ...
        for (int i = 0; i < tamanio; i++) {
            ...
            itMinima = 1000;
            for (int j = 0; j < 11; j++) {
                itTemp = (int) Math.ceil(posMiJugador.distance(this.sp.rivalPlayers()[j]) / Constants.getVelocidad(this.sp.rivalPlayersDetail()[j].getSpeed()));
                if (itTemp < itMinima) {
                    itMinima = itTemp;
                    jugMin = j;
                }
    ...
    private LinkedList<Command> defensaGolpearBalon(LinkedList<Command> listaComandos) {
        ...
        for (int i = 1; i < 4; i++) {
            if (this.buscarNumEnArray(this.sp.canKick(), i, 0, 100) != -1) //el defensa puede rematar
            {
                jugsAPasar = this.jugadoresMiosMasCercanos(this.sp.myPlayers()[i], 6, -100, 200, this.sp.myPlayers()[i].getY() + 2, 200, 30, 1, 2, 3);
        ...
        for (int i = 4; i < 7; i++) {
            if (this.buscarNumEnArray(this.sp.canKick(), i, 0, 100) != -1) //el defensa puede rematar
            {
                ...
                if (this.sp.myPlayers()[i].getY() < 35) {
                    ...
                    if (i == 4) { // medio derecha
                        for (int j = 6; j > -6 && !corro; j--) {
                            if (!this.interceptariaContrarioRegate(this.sp.myPlayersDetail()[i], this.sp.myPlayers()[i], new Position(j, 53)) && !this.sp.isStarts()) {
                ...
                if (this.sp.myPlayers()[i].getY() >= 35 || !corro) {
                    jugsAPasar = this.jugadoresMiosMasCercanos(this.sp.myPlayers()[i], 6, -100, 200, this.sp.myPlayers()[i].getY() + 7, 200, 30, 1, 2, 3, 10);
                    //System.out.println(Arrays.toString(jugsAPasar));
                    jugAPasar = this.jugMasLibre(jugsAPasar);
                    dist = this.sp.ballPosition().distance(this.sp.myPlayers()[jugAPasar]);
                    angulo = this.calcularAnguloVerticalLlegarJusto(dist, this.sp.myPlayersDetail()[i].getPower());
    ...
    private LinkedList<Command> porteroGolpearBalon(LinkedList<Command> listaComandos) {
        ...
        {
            jugsAPasar = this.jugadoresMiosMasCercanos(this.sp.myPlayers()[10], 6, -100, 200, this.sp.myPlayers()[10].getY() + 10, 200, 40, 0);
            //System.out.println(Arrays.toString(jugsAPasar));
            jugAPasar = this.jugMasLibre(jugsAPasar);
            dist = this.sp.ballPosition().distance(this.sp.myPlayers()[jugAPasar]);
            angulo = this.calcularAnguloVerticalLlegarJusto(dist, this.sp.myPlayersDetail()[10].getPower());
    ...
    private LinkedList<Command> mediaPuntaGolpearBalon(LinkedList<Command> listaComandos) {
        ...
        double distDelanteroDefensa = delantero.distance(this.sp.rivalPlayers()[delantero.nearestIndex(this.sp.rivalPlayers())]);
        double distExtremoIzqDefensa = extremoIzq.distance(this.sp.rivalPlayers()[extremoIzq.nearestIndex(this.sp.rivalPlayers())]);
        double distExtremoDchaDefensa = extremoDcha.distance(this.sp.rivalPlayers()[extremoDcha.nearestIndex(this.sp.rivalPlayers())]);
        ...
        if (distDelanteroDefensa > 7 && distDelanteroPorteria < 16 && distanciaDelanteroYo < 32) {
            mejorAngulo = this.calcularAnguloVerticalLlegarJusto(this.sp.ballPosition().distance(delantero), this.sp.myPlayersDetail()[9].getPower());
            ...
            for (int j = -4; j < 4 && !corro; j++) {
                if (!this.interceptariaContrarioRegate(this.sp.myPlayersDetail()[9], this.sp.myPlayers()[9], new Position(j, 53))) {
            ...
            if (!corro) {
                if (distExtremoIzqDefensa > 10 && distExtremoDchaDefensa > 10) {
                    ...
                    mejorAngulo = this.calcularAnguloVerticalLlegarJusto(this.sp.ballPosition().distance(this.sp.myPlayers()[jugMasLibre]), this.sp.myPlayersDetail()[9].getPower());
                    listaComandos.add(new CommandHitBall(9, this.sp.myPlayers()[jugMasLibre], 1, mejorAngulo));
                } else if (distDelanteroDefensa > 5 && distDelanteroPorteria < 20 && distanciaDelanteroYo < 40) {
                    mejorAngulo = this.calcularAnguloVerticalLlegarJusto(this.sp.ballPosition().distance(delantero), this.sp.myPlayersDetail()[9].getPower());
                    listaComandos.add(new CommandHitBall(9, this.sp.myPlayers()[0], 1, mejorAngulo));
    ...
    private LinkedList<Command> delanteroGolpearBalon(LinkedList<Command> listaComandos) {
        ...
        if (this.buscarNumEnArray(this.sp.canKick(), 0, 0, 30) != -1) {
            ...
            if (this.sp.ballPosition().getY() < 30 && !this.interceptariaContrarioRegate(this.sp.myPlayersDetail()[0], this.sp.myPlayers()[0], posPorteria)) {
    ...
    private LinkedList<Command> extremoIzquierdoGolpearBalon(LinkedList<Command> listaComandos) {
        ...
        Position porteria = new Position(0, 52.5);
        double distDelanteroDefensa = delantero.distance(this.sp.rivalPlayers()[delantero.nearestIndex(this.sp.rivalPlayers())]);
        double distMediaPuntaDefensa = mediaPunta.distance(this.sp.rivalPlayers()[mediaPunta.nearestIndex(this.sp.rivalPlayers())]);
        double mejorAngulo;

        if (extIzq.distance(porteria) < 26 && (this.sp.ballPosition().getY() < 49 || this.sp.ballPosition().getX() > -9)) {
            listaComandos.add(this.tirarAPorteria(8));

        } else if (delantero.distance(porteria) < 17 && distDelanteroDefensa > 7) {
            mejorAngulo = this.calcularAnguloVerticalLlegarJusto(this.sp.ballPosition().distance(delantero), this.sp.myPlayersDetail()[8].getPower());
            listaComandos.add(new CommandHitBall(8, delantero, 1, mejorAngulo));

        } else {
            boolean corro = false;
            for (int j = -6; j < 6 && !corro; j++) {
                if (!this.interceptariaContrarioRegate(this.sp.myPlayersDetail()[8], this.sp.myPlayers()[8], new Position(j, 53))) {
            ...
            if (!corro) {
                if (extIzq.distance(porteria) < 29 && (this.sp.ballPosition().getY() < 49 || this.sp.ballPosition().getX() > -9)) {
                    listaComandos.add(this.tirarAPorteria(8));
                    ...
                    mejorAngulo = this.calcularAnguloVerticalLlegarJusto(this.sp.ballPosition().distance(mediaPunta), this.sp.myPlayersDetail()[8].getPower());
                    listaComandos.add(new CommandHitBall(8, mediaPunta, 1, mejorAngulo));

                } else if (extIzq.distance(porteria) < 32 && (this.sp.ballPosition().getY() < 49 || this.sp.ballPosition().getX() > -9)) {
                    listaComandos.add(this.tirarAPorteria(8));
    ...
    private LinkedList<Command> extremoDchaGolpearBalon(LinkedList<Command> listaComandos) {
        ...
        if (extDcho.distance(porteria) < 26 && (this.sp.ballPosition().getY() < 49 || this.sp.ballPosition().getX() < 9)) {
            listaComandos.add(this.tirarAPorteria(7));

        } else if (delantero.distance(porteria) < 17 && distDelanteroDefensa > 7) {
            mejorAngulo = this.calcularAnguloVerticalLlegarJusto(this.sp.ballPosition().distance(delantero), this.sp.myPlayersDetail()[7].getPower());
            listaComandos.add(new CommandHitBall(7, delantero, 1, mejorAngulo));

        } else {
            boolean corro = false;
            for (int j = 6; j > -6 && !corro; j--) {
                if (!this.interceptariaContrarioRegate(this.sp.myPlayersDetail()[7], this.sp.myPlayers()[7], new Position(j, 53))) {
            ...
            if (!corro) {
                if (extDcho.distance(porteria) < 29 && (this.sp.ballPosition().getY() < 49 || this.sp.ballPosition().getX() < 9)) {
                    listaComandos.add(this.tirarAPorteria(7));
                    ...
                    mejorAngulo = this.calcularAnguloVerticalLlegarJusto(this.sp.ballPosition().distance(mediaPunta), this.sp.myPlayersDetail()[7].getPower());
                    listaComandos.add(new CommandHitBall(7, mediaPunta, 1, mejorAngulo));

                } else if (extDcho.distance(porteria) < 32 && (this.sp.ballPosition().getY() < 49 || this.sp.ballPosition().getX() < 9)) {
                    listaComandos.add(this.tirarAPorteria(7));
    ...
    private LinkedList<Command> sacarDeCentro(LinkedList<Command> listaComandos) {
        double dist, angulo;
        if (this.sp.ballPosition().getX() == 0 && this.sp.ballPosition().getY() == 0 && this.sp.getTrajectory(1)[0] == 0 && this.sp.getTrajectory(1)[1] == 0 && this.sp.getTrajectory(1)[2] == 0) {
            for (int jug = 0; jug < 10; jug++) {
                if (this.buscarNumEnArray(this.sp.canKick(), jug, 0, 100) != -1) {
                    if (this.sp.myGoals() == 0 && this.sp.rivalGoals() == 0) {
                        ...
                        angulo = this.calcularAnguloVerticalLlegarJusto(dist, this.sp.myPlayersDetail()[jug].getPower());
    ...
    private LinkedList<Command> irAlSaqueDeCentro(LinkedList<Command> listaComandos) {
        if (this.sp.ballPosition().getX() == 0 && this.sp.ballPosition().getY() == 0 && this.sp.getTrajectory(1)[0] == 0 && this.sp.getTrajectory(1)[1] == 0 && this.sp.getTrajectory(1)[2] == 0 && this.sp.myGoals() == 0 && this.sp.rivalGoals() == 0) {
    ...
    private Command tirarAPorteria(int numJug) {
        ...
        puntuacionesPorteria = this.generarArrayPuntuacionesGol(angulosPorteria, this.sp.myPlayersDetail()[numJug], this.sp.ballPosition());
        ...
        for (int i = 0; i < cantidadMiniTrozos; i++) { //como el array esta ordenado, no hace falta recorrerlo entero
            ...
            if (puntuacionesPorteria[i].getProbabilidad() >= (mayorProbabilidad - 0.05) || puntuacionesPorteria[i].getProbabilidad() >= 0.9) {
                if (tiro.distance(this.sp.rivalPlayers()[this.porteroContrario]) > mayorDistanciaPortero) {
                    mayorDistanciaPortero = posDisparo.distance(this.sp.rivalPlayers()[this.porteroContrario]);
                    posDisparo = tiro;
                    indice = i;
        ...
        if (this.tieneMasDeUnPortero()) {
            ...
            anguloVertRapido = calcularAnguloVerticalRapido(this.sp.ballPosition().distance(posDisparo), this.sp.myPlayersDetail()[numJug].getPower(), 3.5, 4.7);

        } else {
            anguloVertRapido = calcularAnguloVerticalRapido(this.sp.ballPosition().distance(posDisparo), this.sp.myPlayersDetail()[numJug].getPower(), 0, 100);
        }
    ...
    private boolean tieneMasDeUnPortero() {
        ...
        Position posPorteria = new Position(0, 52.5);
        int contrarioMasCercano = posPorteria.nearestIndex(this.sp.rivalPlayers(), this.porteroContrario);
