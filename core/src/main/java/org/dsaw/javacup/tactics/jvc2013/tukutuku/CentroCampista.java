...
public class CentroCampista extends Jugador {
  ...
  public ArrayList<Command> accionRemate() {
    ...
    if (this.posicionJugador.distance(Constants.centroArcoSup)
        < Constants.ANCHO_AREA_GRANDE * 1.5) {
      TiroInfo tInfo = seleccionarTiro();

      if (tInfo != null) {
        accion.add(new CommandHitBall(numero, tInfo.angulo * (180 / Math.PI), tInfo.fuerza,
                                      tInfo.anguloVert * (180 / Math.PI)));
      }
    ...
    if ((accion.size() == 0) && (this.posicionJugador.distance(Constants.centroArcoSup)
      ...
      if (this.posicionJugador.distance(Constants.centroArcoSup) < Constants.ANCHO_AREA_GRANDE) {
        switch (variacionTiro) {
          case 0:
            accion.add(new CommandHitBall(numero, entrenador.Gs.myPlayers()[numero]
                                                      .angle(Constants.centroArcoSup) * (180
                                                                                         / Math.PI),
                                          1, 20));
            break;
          case 1:
            accion.add(new CommandHitBall(numero, entrenador.Gs.myPlayers()[numero].angle(
                Constants.posteDerArcoSup.movePosition(-1, 0)) * (180 / Math.PI), 1, 20));
            break;
          case 2:
            accion.add(new CommandHitBall(numero, entrenador.Gs.myPlayers()[numero].angle(
                Constants.posteIzqArcoSup.movePosition(2, 0)) * (180 / Math.PI), 1, 20));
            break;
          case 3:
            accion.add(new CommandHitBall(numero, entrenador.Gs.myPlayers()[numero].angle(
                Constants.centroArcoSup.movePosition(-3, 0)) * (180 / Math.PI), 1, 15));
            break;
          case 4:
            accion.add(new CommandHitBall(numero, entrenador.Gs.myPlayers()[numero].angle(
                Constants.centroArcoSup.movePosition(3, 0)) * (180 / Math.PI), 1, 15));
        ...
        int
            jugadorCercanoNum =
            Constants.centroArcoSup.nearestIndex(entrenador.Gs.myPlayers(), numero);
        Position jugadorCercanoPos = entrenador.Gs.myPlayers()[jugadorCercanoNum];

        accion.add(new CommandHitBall(numero, this.posicionJugador.angle(jugadorCercanoPos) * (180
                                                                                               / Math.PI),
                                      (this.posicionJugador.distance(jugadorCercanoPos) < 20) ? 0.8
                                                                                              : 1,
                                      30));
