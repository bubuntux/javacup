...
public class lacras implements Tactic {
  ...
  class TacticDetailImpl implements TacticDetail {
    ...
    public PlayerDetail[] getPlayers() {
      return new PlayerDetail[]{
          new JugadorImpl("Cano", 1, new Color(255, 200, 150), new Color(50, 0, 0), 1.0d, 1.0d,
                          0.5d, true),
          new JugadorImpl("Rubia", 2, new Color(255, 200, 150), new Color(204, 204, 0), 1.0d, 0.66d,
                          0.5d, false),
          new JugadorImpl("Negro", 3, new Color(255, 255, 255), new Color(255, 255, 255), 1.0d,
                          0.66d, 0.5d, false),
          new JugadorImpl("Furier", 4, new Color(51, 0, 0), new Color(50, 0, 0), 1.0d, 0.66d, 0.5d,
                          false),
          new JugadorImpl("sr burns", 5, new Color(255, 200, 150), new Color(204, 204, 204), 1.0d,
                          0.66d, 0.5d, false),
          new JugadorImpl("Araya", 6, new Color(255, 200, 150), new Color(50, 0, 0), 1.0d, 1.0d,
                          1.0d, false),
          new JugadorImpl("Migue", 7, new Color(255, 200, 150), new Color(50, 0, 0), 1.0d, 0.66d,
                          0.5d, false),
          new JugadorImpl("Dany", 8, new Color(255, 200, 150), new Color(50, 0, 0), 1.0d, 0.66d,
                          0.5d, false),
          new JugadorImpl("Tron", 9, new Color(153, 153, 255), new Color(102, 102, 255), 1.0d, 1.0d,
                          1.0d, false),
          new JugadorImpl("Hulk", 10, new Color(0, 153, 0), new Color(50, 0, 0), 1.0d, 1.0d, 1.0d,
                          false),
          new JugadorImpl("Chicle", 11, new Color(255, 102, 255), new Color(153, 0, 0), 1.0d, 1.0d,
                          1.0d, false)
      };
    }
  }
  ...
  public int powerOnTheBall(double altura, Position[] myPlayers, Position[] rivalPlayers,
                            Position ball) {
    ...
    for (int i = 0; i < playerPos.length; i++) {
      if (playerPos[i].getX() < Constants.cornerInfIzq.getX() + Constants.ANCHO_CAMPO_JUEGO / 3) {
        if (playerPos[i].getY() < Constants.cornerInfIzq.getY() + Constants.LARGO_CAMPO_JUEGO / 6) {
          zones[0][0]++;
          zones[0][zones[0][0]] = i;
        } else if (playerPos[i].getY()
                   < Constants.cornerInfIzq.getY() + (Constants.LARGO_CAMPO_JUEGO / 6) * 2) {
          zones[3][0]++;
          zones[3][zones[3][0]] = i;
        } else if (playerPos[i].getY()
                   < Constants.cornerInfIzq.getY() + (Constants.LARGO_CAMPO_JUEGO / 6) * 3) {
          zones[6][0]++;
          zones[6][zones[6][0]] = i;
        } else if (playerPos[i].getY()
                   < Constants.cornerInfIzq.getY() + (Constants.LARGO_CAMPO_JUEGO / 6) * 4) {
          zones[9][0]++;
          zones[9][zones[9][0]] = i;
        } else if (playerPos[i].getY()
                   < Constants.cornerInfIzq.getY() + (Constants.LARGO_CAMPO_JUEGO / 6) * 5) {
          zones[12][0]++;
          zones[12][zones[12][0]] = i;
        } else if (playerPos[i].getY()
                   < Constants.cornerInfIzq.getY() + Constants.LARGO_CAMPO_JUEGO) {
          zones[15][0]++;
          zones[15][zones[15][0]] = i;
        }
      } else if (playerPos[i].getX()
                 > Constants.cornerInfDer.getX() - Constants.ANCHO_CAMPO_JUEGO / 3) {
        if (playerPos[i].getY() < Constants.cornerInfIzq.getY() + Constants.LARGO_CAMPO_JUEGO / 6) {
          zones[2][0]++;
          zones[2][zones[2][0]] = i;
        } else if (playerPos[i].getY()
          ...
          zones[17][zones[17][0]] = i;
                 ...
                 < Constants.cornerInfIzq.getY() + Constants.LARGO_CAMPO_JUEGO / 6) {
        zones[1][0]++;
        zones[1][zones[1][0]] = i;
      } else if (playerPos[i].getY()
                 < Constants.cornerInfIzq.getY() + (Constants.LARGO_CAMPO_JUEGO / 6) * 2) {
  ...
  /**
   * Devuelve la posicion que debe situarse el defensor a partir de la posicion del atacante
   */
  ...
  /**
   * Recuperción de balon y cuantos queremos q vallan Devuelve int[] de los jugadores que van
   */
  public int[] catchBall(GameSituations mySp, int howMany) {
    ...
    if (isPosible) {
      ...
      for (int i = 0; i < howMany; i++) {
        ...
        myPlayers[select[i]] =
            new Position(jty.rebounds.get(jaux).getX(), jty.rebounds.get(jaux).getY());
  ...
  public void brains(GameSituations mySp) {
    ...
    double
        ballAngle =
        Math.acos(
            ((dPorter * dPorter) - (dRight * dRight) - (dLeft * dLeft)) / (-2 * dRight * dLeft));
    double
        rightAngle =
        Math.acos(
            ((dLeft * dLeft) - (dPorter * dPorter) - (dRight * dRight)) / (-2 * dPorter * dRight));
    ...
    myPlayers[0] =
        new Position((Constants.posteDerArcoInf.getX() - dPorterRight), alineacion1[0].getY());
    ...
    for (int i = 0; i < pty.rebounds.size(); i++) {
      if (Math.abs(pty.rebounds.get(i).getX()) < Constants.posteDerArcoInf.getX() &&
          pty.rebounds.get(i).getY() < (Constants.centroArcoInf.getY()
    ...
    if (mySp.ballPosition().getY() < 0) {
      commands
          .add(new CommandMoveTo(5, new Position(-Constants.RADIO_CIRCULO_CENTRAL, 0 - yadd * 2)));
      commands
          .add(new CommandMoveTo(7, new Position(Constants.RADIO_CIRCULO_CENTRAL, 0 - yadd * 2)));
      catchBall(mySp, 1);
      for (int i = 0; i < rivalPos.length; i++) {
        if (rivalPos[i].getX() < (Constants.centroArcoInf.getX()
        ...
        } else if (rivalPos[i].getX() > (Constants.centroArcoInf.getX()
                                         + Constants.LARGO_AREA_CHICA / 2)) {
  ...
  public class MyTrajectory {
    ...
    public ArrayList<Position>
        rebounds;
        //puntos donde será recuperable (si va a ras pos seran todos)
    public ArrayList<Integer>
        iterations;
        //iteración asociada a cada punto (importante para interceptar)
  ...
  public MyTrajectory predictedTrajectory(GameSituations mySp, boolean porter) {
    ...
    for (int i = 0; i != -1; i++) {
      ...
      if (myPoint[0] == mySp.getTrajectory(i + 1)[0] && myPoint[1] == mySp
          .getTrajectory(i + 1)[1]) {
        break;
      }
