...
//		getTipoAlineacion();
  ...
  public void setearPorteroRival() {
    int i;
    for (i = 0; i < sp.rivalPlayersDetail().length && !sp.rivalPlayersDetail()[i].isGoalKeeper();
         i++) {
      ;
    }
    porteroRival = i;
    ...
    if (sp.rivalGoals() > sp.myGoals()) {
      tipo = Constantes.ALINEACION_INICIAL;
               ...
               && Constants.ITERACIONES - sp.iteration()
                  <= Constantes.NUMERO_INTERACIONES_RESTANTES_DEFENDER) {
  ...
  public Position jugadorAdelantadoMasCercano(Position punto) {
    ...
    for (Position p : jug) {
      ...
      if ((distancia == 0 || aux < distancia) && !isInside(p, margen / 2, margen / 2,
                                                           sp.rivalPlayers())) {
        distancia = aux;
        posOptima = p;
      }
...
//					if (alineaciones.getMedios().contains(i) || alineaciones.getDelanteros().contains(i)) {
      ...
      for (int i : sp.canKick()) {
        if (!isInsideAreaPropia(sp.ballPosition())) {
          if (alineaciones.getDefensores().contains(i) || alineaciones.getMedios().contains(i)
              || alineaciones.getDelanteros().contains(i)) {
            calcularSiguientePosicion(i, comandos);
          }
        }
      }
...
//			position = new Position(jug.getX() + margen,jug.getY()+ margen);
  ...
  /**
   * Devuelve el cuadrante donde se encuentra el punto p respecto del punto de referencia. En el
   * orden de las agujas del reloj empezando por Arriba derecha
   *
...
//				System.out.println(" el rival mas cercano es : " + rivalesCercanos[i]);
  ...
  private Position avanzaLineaRecta(Position jug, int margen) {
    ...
    if ((-1) * Constants.esqSupIzqCampoJuego.getY() - margen < jug.getY()
        && jug.getX() < (-1) * Constants.esqSupIzqCampoJuego.getY()) {
    ...
    } else if (Constants.esqSupIzqCampoJuego.getX() - margen < jug.getX()
               && jug.getX() < Constants.centroArcoSup.getX()
               && jug.getY() > Constants.centroArcoSup.getY() * 2 / 3) {
      // Estoy en la banda derecha
      p = new Position(jug.getX() + margen, jug.getY() + margen);
    } else if (Constants.centroArcoSup.getX() < jug.getX()
               && jug.getX() < -Constants.esqSupIzqCampoJuego.getX() - margen
               && jug.getY() > Constants.centroArcoSup.getY() * 2 / 3) {
      // Estoy en la banda izquierda
      p = new Position(jug.getX() - margen, jug.getY() + margen);
    } else {
      // SAQUE DE BANDA ??
      if (sp.isStarts() && Constants.esqSupIzqCampoJuego.getX() < jug.getX()
          && jug.getX() < Constants.centroArcoSup.getX()) {
        p = new Position(jug.getX() + margen, jug.getY() + margen);
      } else if (sp.isStarts() && Constants.centroArcoSup.getX() < jug.getX()
                 && jug.getX() < -Constants.esqSupIzqCampoJuego.getX()) {
        p = new Position(jug.getX() - margen, jug.getY() + margen);
      } else {
  ...
  private boolean isInside(Position referencia, int margenX, int margenY, Position[] pos) {
    ...
    for (Position p : pos) {
      resultado =
          resultado || (minX < p.getX() && p.getX() < maxX && minY < p.getY() && p.getY() < maxY);
    }
  ...
  public void situarPortero(LinkedList<Command> comandos) {
    ...
    if (sp.isStarts() && isInsideAreaPropia(posBalon)) {
      if (inicioRetrasoSaquePorteria == -1) {
        ...
        comandos.add(despejar(alineaciones.getPortero(),
                              sp.myPlayers()[(Integer) alineaciones.getExtremos().get(randon)]));
        inicioRetrasoSaquePorteria = -1;
      } else {
...
//	        	System.out.println("############### CONT " + cont);
        ...
        Position
            centroP =
            new Position(Constants.centroArcoInf.getX(), Constants.centroArcoInf.getY() - 1);
  ...
  public void comandosDefensa(LinkedList<Command> comandos) {
    ...
    for (int i : sp.canKick()) {
      if ((i == alineaciones.getPortero() && sp.isStarts() && isInsideAreaPropia(sp.ballPosition())
           && inicioRetrasoSaquePorteria == Constants.ITERACIONES_SAQUE - 2)
          || (i == alineaciones.getPortero() && !sp.isStarts()) || alineaciones.getDefensores()
          .contains(i)) {
  ...
  public int elegirMejorDelantero(int j) {
    ...
    for (int i = 0; i < alineaciones.getDelanteros().size() && !res; i++) {
      if (i != j) {
        ...
        res =
            isInsideAreaContraria(sp.myPlayers()[index]) && isInside(sp.myPlayers()[index], 3, 3,
                                                                     sp.rivalPlayers());
      }
    }
    ...
    for (int i : sp.canKick()) {
      if (alineaciones.getDelanteros().contains(i)) {
        if (isSaqueCentro()) {
          ...
          if (isInsideAreaContraria(jugadores[i]) || i == 0 || !isInsideAreaPequeñaContraria(
              posicionPorteroContrario)) {
...
//		}else {
    ...
    } else if (Constants.posteIzqArcoSup.getX() < jug.getX()
               && jug.getX() < Constants.posteDerArcoSup.getX()) { // Centrados
      p = calcularPtoTiro(j, jug, porteroRival, 1, -1);
    } else { // A la derecha
      p = calcularPtoTiro(j, jug, porteroRival, 1, 1);
    }
...
//		}
  ...
  public Position calcularPtoTiro(int j, Position jug, Position porteroRival, int signo1,
                                  int signo2) {
    Position p;
    ...
    double angGrados;
    double angMasError;
    double angMenosError;
    double angError2 = sp.myPlayersDetail()[j].getPrecision() / 100 * 180 / 2;
    double
        angError =
        Math.toDegrees(Constants.getErrorAngular(sp.myPlayersDetail()[j].getPrecision()));
    double angDelta;
    double angOmega;
    ...
    double p2;
    double distPortero;
    double distPorteroAux;
    Position
        pRivalIzq =
        new Position(porteroRival.getX() - Constants.DISTANCIA_CONTROL_BALON_PORTERO,
    ...
    if (pRivalDer.distance(Constants.posteDerArcoSup) > pRivalIzq
    ...
    for (double i = Constants.posteIzqArcoSup.getX(); i < Constants.posteDerArcoSup.getX();
         i = i + 0.25) {
