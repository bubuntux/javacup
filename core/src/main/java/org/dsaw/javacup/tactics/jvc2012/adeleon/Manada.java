...
public class Manada implements Tactic {
  ...
  class TacticDetailImpl implements TacticDetail {
    ...
    public PlayerDetail[] getPlayers() {
      return new PlayerDetail[]{
          new JugadorImpl("Howard", 1, new Color(255, 200, 150), new Color(50, 0, 0), 1.0d, 1.0d,
                          1.0d, true),
          new JugadorImpl("Errasti", 2, new Color(255, 200, 150), new Color(50, 0, 0), 1.0d, 1.0d,
                          1.0d, false),
          new JugadorImpl("Evans", 3, new Color(255, 200, 150), new Color(50, 0, 0), 1.0d, 1.0d,
                          1.0d, false),
          new JugadorImpl("Santillana", 4, new Color(255, 200, 150), new Color(50, 0, 0), 1.0d,
                          1.0d, 1.0d, false),
          new JugadorImpl("Lozano", 5, new Color(255, 200, 150), new Color(50, 0, 0), 1.0d, 0.5d,
                          0.5d, false),
          new JugadorImpl("Marrokin", 6, new Color(255, 200, 150), new Color(50, 0, 0), 1.0d, 0.5d,
                          0.5d, false),
          new JugadorImpl("Teran", 7, new Color(255, 200, 150), new Color(50, 0, 0), 1.0d, 0.5d,
                          0.5d, false),
          new JugadorImpl("Luisillo", 8, new Color(102, 51, 0), new Color(50, 0, 0), 1.0d, 1.0d,
                          1.0d, false),
          new JugadorImpl("Vila", 9, new Color(255, 200, 150), new Color(50, 0, 0), 1.0d, 0.5d,
                          0.5d, false),
          new JugadorImpl("De Leon", 10, new Color(255, 200, 150), new Color(50, 0, 0), 1.0d, 1.0d,
                          0.5d, false),
          new JugadorImpl("Cantu", 11, new Color(255, 200, 150), new Color(50, 0, 0), 1.0d, 0.5d,
                          0.5d, false)
      };
    }
  }
  ...
  public boolean filtrarCentro(Position p1, Position[] enemigos) {
    ...
    for (int i = 0; i < enemigos.length; i++) {
      if ((enemigos[i].getX() > p1.getX() - 6 && enemigos[i].getX() < p1.getX() + 6) && (
          enemigos[i].getY() > p1.getY() - 3 && enemigos[i].getY() < p1.getY() + 20)) {
        return false;
      }
    }
  ...
  public boolean filtrarDerecha(Position p1, Position[] enemigos) {
    ...
    for (int i = 0; i < enemigos.length; i++) {
      if ((enemigos[i].getX() > p1.getX() - 3 && enemigos[i].getX() < p1.getX() + 10) && (
          enemigos[i].getY() > p1.getY() - 3 && enemigos[i].getY() < p1.getY() + 20)) {
        return false;
      }
    }
  ...
  public boolean filtrarIzquierda(Position p1, Position[] enemigos) {
    ...
    for (int i = 0; i < enemigos.length; i++) {
      if ((enemigos[i].getX() > p1.getX() - 10 && enemigos[i].getX() < p1.getX() + 3) && (
          enemigos[i].getY() > p1.getY() - 3 && enemigos[i].getY() < p1.getY() + 20)) {
        return false;
      }
    }
  ...
  public boolean porteriaLibre(Position[] enemigos) {
    ...
    for (int i = 0; i < enemigos.length; i++) {
      if (enemigos[i].getX() > -5 && enemigos[i].getX() < 5 && enemigos[i].getY() > 48
          && enemigos[i].getY() < 53) {
  ...
  public List<Command> execute(GameSituations sp) {
    ...
    if (recuperadores.length > 0 && tiradores.length == 0) {
      ...
      for (int i = 1; i < recuperadores.length; i++) {
        commands
            .add(new CommandMoveTo(recuperadores[i], new Position(trayectoria[0], trayectoria[1])));
      }
    }

    if (tiradores.length > 0 && tiradores[0] != 7 && sp.ballPosition().getX() > -3
        && sp.ballPosition().getX() < 3 && sp.ballPosition().getY() > -3
        && sp.ballPosition().getY() < 3) {
      for (int i = 0; i < tiradores.length; i++) {
        commands.add(new CommandHitBall(tiradores[i], jugadores[0], 1, true));
      }
    } else if (tiradores.length > 0 && tiradores[0] != 7
               && estaLibre(jugadores[7], rivales) == false && tiradores[0] != 0
               && tiradores[0] != 1 && tiradores[0] != 2 && tiradores[0] != 3) {
      for (int i = 0; i < tiradores.length; i++) {
    ...
    if (tiradores.length > 0 && tiradores[0] == 7 && sp.ballPosition().getY() > 43) {
      commands.add(new CommandHitBall(7, new Position(Constants.posteDerArcoSup.getX() - 2,
                                                      Constants.posteDerArcoSup.getY()), 1, 19));
    }
