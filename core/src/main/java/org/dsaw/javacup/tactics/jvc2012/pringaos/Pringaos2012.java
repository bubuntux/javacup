    ...
    while ((!found) && (it < 100)) {
      ...
      if (posBalon[2] <= Constants.ALTO_ARCO) {
        for (i = 0; i < sp.rivalPlayers().length; i++) {
          if (posBalon[2] <= (sp.rivalPlayersDetail()[i].isGoalKeeper() ? Constants.ALTO_ARCO
                                                                        : Constants.ALTURA_CONTROL_BALON)) {
    ...
    for (angulo = 0; angulo <= Constants.ANGULO_VERTICAL_MAX; angulo += 10) {
      ...
      for (fuerza = 0.2f; fuerza <= 1; fuerza += 0.2f) {
        ...
        for (anguloH = 0.0f; anguloH < 360; anguloH += 10) {
          tra =
              new Trayectoria(p1, Constants.getVelocidadRemate(fuerzaTotal),
          ...
          if ((iteraciones != null) &&
            ...
            if ((iteraciones[0] < iteracionesrival) || ((iteracionesrival == -1) && (iteraciones[0]
                                                                                     != -1))) {

              pases.add(new Pases(
                  new Position(tra.getPos(iteraciones[0])[0], tra.getPos(iteraciones[0])[1]),
                  0,
                  angulo,
                  fuerza,
                  iteraciones[0],
