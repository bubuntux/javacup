...
public class Utilitarios {
    ...
    /**
     * Retorna las distancia de cada uno de los jugadores a la posición indicada
     * ordenadas de menor a mayor.
     *
     * @param posicion La posición de referencia.
     * @param jugadores Las posiciones de los jugadores.
     * @return Una arreglo de <code>double</code> en donde la primer posición es
     *         el indice del jugador y la segunda posición es la distancia al
     *         punto de referencia. El arreglo esta ordenado por las distancias
     *         siendo el primero elemento el jugador mas cercano y el ultimo el
     *         mas lejano a la posición de referencia.
     */
    ...
    /**
     * Indica si en la iteraciòn anterior se efectuó un remate con base en los
     * datos proporcionados
     * @param puedenRematarItAnterior Arreglo con los indices de los jugadores
     *        que podían efectuar un remate en la iteración anterior. Se puede
     *        obtener invocando a {@link GameSituations#canKick()} ó
     *        {@link GameSituations#rivalCanKick()}, dependiendo de si
     *        se desea saber si hubo un remate propio o de los rivalPlayers.
     * @param iteracionesParaRematarItActual Arreglo con las iteraciones que 
     *        restan para que los rivalPlayers puedan rematar en la iteración actual.
     *        Se puede obtener invocando a {@link 
     *        GameSituations#iterationsToKick()} ó {@link
     *        GameSituations#rivalIterationsToKick()}, dependiendo de si
     *        se desea saber si hubo un remate propio o de los rivalPlayers.
     * @return <code>true</code> si en la iteración anterior se efectuó un 
     *         remate, <code>false</code> de lo contrario.
     */
    public static boolean huboRemate(int[] puedenRematarItAnterior, int[] iteracionesParaRematarItActual) {
     ...
     * @param posicionBalonAnterior Posición del balón en la iteración anterior. Se puede obtener desde
     *        el método {@link SituacionPartidoCache#ballPosition()}.
	...
	for ( int iteraciones : situacion.iterationsToKick())
	    if( iteraciones > iteracionesMaximoPropias ) iteracionesMaximoPropias = iteraciones;

	for ( int iteraciones : situacion.rivalIterationsToKick())
	    if( iteraciones > iteracionesMaximoRival ) iteracionesMaximoRival = iteraciones;
    ...
    /**
     * Indica el cuarto de cancha en el cual se encuentra el balón, de la siguiente manera:
     * <ul>
	...
	if( posicioBalon.getY() < lineaPrimerCuarto ) return PRIMER_CUARTO_CAMPO;
	else if( posicioBalon.getY() < lineaSegundoCuarto ) return SEGUNDO_CUARTO_CAMPO;
	else if( posicioBalon.getY() < lineaTercerCuarto ) return TERCER_CUARTO_CAMPO;
     ...
     */
    public static double obtenerPendienteRemate(Position posicionBalon, Position posicionBalonAnterior) {
	return ((posicionBalonAnterior.getY() - posicionBalon.getY())/(posicionBalonAnterior.getX() - posicionBalon.getX()));
    }
