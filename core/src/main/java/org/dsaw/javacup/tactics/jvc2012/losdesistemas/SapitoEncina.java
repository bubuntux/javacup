...
public class SapitoEncina {
  ...
  private ElTurboVargas shoot;
  private Position[] players;
  private Position[] opponents;
  private int[] iterToShoot;

  public SapitoEncina(List<Command> comandos, Datines data) {
    this.comandos = comandos;
    this.data = data;
    shoot = new ElTurboVargas(comandos, data);
  }

  public void execute(GameSituations sp, Mentalidad mentality) {
    this.sp = sp;
  ...
  private double getThreshold(Mentalidad mentality) {
    if (mentality == Mentalidad.Normal) {
      return .975;
    }
    if (mentality == Mentalidad.Offensive) {
      return .985;
    }
    return .995;
  }
  ...
  private void pass(Position[] players, int index, PassInfo passInfo) {
    if (passInfo.exist) {
      double
          power =
          passInfo.velocity / Constants.getVelocidadRemate(sp.myPlayersDetail()[index].getPower());
      if (sp.isStarts()) {
        power /= .75;
      }
      comandos.add(
          new CommandHitBall(index, passInfo.position, power, passInfo.angleVer * 180 / Math.PI));
    }
  }
  ...
  private PassInfo calculatePass(int[] canShoot) {
    ...
    if (power > 0.65) {
      pass = comparePass(pass,
                         ...
                         calculatePass(Constants.getVelocidadRemate(0.65), 3 * maxVertAngle / 4,
                                       false),
                         ...
                         calculatePass(Constants.getVelocidadRemate(0.65), maxVertAngle / 8,
                                       false));
    }
    if (power > 0.35) {
      pass = comparePass(pass,
                         calculatePass(Constants.getVelocidadRemate(0.35), 0, false),
                         calculatePass(Constants.getVelocidadRemate(0.35), 3 * maxVertAngle / 4,
                                       false),
                         ...
                         calculatePass(Constants.getVelocidadRemate(0.35), maxVertAngle / 8,
                                       false));
    }
    return pass;
  }
  ...
  private PassInfo calculatePass(double velocity, double angleVer, boolean forward) {
    ...
    Leandorffffffff[] trajectory = new Leandorffffffff[n];
    for (int i = 0; i < n; i++) {
      trajectory[i] =
          new Leandorffffffff(sp.ballPosition(), velocity, i * dAngle * Math.PI / 180, angleVer);
    }
    ...
    for (int i = 0; i < m; i++) {
      for (int j = 0; j < trajectory[i].length; j++) {
        if (j == 0) {
          fitness =
              calculateFitness(trajectory[i].positions[j], trajectory[i].z[j], j + 1,
                               sp.ballPosition(), 0);
        } else {
          fitness =
              calculateFitness(trajectory[i].positions[j], trajectory[i].z[j], j + 1,
                               trajectory[i].positions[j - 1], trajectory[i].z[j - 1]);
        }
        if (fitness != 0) {
  ...
  private double calculateFitness(Position position, double z, int iter, Position last,
                                  double lastZ) {
    int
        oppIterToBall =
        data.calculateIterToBall(position, z, opponents, sp.rivalPlayersDetail(),
    ...
    if (!position.isInsideGameField(0)) {
      if (position.getY() > Constants.LARGO_CAMPO_JUEGO / 2) {
        double
            posX =
            (Dx / Dy) * (Constants.LARGO_CAMPO_JUEGO / 2 - position.getY()) + position
                .getX();//proyeccion x de la trayectoria del ballPosition en la linea de meta
        double Dz = z - lastZ;
        double
            posZ =
            (Dz / Dy) * (Constants.LARGO_CAMPO_JUEGO / 2 - position.getY())
        ...
        if (posZ <= Constants.ALTO_ARCO &&
          ...
          double
              fx =
              Math.max(0, Math.min(1, 1 - Math.abs(posX) / (Constants.LARGO_ARCO / 2
    ...
    int
        iterToBall =
        data.calculateIterToBall(position, z, players, sp.myPlayersDetail(), iterToShoot);
    if (oppIterToBall <= iter) {
      double
          fdist =
          Math.max(0, 1 - Constants.penalSup.distance(position) / Constants.penalSup
              .distance(Constants.cornerInfDer));
