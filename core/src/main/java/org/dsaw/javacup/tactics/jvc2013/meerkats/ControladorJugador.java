...
public class ControladorJugador {
    ...
    /**
     * Número experimental de iteraciones por cada dribbling obtenido al
     * utilizar la constante Garzón.
     */
	...
	if (sp.ballPosition().getY() < 0d) {
		...
		if (rematador == arquero) {
			...
			double[] fuerzaAnguloRechazo = obtenerFuerzaAnguloPase(arquero, sp.myPlayers()[Alineacion.INDICE_VOLANTE_D]);
			comandos.add(new CommandHitBall(arquero,
				sp.myPlayers()[Alineacion.INDICE_VOLANTE_D], fuerzaAnguloRechazo[0], fuerzaAnguloRechazo[1]));
		    } else {
			double[] fuerzaAnguloRechazo = obtenerFuerzaAnguloPase(arquero, sp.myPlayers()[Alineacion.INDICE_VOLANTE_I]);
			comandos.add(new CommandHitBall(arquero,
				sp.myPlayers()[Alineacion.INDICE_VOLANTE_I], fuerzaAnguloRechazo[0], fuerzaAnguloRechazo[1]));
		    }
		    return;
		}
	    }
	...
	for (int i = 0; i < Utilitarios.NUMERO_JUGADORES; i++) {
		...
		posicionDesplazamiento = ( posicionDesplazamiento == null ? referenciasDrible[0] : posicionDesplazamiento );
    ...
    /**
     * Asigna la situación del partido, el cache de la situación del partido y
     * ls lista de comando a manipular.
     * 
     * @param sp Objeto {@link GameSituations} que representa la situación del
     *        partido en la iteración actual.
     ...
     * @param cuartoCancha cuarto de cancha en el cual se encuentra el balón actualmente. Toma los 
     * valores {@link Utilitarios#PRIMER_CUARTO_CAMPO}, {@link Utilitarios#SEGUNDO_CUARTO_CAMPO},
     * {@link Utilitarios#TERCER_CUARTO_CAMPO} o {@link Utilitarios#CUARTO_CUARTO_CAMPO}.
     * @param jugadoresMarcados Lista {@link LinkedList} con los indices de los rivalPlayers que ya recibieron marcaje.
     */
    public void update( GameSituations sp, SituacionPartidoCache spItAnterior,
            LinkedList<Command> comandos, int cuartoCancha, LinkedList<Integer> jugadoresMarcados, boolean atacando) {
	this.sp = sp;
    ...
    /**
     * Calcula la posición del arquero de acuerdo a la posición de la pelota en nuestro lado de la cancha.
     *
	...
	if( balonAlArco ) {
		...
		double yRetroceder = posicionInterceptacionArquero.getY();
		...
		if( yRetroceder > -Constants.LARGO_CAMPO_JUEGO/2)
		    posicionInterceptacionArquero = posicionInterceptacionArquero.setPosition(xRetroceder, yRetroceder);}

	    return posicionInterceptacionArquero;
	}

	// Se calculan las dos raices y sus dos valores en y
	double x1 = (-6.71d * m - Math.sqrt(95.68d + 140.71d * Math.pow(m, 2)))/(2 * (1 + Math.pow(m, 2)));
	double x2 = (-6.71d * m + Math.sqrt(95.68d + 140.71d * Math.pow(m, 2)))/(2 * (1 + Math.pow(m, 2)));
	double y1 = m * x1 - 52.5d;
	double y2 = m * x2 - 52.5d;

	if ( y1 > y2 ) posicionArquero = posicionArquero.setPosition(x1, y1);
	else posicionArquero = posicionArquero.setPosition(x2, y2);
	...
	if(jugadorImpl.esControlador()) {
		...
		comandos.add(new CommandHitBall(jugador, sp.ballPosition().movePosition(0d, 0.1d), 0.1d, 0d));
    ...
    /**
     * Verifica que el control del balón es seguro, es decir, que el jugador va a poder controlar
    ...
    /**
     * Verifica que el dribbling es seguro, es decir, que el jugador va a poder driblar sin perder el balón.
     * 
     * @param posicion Objeto {@link Posicion} que representa la posición del jugador que va a driblar.
	...
	double[][] distanciaJugadores = Utilitarios.distanciaJugadores(posicionPaseJugador, sp.rivalPlayers());
     ...
     * 
     * @param jugador Índice del jugador que dribla, sobre el cual se desea hacer la evalaución de marca.
	...
	double fuerzaDribbling = CONSTANTE_GARZON * (jugadorTemp.getSpeed() * 0.25d + 0.25d) / (jugadorTemp.getPower() * 1.2d + 1.2d); //jugadorTemp.getPower()/CONSTANTE_GARZON;
     ...
     * @return Un arreglo de dos posiciones en donde la primera posición es la
     *         fuerza óptima del pase y la segunda posición es el ángulo óptimo
     *         del pase.
     */
	...
	for (int i = 0; i < referenciasPase.length; i++) {
		...
		if((sp.ballPosition().getX() - posicionPaseJugador.getX()) >= 0) {
		    posicionPaseJugador = posicionPaseJugador.movePosition(jugadorImpl.getProyeccionPase().getX(), jugadorImpl.getProyeccionPase().getY());
		} else {
		    posicionPaseJugador = posicionPaseJugador.movePosition(-jugadorImpl.getProyeccionPase().getX(), jugadorImpl.getProyeccionPase().getY());
		}
	    } 
	...
	boolean rechaceEfectuado = false;
