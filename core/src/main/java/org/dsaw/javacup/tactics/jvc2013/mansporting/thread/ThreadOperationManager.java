...
public class ThreadOperationManager {
   ...
   */
  private <ID, RESULT> List<IdentificableFuture<ID, RESULT>> internalExecuteThreaded(
      List<? extends IdentificableCallable<ID, RESULT>> operations) {
    ...
    try {
      List<IdentificableFuture<ID, RESULT>>
          futures =
          new LinkedList<>();
      for (IdentificableCallable<ID, RESULT> callable : operations) {
        futures.add(new ThreadOperationFuture<ID, RESULT>(callable.getId(),
   ...
   * @return resultados de las operaciones.
   */
  public <ID, RESULT> List<IdentificableData<ID, RESULT>> executeThreaded(
      List<? extends IdentificableCallable<ID, RESULT>> operations) {
    try {
      List<IdentificableData<ID, RESULT>> results = new LinkedList<>();
      if (!MSGConstants.EXECUTE_THREADED) {
