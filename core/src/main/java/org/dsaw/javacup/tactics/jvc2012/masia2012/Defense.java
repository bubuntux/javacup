...
public class Defense {
  ...
  public void execute() {
    ...
    if (!analysis.attacking) {
      if (analysis.actualPossession > 0.65) {
        ...
        defenseCount +=
            Math.ceil(extraDefenseCount * (1 - Math.max(0,
                                                        Math.min(analysis.actualPossession - 0.55,
                                                                 0.20)) / 0.20));
      } else {
        defenseCount = 9;
      }
  ...
  private int recoveryBall() {
    ...
    while (i < analysis.rivalIterationToBall && i < analysis.ballPositions.length) {
      if (analysis.ballPositions[i].isInsideGameField(0)) {
        if (analysis.myPlayersDetail[analysis.playerClosestToBall].isGoalKeeper()
            && Math.abs(analysis.ballPositions[i].getX()) <= Constants.LARGO_AREA_GRANDE / 2
        ...
        if (height >= analysis.ballZ[i]) {
          int
              it =
              (int) Math.ceil(Math.max(0, analysis.myPlayers[analysis.playerClosestToBall]
                                              .distance(analysis.ballPositions[i])
                                          - ballControlDistance) / Constants.VELOCIDAD_MAX);
          if (it <= i) {
            if (analysis.ballPositions[i].distance(analysis.ballPositions[i - 1])
                < 2 * ballControlDistance) {
              commands.add(new CommandMoveTo(analysis.playerClosestToBall, Position
                  .medium(analysis.ballPositions[i - 1], analysis.ballPositions[i])));
  ...
  private int defensePosition(PositionsDefend defend, int iterations,
    ...
    Position[]
        intersection =
        calculateIntersection(defend.position, analysis.rivalPlayersDetail[rival].getSpeed());
    ...
    for (int i = 0; i < intersection.length; i++) {
      ...
      int
          iterationsToPosition =
          (int) Math.ceil(analysis.myPlayers[defender].distance(intersection[i]) / Constants
              .getVelocidad(analysis.myPlayersDetail[defender].getSpeed()));
      ...
      if (iterationsToPosition <= iterations + i) {
        if (defend.isBallPosition) {
          ...
          commands.add(new CommandMoveTo(defender, new Position(intersection[i].getX(),
                                                                Math.min(intersection[i].getY(),
                                                                         0))));
          ...
          double
              ratio =
              Math.max(analysis.ball.distance(defend.position) / Constants.REMATE_VELOCIDAD_MAX
                       * Constants.VELOCIDAD_MAX - defend.position.distance(intersection[i]), 0);
          commands.add(new CommandMoveTo(defender, intersection[i]
              .moveAngle(intersection[i].angle(Constants.centroArcoInf), ratio)));
        }
        return defender;
      }
    ...
    if (defend.isBallPosition || bestDefender > analysis.mentality.centralDefenseCount) {
      ...
      commands.add(
          new CommandMoveTo(bestDefender, new Position(best.getX(), Math.min(best.getY(), 0))));
    }
    return bestDefender;
  }
  ...
  class PositionsDefend implements Comparable<PositionsDefend> {
    ...
    public int compareTo(PositionsDefend arg0) {
      return ((Double) position.distance(Constants.centroArcoInf))
          .compareTo(arg0.position.distance(Constants.centroArcoInf));
    }
  }
}
