...
/**
 * <br> Clase que representa un element. <br> <br>Copyright Fi2net S.A. 10-ene-2011
 */
public class VectorDrawable extends AbstractDrawable<Segmento> {
  ...
  public void paint(VisualDebugGraphics g, Rectangulo canvas) {
    ...
    double
        x =
        element.getPuntoB().getX() + (canvas.getCentro().getX() + canvas.getAncho() / 2) - RADIUS;
    double
        y =
        -element.getPuntoB().getY() + (canvas.getCentro().getY() + canvas.getAlto() / 2) - RADIUS;
