...
//					TODO RGB si no tengo a quien pasar, que hago?? paso al hueco!!--> tengo que hacerlo
					comandos.add(new CommandMoveTo(jugador, direccionCorrer(jugadores[jugador], sp.rivalPlayers())));
...
//						va a por el balón el lateral correspondiente (10 --> 7 y 11 --> 8)
						...
						int[] iteracionesParaRematarRivalAux = Arrays.copyOf(sp.rivalIterationsToKick(), sp.rivalIterationsToKick().length);
...
//							TODO RGB no va a por la bola, se pone entre la posición de remate del rival y el centro de la portería
     ...
     * @param r
     */
    private void setComandosDelanteros(Position[] jugadores, int jugador, GameSituations sp, Random r){
...
//    	TODO RGB Comprobar la distancia a la que los tiros a puerta efectivos
		if ( distanciaArco < 36.0D ){
			final Position dondeRematar = getADondeRematar(jugadores[jugador], sp.rivalPlayers(), sp.rivalPlayersDetail());
...
//			calculamos el ángulo portero-rematador-palo_derecho
			final double anguloPRD = pRematador.angle(portero) - pRematador.angle(Constants.posteDerArcoSup);
//			calculamos el ángulo portero-rematador-palo_izquierdo
			final double anguloPRI = pRematador.angle(portero) - pRematador.angle(Constants.posteIzqArcoSup);
...
//				Si hay más hueco por la izquierda (ángulo izquierdo mayor) tiramos con a 3/4 del portero hacia la izquierda
    ...
    private Position[] getCubridores(Position[] rivales, Position jugador, Position compa, double distanciaCompa){
    	final double anguloCompa = jugador.angle(compa);
    	
    	List<Position> cubridores = new ArrayList<Position>();
     ...
     * @return
     */
    private int getJugadorDestino(Random r, int tirador, Position[] jugadores, GameSituations sp, int[] jugadoresDestino ){
    	int count = 0;
