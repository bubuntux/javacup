...
/**
 * Clase utilizada para generar todas las acciones que puede realizar el jugador que se encuentra en
 * contacto con el balï¿½n.
 *
...
public class Generador {
  ...
  public AccionAvanzar avanzar(GameSituations sp, Jugador jugador, Rival[] arrayRivales) {
    ...
    try {
      ...
      Double
          angulo =
          avanzarRango(sp, jugador, arrayJugadores, arrayRivales, 30, 0,
      ...
      if (angulo != null) {
        ...
        accion = new AccionAvanzar(new CommandHitBall(
            ...
            jugador.getFuerzaGolpeoAvanzar(),
            false),
                                   new CommandMoveTo(jugador.getIndice(),
            ...
            do {
              ...
              if (paseAdelantado > 0) {
                ...
                posicionPase = new Position(sp.myPlayers()[jugadorDestino.getIndice()].getX(),
                                            Math.min(
                                                (sp.myPlayers()[jugadorDestino.getIndice()].getY()
                                                 + 5),
              ...
              if (distanciaPase < jugador.getDistanciaGolpeoBajo()) {
                ...
                anguloZ = 30;
                fuerza =
                    ((Entorno.FUERZA_MAXIMA_GOLPEO * distanciaPase) / jugador
                        .getDistanciaGolpeoAlto()) + 0.2;
              }
              ...
              if (paseSeguro(sp, arrayJugadores, arrayRivales,
                             jugador, sp.myPlayers()[jugadorDestino.getIndice()],
                             (paseAdelantado + 1))) {
                ...
                // Si nos encontramos en una acciï¿½n ofensiva calculamos el mejor pase.
                  ...
                  if (jugadorAsignado != null) {
                    boolean
                        puertaVaciaActual =
                        puertaVacia(sp, sp.myPlayers()[jugadorDestino.getIndice()]);
  ...
  public AccionTirar tirar(GameSituations sp, Jugador jugador, boolean calculoParaJugadorPosesion,
    ...
    try {
      ...
      Position
          inicioTiro =
          calculoParaJugadorPosesion ? sp.ballPosition() : sp.myPlayers()[jugador.getIndice()];
      ...
      if (jugador.getDistanciaGolpeoAlto() > distanciaPorteria &&
        ...
        if (tabla != null) {
          ...
          accion = new AccionTirar(new CommandHitBall(jugador.getIndice(),
                                                      ...
                                                      (Double) tabla.get("fuerza"),
                                                      //Entorno.FUERZA_MAXIMA_GOLPEO,
                                                      (Double) tabla.get("anguloZ")),
  ...
  public AccionDespejar despejar(GameSituations sp, Jugador jugador, Rival[] arrayRivales) {
    ...
    try {
      ...
      double
          fuerza =
          (sp.ballPosition().getY() > 0
           && sp.myPlayersDetail()[jugador.getIndice()].getPower() == 1) ? 0.8
                                                                         : Entorno.FUERZA_MAXIMA_GOLPEO;
      ...
      do {
        ...
        if (j.getY() > sp.ballPosition().getY()) {
          ...
          if (sp.ballPosition().distance(j) > (jugador.getDistanciaGolpeoAlto() / 2)) {
            ...
            realizaDespeje =
                alcanzoAlturaDespeje(sp, jugador, arrayRivales, angulo, anguloVertical, fuerza);
          }
        }
  ...
  public AccionRegatear regatear(GameSituations sp, Jugador jugador, Rival[] arrayRivales) {
    ...
    try {
      ...
      anguloRegate =
          avanzarRango(sp, jugador, arrayJugadores, arrayRivales, 180, 0,
                       jugador.getFuerzaGolpeoAvanzar());
      ...
      if (anguloRegate != null) {
        ...
        double
            anguloMax =
            (sp.ballPosition().getX() < 0) ? Math
                .toDegrees(sp.ballPosition().angle(Constants.posteIzqArcoSup)) :
            180;
        ...
        boolean
            regateHaciaPorteria =
            anguloRegateFormateado < anguloMax && anguloRegateFormateado > anguloMin;
  ...
  public AccionPasarAlHueco pasarAlHueco(GameSituations sp, Jugador jugador,
    ...
    try {
      ...
      double
          yMin =
          (sp.ballPosition().getY() > 0) ? sp.ballPosition().getY() - 20 : sp.ballPosition().getY();
      ...
      do {
        ...
        if (distanciaPase < jugador.getDistanciaGolpeoBajo()) {
          ...
          anguloZ = 30;
          fuerza =
              (Entorno.FUERZA_MAXIMA_GOLPEO * distanciaPase) / jugador.getDistanciaGolpeoAlto();
        }
        ...
        if (alcanzoBalon) {
          ...
          double
              xAreaIzquierda =
              -(Constants.ANCHO_CAMPO_JUEGO / 2 - Constants.LARGO_AREA_GRANDE / 2);
          ...
          if (sp.ballPosition().getX() < xAreaIzquierda) {
            ...
            anguloMax =
                Math.toDegrees(sp.ballPosition().angle(
                    new Position(xAreaIzquierda, Constants.LARGO_CAMPO_JUEGO / 2)));
          } else if (sp.ballPosition().getX() < xAreaDerecha) {
            anguloMin =
                Math.toDegrees(sp.ballPosition().angle(
                    new Position(xAreaDerecha, Constants.LARGO_CAMPO_JUEGO / 2)));
            anguloMax =
                Math.toDegrees(sp.ballPosition().angle(
                    new Position(xAreaIzquierda, Constants.LARGO_CAMPO_JUEGO / 2)));
          } else {
            anguloMin =
                Math.toDegrees(sp.ballPosition().angle(
                    new Position(xAreaDerecha, Constants.LARGO_CAMPO_JUEGO / 2)));
            anguloMax =
                Math.toDegrees(sp.ballPosition().angle(new Position(xAreaIzquierda, yArea)));
          }
          if (anguloMin < 0) {
            anguloMin = anguloMin + 360;
          }
          ...
          boolean
              paseHaciaElArea =
              anguloPaseFormateado < anguloMax && anguloPaseFormateado > anguloMin;
  ...
  private boolean cumpleCondicionesPase(GameSituations sp, Jugador jugador,
    ...
    if (sp.myPlayers()[jugador.getIndice()].distance(sp.myPlayers()[jugadorDestino.getIndice()]) <=
      ...
      if (sp.myPlayersDetail()[jugador.getIndice()].isGoalKeeper()) {
        ...
        int
            rivalCercano =
            sp.myPlayers()[jugadorDestino.getIndice()].nearestIndex(sp.rivalPlayers());
        if (sp.myPlayers()[jugadorDestino.getIndice()].getY() < -10 &&
          ...
          if (sp.myPlayers()[jugador.getIndice()].getX() > Constants.posteDerArcoInf.getX()) {
            ...
            if (sp.myPlayers()[jugadorDestino.getIndice()].getX() >= sp.myPlayers()[jugador
                .getIndice()].getX()) {
  ...
  private boolean alcanzoAntesBalon(GameSituations sp, Jugador jugador,
    ...
    double
        vel =
        fuerza * Constants.getVelocidadRemate(sp.myPlayersDetail()[jugador.getIndice()].getPower());
    double v0 = vel;
    ...
    while (!alcanzoAntesBalon && !salir &&
      ...
      for (Jugador miJugador : arrayJugadores) {
        ...
        if ((miJugador.getIndice() == jugador.getIndice()
             && ciclos > Constants.ITERACIONES_GOLPEAR_BALON) ||
  ...
  private Double avanzarRango(GameSituations sp, Jugador jugador,
    ...
    if (sp.myPlayers()[jugador.getIndice()].getY() > Entorno.LIMITE_AVANCE_Y) {
      ...
      // Lado derecho.
      else if (sp.myPlayers()[jugador.getIndice()].getX() > (Constants.posteDerArcoSup.getX()
                                                             + 1.5)) {
  ...
  private Hashtable<String, Object> calculaPosicionTiro(GameSituations sp, Jugador jugador,
    ...
    ArrayList<String>
        listadoZona =
        listaZonaTiro(sp, jugador, inicioTiro, opcionTiroSeguro, fuerza, anguloZ);
  ...
  private double[] calculaValoresTiro(GameSituations sp, Jugador jugador, Position posTiro) {
    ...
    do {
      ...
      double
          vel =
          fuerza * Constants
              .getVelocidadRemate(sp.myPlayersDetail()[jugador.getIndice()].getPower());
      double v0 = vel;
      vel = vel * Math.cos(angVer);
  ...
  private Hashtable<String, Object> corregirAnguloZVariacionTiro(GameSituations sp, Jugador jugador,
                                                                 Position inicioTiro, double fuerza,
                                                                 Position posicionXY,
                                                                 double anguloZ) {
  ...
  private boolean forzadoTiro(GameSituations sp) {
    ...
    boolean
        forzado =
        sp.ballPosition().distance(Constants.centroArcoSup) < Entorno.DISTANCIA_FORZAR_TIRO;
