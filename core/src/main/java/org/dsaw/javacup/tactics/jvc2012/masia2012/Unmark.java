...
public class Unmark {
  ...
  class AttackPosition implements Comparable<AttackPosition> {
    ...
    private AttackPosition(Position position, double precision) {
      ...
      double
          rivalDistance =
          Math.min(Math.max(position.distance(
              analysis.rivalPlayers[position.nearestIndex(analysis.rivalPlayers)]),
                            Constants.DISTANCIA_CONTROL_BALON_PORTERO), 5) / 5;
      double
          goalDistance =
          Math.max(1 - (Math.max(position.distance(Constants.penalSup), Constants.ANCHO_AREA_GRANDE)
                        - Constants.ANCHO_AREA_GRANDE) / 100.0, 0);
