...
public class TacticaRomedalusTeam implements Tactic, Alineaciones {
  ...
  private static Position
      metaAbajoIzquierda =
      Constants.esqSupIzqAreaChicaSup.movePosition(0, Constants.ANCHO_AREA_CHICA);
  private static Position
      metaAbajoDerecha =
      Constants.esqSupIzqAreaChicaSup
          .movePosition(Constants.LARGO_AREA_CHICA, Constants.ANCHO_AREA_CHICA);
  private static Position metaArribaIzquierda = Constants.esqSupIzqAreaChicaInf;
  private static Position
      metaArribaDerecha =
      Constants.esqSupIzqAreaChicaInf.movePosition(Constants.LARGO_AREA_CHICA, 0);

  // comandos a ejecutar por iteracion
  private final ArrayList<Command> comandos = new ArrayList<>();
  // detalle de mis jugadores
  private final DetalleTactica
      detalle =
      new DetalleTactica(this.getClass().getSimpleName().substring(7));
  ...
  public List<Command> execute(final GameSituations sp) {
    ...
    if (destino == null || destino.isGanaRival() || (trayectoria.size() <= 1 && !sp.isStarts())
        || tieneElbalon()) {
      laTengo = false;
    }
        ...
        if (!opcionesDisparo.contains(opcion)) {
          ...
          for (double fuerza = 1; fuerza >= 0.2; fuerza -= 0.01d) {
            ...
            for (double angZ = maxAng; round(angZ, 2) > 10; angZ -= 0.05) {
              ...
              final InfoTrayectoria
                  info =
                  obtenerDistanciaCalculada(balon, fuerza, potencia, angRadZ, velocidadJugador);
              if (info != null) {
            ...
            final InfoTrayectoria
                info =
                obtenerDistanciaCalculada(balon, fuerza, potencia, 0, velocidadJugador);
   ...
   */
  private InfoTrayectoria obtenerDistanciaCalculada(final Balon balon, final double fuerza,
                                                    final double potencia, final double anguloZ,
                                                    final double velocidadJugador) {
    // instancia para manejar la trayectoria
    final AbstractTrajectory
        trajectory =
        new AirTrajectory(Math.cos(anguloZ) * potencia, Math.sin(anguloZ) * potencia, 0, 0);
    ...
    for (int step = 1; step <= 200; step++) {
      ...
      if (dxAnt == dX || !nuevoBalon.getPosition().isInsideGameField(0) || (
          (elevando && !dzAntPosivivo) || (!elevando && dY < altControl))) {
        return null;
      }
      oldtrY = dY;
      dxAnt = dX;
   ...
   */
  private int obtenerTrayectoriaCalculada(final LinkedList<Balon> trayectoriaBalon,
                                          final Balon balon, final double potencia,
                                          final double angleXY, final double angleZ,
                                          final Position pFinal, final boolean completa,
                                          final int idxReceptor,
                                          final int maxPasos) {
    ...
    final AbstractTrajectory
        trajectory =
        new AirTrajectory(Math.cos(angleZ) * potencia, Math.sin(angleZ) * potencia, 0, 0);

    final double[] datosControl = getDatosControl(idxReceptor, pFinal, false);

    double distanciaAPunto;
    ...
    for (int step = 1; step <= maxPasos; step++) {
        ...
        if (iteracionesAlcance == -1) {
          ...
          if (idxReceptor == -1 && !trayectoriaBalon.isEmpty()) {
            if (angleZ > 0) {
              ...
              final boolean
                  antAdentro =
                  balonAnterior.getAltura() > Constants.ALTO_ARCO
                  && balonAnterior.getPosition().getY() <= yMeta;
              final boolean
                  actAfuera =
                  nuevoBalon.getAltura() < Constants.ALTO_ARCO
                  && nuevoBalon.getPosition().getY() > yMeta;
              if (antAdentro && actAfuera) {
                final double
                    deltaX =
                    balonAnterior.getPosition().distance(nuevoBalon.getPosition());
                ...
                final double
                    teta =
                    Math.acos((deltaX * deltaX + h * h - deltaY * deltaY) / (2 * deltaX * h));
                ...
                if (alturaMeta <= Constants.ALTO_ARCO && (
                    nuevoBalon.getAltura() - (balonAnterior.getAltura() - b0.getAltura())
                    <= Constants.ALTO_ARCO || centro)) {
          ...
          else if (iteracionesAlcance == -1 && distanciaAPunto <= datosControl[0] && (angleZ == 0
                                                                                      || (dY
                                                                                          <= datosControl[1]))) {
      ...
      if (!completa && iteracionesAlcance == -1 && (angleZ > 0 && ((elevando && !dzAntPosivivo) || (
          !elevando && dY < datosControl[1])))) {
        return iteracionesAlcance;
      }
  ...
  private void ubicarPortero() {
    ...
    if (!sp.isStarts()) {
      ...
      for (final Command c : comandos) {
        if (c.getPlayerIndex() == idxMiPortero && c instanceof CommandMoveTo
            && ultimoPasador != destinoPase) {
          comandos.remove(c);
          break;
        }
      }
      ...
      if (destino != null /* && posFinalEnAreaGrande(destino.getPosBalon(), false) */ && !destino
          .isGanaRival()) {
      ...
      if (sp.ballPosition().equals(metaAbajoDerecha) || sp.ballPosition()
          .equals(metaAbajoIzquierda)) {
  ...
  private Position mejorPosArquero() {
    ...
    final double
        distRival =
        sp.ballPosition()
            .distance(sp.rivalPlayers()[sp.ballPosition().nearestIndex(sp.rivalPlayers())]);
    ...
    if (minDist <= Constants.ANCHO_AREA_GRANDE && distRival <= Constants.DISTANCIA_CONTROL_BALON) {
      ...
      final Position
          posRet =
          Position
              .Intersection(sp.ballPosition(), Constants.centroArcoInf, Constants.posteDerArcoInf,
                            Constants.posteIzqArcoInf.moveAngle(anguloPoste, distIzq));
      if (posRet != null) {
        return posRet;
      }
    }
    ...
    return Constants.centroArcoInf
        .moveAngle(angulo, (Constants.LARGO_ARCO - Constants.DISTANCIA_CONTROL_BALON_PORTERO) / 2);
  }
  ...
  private void recuperarBalon(final DestinoBalon destino) {
    ...
    if (ultimoPasador == destinoPase && destinoPase >= 0 && (laTengo
                                                             || ultimoPasador != idxMiPortero)) {
      autopase = true;
      recuperando.add(ultimoPasador);
      comandos.add(new CommandMoveTo(destinoPase, trayectoria.getLast().getPosition()));
    }
    // si se hizo un pase y el jugador lo alcanza
    else if (balonDestinoPase != null && (laTengo || destinoPase != idxMiPortero) && moverAlHueco(
        trayectoria)) {
      paseAlVacio = true;
      recuperando.add(destinoPase);
      ...
      if (ultimoPasador >= 0 && ultimoPasador != idxMiPortero && ultimoPasador != destinoPase) {
        comandos.add(new CommandMoveTo(ultimoPasador, sp.myPlayers()[ultimoPasador]
            .moveAngle(90 * TO_RAD, sp.getMyPlayerSpeed(ultimoPasador)).setInsideGameField()));
        recuperando.add(ultimoPasador);
      }
    }
    if (laTengo && ultimoPasador >= 0 && ultimoPasador != destinoPase) {
      recuperando.add(ultimoPasador);
      comandos.add(
          new CommandMoveTo(ultimoPasador, sp.myPlayers()[ultimoPasador].movePosition(0, 0.5)));
    }
    if (destino != null && !destino.isGanaRival() && !autopase && !paseAlVacio) {
      comandos.add(
          new CommandMoveTo(destino.getIdxJugador(), destino.getPosBalon().setInsideGameField()));
      recuperando.add(destino.getIdxJugador());
    }
    // si no la tengo
    if (!laTengo) {
      if (trayectoria.getLast().getPosition().getY() >= 52.5 || sp.ballPosition()
          .equals(metaArribaDerecha) || sp.ballPosition().equals(metaArribaIzquierda)) {
      ...
      final Position
          pDest =
          destino != null && destino.isGanaRival() ? destino.getPosBalon()
                                                   : trayectoria.getLast().getPosition();
  ...
  private void marcarRivales() {
    ...
    for (final int i : rivales) {
      ...
      if (!enAreaChica(rival, false) && (marcar || marcas < minMarcas || (!laTengo
                                                                          && rival.getY() < -30))) {
    ...
    if (!trayectoriaBalon.contains(balonDestinoPase)) {
      ...
      for (final Balon balon : trayectoriaBalon) {
        if (balon.getPosition().isInsideGameField(0)) {
          ...
          if (distancia <= Constants.DISTANCIA_CONTROL_BALON
              && balon.getAltura() <= Constants.ALTURA_CONTROL_BALON && balon.getIteracion() >= sp
              .iterationsToKick()[destinoPase]) {
            alcanzaBalon = true;
            opcion = balon;
    ...
    if (sp.canKick().length == 0 || sp.isStarts()) {
      ...
      if (iterSaco <= Constants.ITERACIONES_SAQUE * 2 / 5) {
        ...
        for (int idx = 0; idx < 11; idx++) {
          if (sp.getMyPlayerError(idx) >= 0.9
              && sp.myPlayers()[idx].distance(sp.ballPosition()) <= (
              ((Constants.ITERACIONES_SAQUE / 2) - iterSaco) * 0.5)) {
            hayOpcion = true;
            i = idx;
            break;
          }
        ...
        if (sp.ballPosition().distance(Constants.cornerSupDer) == 0
            || sp.ballPosition().distance(Constants.cornerSupIzq) == 0) {
  ...
  private boolean hacerPaseAlVacio(final boolean adelante, final boolean autopase,
                                   final int minIterDif, final boolean mejorSkills) {
    final int[]
        receptores =
        Constants.centroArcoSup.nearestIndexes(sp.myPlayers(), idxMiPortero, ultimoPasador);
    ...
    if (autopase) {
      ...
      final double
          skillPas =
          sp.getMyPlayerPower(ultimoPasador) + sp.getMyPlayerError(ultimoPasador);
      double mejorDif = 0d;
      for (final int idxReceptor : receptores) {
        if (idxReceptor != ultimoPasador) {
          final double
              skilRec =
              sp.getMyPlayerPower(idxReceptor) + sp.getMyPlayerError(idxReceptor);
          ...
          if ((mejorSkills && esMejor) || !mejorSkills) {
            ...
            final ComandoGolpear
                dest =
                getOpcionPase(ultimoPasador, idxReceptor, adelante, minIterDif);
            if (dest != null) {
    ...
    if (mejorDestino != null) {
      ...
      balonDestinoPase =
          new Balon(comando.getDestiny(),
                    comando.getVerticalAngle() > 0 ? Constants.ALTURA_CONTROL_BALON : 0);
  ...
  private boolean hacerDisparo(final boolean obstaculo, final double minAngZ,
                               final double maxAngZ) {
  ...
  private void atacar() {
    final boolean
        puedeRematar =
        sp.getMyPlayerPower(ultimoPasador) >= 0.8 || sp.myGoals() < sp.rivalGoals();
    // se intenta pasar el balon hacia adelante
  ...
  private void definir() {
    final boolean
        noSacoRival =
        sp.isStarts() && (sp.ballPosition().equals(metaArribaDerecha) || sp.ballPosition()
            .equals(metaArribaIzquierda));
    final boolean
        paseAtras =
        Math.abs(sp.ballPosition().getX()) > Constants.LARGO_AREA_GRANDE / 2
        && sp.ballPosition().getY() >= Constants.penalSup.getY();

    final boolean
        enAreaChica =
        sp.ballPosition().getY() > 42 && Math.abs(sp.ballPosition().getX()) < (
  ...
  private boolean quedarseQuieto() {
    ...
    final boolean
        tendreControl =
        trayectoria.size() > 1
        && trayectoria.get(1).getPosition().distance(sp.myPlayers()[ultimoPasador])
    ...
    if (tendreControl && (destinoRival == null || destinoRival.getIterJugador() > 3)) {
      final Position
          posFinal =
          trayectoria.size() > 1 ? trayectoria.get(1).getPosition() : sp.ballPosition();
      ...
      if (!hacerPaseAlVacio(true, false, -1, false)) {
        if (sp.ballPosition().getY() < 0 || !hacerPaseAlVacio(true, true, -1, false)) {
          if (sp.ballPosition().getY() < 0 || !hacerPaseAlVacio(true, false, -1, false)) {
            comandos.add(new CommandHitBall(ultimoPasador, Constants.centroArcoSup, 1,
                                            sp.ballPosition().getY() < -10 ? 35 : 50));
          }
        }
      }
    }
  ...
  private void setAlineacion() {
    ...
    if (sp.isStarts() && sp.ballPosition().getY() < -45) {
      ...
      int
          seccionAct =
          Math.max(0, Math.min(alineaciones.length - 1, (int) Math.floor(y / seccion)));
      if (sp.isRivalStarts()) {
  ...
  private void alinear(final Position alineacion[], final List<Integer> excluir) {
    ...
    final double
        dy =
        balonAnt != null && !sp.isStarts() ? (sp.ballPosition().getY() - balonAnt.getPosition()
            .getY()) : sp.isStarts() ? 3 : 0;
  ...
  private Position getMejorPosJugador(final Position pos) {
    final double
        height =
        Math.max(Constants.LARGO_CAMPO_JUEGO / 3, 2 * Math
            .min(Math.abs(Constants.centroArcoInf.getY() - sp.ballPosition().getY()),
                 Math.abs(Constants.centroArcoSup.getY() - sp.ballPosition().getY())));
    final double
        width =
        Math.max(Constants.ANCHO_CAMPO_JUEGO * 2 / 3, 2 * Math
            .min(Math.abs(Constants.cornerInfIzq.getX() - sp.ballPosition().getX()),
                 Math.abs(Constants.cornerInfDer.getX() - sp.ballPosition().getX())));
    ...
    if (center.getY() + height / 2 > Constants.centroArcoSup.getY()) {
      center =
          center.movePosition(0, -(center.getY() + height / 2 - Constants.centroArcoSup.getY()));
    }
    if (center.getY() - height / 2 < Constants.centroArcoInf.getY()) {
      center =
          center.movePosition(0, Constants.centroArcoInf.getY() - (center.getY() - height / 2));
    }
    return new Position(pos.getX() * width / Constants.ANCHO_CAMPO_JUEGO + center.getX(),
                        pos.getY() * height / Constants.LARGO_CAMPO_JUEGO + center.getY());
  }

  /**
   * Metodo que retorna el jugador con mas posibilidades de interceptar el balon en iteraciones
   * futuras
   *
   * @return Posicion 0: id del jugador, Posicion 1: id del rival, Posicion 2: Jugador que llega
   * primero 0->jugador propio 1->rival
   */
  private DestinoBalon obtenerMejorUbicado(final LinkedList<Balon> trayectoriaBalon,
                                           final boolean esTrReal, final boolean analizarPortero,
                                           final boolean soloMios) {
    DestinoBalon destinoMio = null;
    DestinoBalon destinoRival;
    if (!analizarPortero) {
      for (int i = 0; i < 11; i++) {
        if (i != idxMiPortero) {
          final DestinoBalon
              bDestino =
              obtenerPosDestino(trayectoriaBalon, i, false,
                                destinoMio == null ? 100 : destinoMio.getIterBalon(), esTrReal);
          if (bDestino != null
              && (destinoMio == null || (destinoMio != null && bDestino.compareTo(destinoMio) > 0
                                         || (bDestino.compareTo(destinoMio) == 0 &&
                                             sp.myPlayers()[bDestino.getIdxJugador()]
                                                 .distance(bDestino.getPosBalon()) < sp
                                                 .myPlayers()[destinoMio
                                                 .getIdxJugador()]
                                                 .distance(destinoMio.getPosBalon()))))) {
            destinoMio = bDestino;
          }
        }
      }
    ...
    if (analizarPortero && destinoMio != null && destinoRival != null) {
      ...
      if ((destinoMio.getAltura() > Constants.ALTURA_CONTROL_BALON
           && destinoMio.getPosBalon().distance(sp.ballPosition()) <= destinoRival.getPosBalon()
          .distance(sp.ballPosition()))
  ...
  private DestinoBalon obtenerRivalMejorUbicado(final LinkedList<Balon> trayectoriaBalon,
                                                final boolean esTrReal) {
    final int[] indices = sp.ballPosition().nearestIndexes(sp.rivalPlayers());
    DestinoBalon destino = null;
    for (final int i : indices) {
      final DestinoBalon
          bDestino =
          obtenerPosDestino(trayectoriaBalon, i, true,
                            destino == null ? 1000 : destino.getIterBalon(), esTrReal);
      if (bDestino != null) {
        ...
        if (destino == null) {
          ...
          if (bDestino.compareTo(destino) == 0) {
            final Position
                posDestinoAnt =
                destino.isGanaRival() ? sp.rivalPlayers()[destino.getIdxJugador()]
                                      : sp.myPlayers()[destino.getIdxJugador()];
  ...
  private DestinoBalon obtenerPosDestino(final LinkedList<Balon> trayectoriaBalon, final int idx,
                                         final boolean esRival, final int maxIteraciones,
                                         final boolean esTrReal) {
    ...
    if (trayectoriaBalon.size() > 1) {
      ...
      final double
          velocidad =
          Constants.getVelocidad(esRival ? sp.getRivalPlayerSpeed(idx) : sp.getMyPlayerSpeed(idx));
      final double
          alturaMax =
          esArquero(idx, esRival) ? Constants.ALTO_ARCO : Constants.ALTURA_CONTROL_BALON;

      // se definen las iteraciones en las cuales puede controlar el balon el jugador
      final int
          iteracionesControl =
          esRival ? sp.rivalIterationsToKick()[idx]
                  : (esTrReal || idx != ultimoPasador ? sp.iterationsToKick()[idx]
      ...
      do {
        ...
        if (infoBalonSig.getAltura() <= alturaMax) {
          ...
          if (distBalon <= velocidad * iterBalon && infoBalonSig.getAltura() <= datosControl[1]
              && iterBalon >= iteracionesControl) {
  ...
  private ComandoGolpear getOpcionPase(final int idxPasador, final int idxReceptor,
                                       final boolean arriba, final int minIterDif) {
    ...
    final int
        tipoDisparo =
        sp.isStarts() && !balon.equals(Constants.cornerSupDer) && !balon
            .equals(Constants.cornerSupIzq) && !balon.equals(metaAbajoDerecha) && !balon
            .equals(metaAbajoIzquierda) ? 0 : 1;
    ...
    for (final int ang : angulosPosibles.keySet()) {
      ...
      for (int paso = ang == angInicial ? pasoIni : 3; paso <= 40; paso += 3) {
        ...
        if ((arriba && paseAdeltante) || (!arriba && !paseAdeltante)) {
          if (autoPase && idxReceptor == idxMiPortero && (!posFinalEnAreaGrande(pFinal, false)
                                                          || pFinal.getY() < sp.ballPosition()
              .getY())) {
            break;
          }
          ...
          if (pFinal.isInsideGameField(-2) && (autoPase || distancia >= distanciaMinima)) {
            ...
            InfoTrayectoria
                infoTrayectoria =
                getOpcionesDisparo(idxPasador, tipoDisparo,
                                   distancia + (autoPase ? Constants.DISTANCIA_CONTROL_BALON : 0),
                                   0, paso);
            // se calcula el disparo al punto y se verifica si es interceptado por un jugador mio antes que un rival
            ComandoGolpear
                comando =
                disparoAlPunto(infoTrayectoria, idxPasador, pFinal, idxReceptor, paso, minIterDif);
            // si no es un autopase y no existe opcion de pase rastrero, se intenta el pase elevado
            if (!autoPase && comando == null) {
              infoTrayectoria =
                  getOpcionesDisparo(idxPasador, tipoDisparo, distancia,
                                     Constants.ALTURA_CONTROL_BALON, paso);
              comando =
                  disparoAlPunto(infoTrayectoria, idxPasador, pFinal, idxReceptor, paso,
                                 minIterDif);
            }
      ...
      if (angulosPosibles.get(ang) != null) {
        ...
        for (final ComandoGolpear c : opciones.descendingSet()) {
          ...
          if (cgRet == null || (dif > cgRet.getIterRival() - cgRet
              .getIterJugador() /* || (dif == cgRet.getIterRival() - cgRet.getIterJugador() && c.getComando().getDestiny().getY() > cgRet.getComando().getDestiny().getY()) */)) {
            cgRet = c;
          }
        }
      }
  ...
  private ComandoGolpear disparoAlPunto(final InfoTrayectoria info, final int idxPateador,
                                        final Position pFinal, final int idxReceptor,
                                        int maxIteraciones, final double minIterDif) {
    if (info != null) {
      ...
      final Position
          receptor =
          idxReceptor == -1 ? Constants.centroArcoSup : sp.myPlayers()[idxReceptor];
      final double
          distPase =
          idxPateador != idxReceptor ? receptor.distance(pFinal) : pateador.distance(pFinal);
      final int
          minIter =
          idxPateador == idxReceptor || idxReceptor == -1 ? Constants.ITERACIONES_GOLPEAR_BALON
                                                          : sp.iterationsToKick()[idxReceptor];
      // angulo horizontal de disparo
      ...
      final int
          iterReceptor =
          obtenerTrayectoriaCalculada(trayectoriaBalon, balon, info.getPotencia(), angleXY,
                                      info.getAngulo(), pFinal, true, idxReceptor, 100);
      // si el metodo retorna mas de una iteracion, es pq se alcanza el punto deseado
      if ((iterReceptor <= maxIteraciones && iterReceptor >= minIter) || (iterReceptor >= 0
                                                                          && idxReceptor == -1)) {
        ...
        final int
            iterRival =
            destino != null && destino.getIterJugador() > 0 ? destino.getIterJugador() : (int) Math
                .ceil((distRivAlPunto - Constants.DISTANCIA_CONTROL_BALON)
                      / Constants.getVelocidad(sp.rivalPlayersDetail()[idxRival].getSpeed()));
        // si nadie intercepta el pase
        if (iterRival - iterReceptor > minIterDif && (destino == null || distRivAlPunto > distPase
                                                      || (idxReceptor == -1
                                                          && destino.getIdxJugador()
                                                             == idxPortRival))) {
          Position posFin = pFinal;
          if (trayectoriaBalon.size() > iterReceptor) {
            posFin = trayectoriaBalon.get(iterReceptor).getPosition();
          }
          return new ComandoGolpear(
              new CommandHitBall(idxPateador, posFin, info.getFuerza(), info.getAngulo() * TO_ANG),
              trayectoriaBalon, info.getAngulo(), iterReceptor, iterRival, false, false);
        }
        return null;
      }
    }
  ...
  private InfoTrayectoria getOpcionesDisparo(final int idxJugador, final int tipoDisparo,
                                             final double distancia, final double altFinal,
                                             final int minIteraciones) {
  ...
  private void getInfoTrayectoriaReal() {
    ...
    while (balonEnCampo) {
      ...
      final Balon
          b =
          new Balon(new Position(valores[0], valores[1]),
                    sp.ballAltitude() != alturaTeorica ? 0 : valores[2], iter);
  ...
  /**
   * Retorna un arreglo con la distancia y al altura de control del balon del jugador segun la
   * posicion del campo a examinar
   *
  ...
  private boolean posFinalEnAreaGrande(final Position p, final boolean areaRival) {
    final boolean
        cumpleY =
        areaRival ? p.getY() > (Constants.LARGO_CAMPO_JUEGO / 2) - Constants.ANCHO_AREA_GRANDE
  ...
  private ComandoGolpear mejorDisparoAlArco(final int idxPateador, final boolean obstaculo,
                                            final double minAngZ, final double maxAngZ) {
    // datos iniciales del balon
    final Balon balon = new Balon(sp.ballPosition(), sp.ballAltitude());
    ...
    for (final Position position : opciones) {
      ...
      for (double fuerza = 1; fuerza > (maxAngZ > 0 ? 0.6 : 0.95); fuerza -= 0.05d) {
        ...
        for (double angZ = minAngZ; angZ <= maxAngZ; angZ += 0.5) {
          ...
          final double
              angRadZ =
              Math.max(0, Math.min(angZ, Constants.ANGULO_VERTICAL_MAX)) * (TO_RAD);
          // si el tiro es preciso al punto calculado
          final LinkedList<Balon> trayectoriaBalon = new LinkedList<>();
          // se obtiene la trayectoria del balon al punto
          final int
              iteraciones =
              obtenerTrayectoriaCalculada(trayectoriaBalon, balon, potencia, angleXY, angRadZ,
                                          position, false, -1, 100);
          // si el metodo retorna mas de una iteracion, es pq se alcanza el punto deseado
          if (iteraciones >= 0) {
            DestinoBalon destino = null;
            if (obstaculo) {
            ...
            int
                iterRival =
                (int) Math.ceil(distRivAlPunto / Constants
                    .getVelocidad(sp.rivalPlayersDetail()[idxRival].getSpeed()));
            if (destino != null) {
              idxRival = destino.getIdxJugador();
            ...
            if (destino == null || (destino.getIterJugador() > iteraciones) || (
                destino.getIdxJugador() == idxPortRival
                && trayectoriaBalon.get(destino.getIterJugador()).getPosition().getY() > 52.5)) {
              // si se trata de un disparo a meta
              return new ComandoGolpear(
                  new CommandHitBall(idxPateador, trayectoriaBalon.getLast().getPosition(), fuerza,
                                     angZ), trayectoriaBalon, angleXY, iteraciones, iterRival,
                  false, false);
            }
          }
        }
