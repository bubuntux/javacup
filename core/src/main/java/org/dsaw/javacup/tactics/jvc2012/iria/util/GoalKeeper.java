...
public class GoalKeeper {
  ...
  private final Position
      pPosteIzquierdo =
      new Position(Constants.posteIzqArcoInf.getX(), -(Constants.LARGO_CAMPO_JUEGO / 2));
  private final Position
      pPosteDerecho =
      new Position(Constants.posteDerArcoInf.getX(), -(Constants.LARGO_CAMPO_JUEGO / 2));
  ...
  public Command calculePositionGoalkeeper(GameSituations sp) {
    ...
    p =
        p.moveAngle(dAngle,
                    ((Constants.posteDerArcoInf.getX() - Constants.posteIzqArcoInf.getX()) / 2));
    Position c = Position.Intersection(pPosteIzquierdo,
                                       pPosteDerecho,
                                       new Position(sp.getTrajectory(0)[0], sp.getTrajectory(0)[1]),
                                       new Position(sp.getTrajectory(1)[0],
                                                    sp.getTrajectory(1)[1]));

    if ((c != null) && (((Constants.posteDerArcoInf.getX()) > c.getX()) && (
        (Constants.posteIzqArcoInf.getX()) < c.getX()))) {
