...
public class AtaquePapa {
  ...
  public AtaquePapa(List<Command> comandos, Datines data) {
  ...
  public void execute(GameSituations sp, Mentalidad mentality) {
    this.sp = sp;
    Position[] newPositions = calculatePositions(mentality == Mentalidad.Normal ? positions1 :
                                                 mentality == Mentalidad.Offensive ? positions2
                                                                                   : positions3);
  ...
  private void getBall() {
    if (data.getIterToBall() >= 0) {
      comandos.add(
          new CommandMoveTo(data.getPlayerClosetBall(), data.getPosBall(data.getIterToBall())));
    }
  }
