...
public class SimuladorPases {
  ...
  public SimuladorPases(org.dsaw.javacup.model.engine.GameSituations gsOriginal) {
  ...
  public CommandHitBallSimulated simularPase(CommandHitBall chb) {
    try {
      ...
      Position
          posicionRecepcionMedia =
          new Position(x / (double) NUMERO_SIMULACIONES, y / (double) NUMERO_SIMULACIONES);

      return new CommandHitBallSimulated(chb, peorDiferenciaIteraciones, indiceReceptor,
                                         posicionRecepcionMedia, iteracionesRecuperacion);
  ...
  private void executeCommand(CommandHitBall cgp) {
    ...
    double angulo, error;//el angle y el error
    Position p;
   ...
   */
  protected IteracionIndicePosicion getMyPlayersRecovery(int indicePasador)
    ...
    while (true) {
      ...
      if (posBalon[2] <= Constants.ALTO_ARCO) {
        for (int i = 0; i < posicionLocal.length; i++) {
          if (posBalon[2] <= (
              detallesLocal[i].isGoalKeeper() && Common.dentroAreaPropia(posicionBalon)
            ...
            if (dist0 >= dist && (indicePasador == i ? it > Constants.ITERACIONES_GOLPEAR_BALON
                                                     : it >= gsOriginal.iterationsToKick()[i])) {
...
//                throw new RecoveryException();
      if (posBalon[2] <= Constants.ALTO_ARCO) {
        for (int i = 0; i < posicionVisita.length; i++) {
          if (posBalon[2] <= (
              detallesVisita[i].isGoalKeeper() && Common.dentroAreaRival(posicionBalon)
              ? Constants.ALTO_ARCO : Constants.ALTURA_CONTROL_BALON)) {
