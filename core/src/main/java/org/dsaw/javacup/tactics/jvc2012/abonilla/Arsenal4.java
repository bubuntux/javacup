  ...
  public List<Command> execute(GameSituations sp) {
    ...
    for (int i = 0; i < jugadores.length; i++) {
      ...
      if (i != 10 && i != 8 && i != 6) {
        ...
        Commands
            .add(new CommandMoveTo(i, Position.medium(PositionPorteroContrario, PositionMedia)));
      } else if (i == 8 || i == 6) {
    ...
    for (int i : sp.canKick()) {
      ...
      if (i == 10) {
        Commands.add(new CommandHitBall(i, Position
            .medium(PositionPorteroContrario, Constants.centroArcoSup), 1, 12 + r.nextInt(6)));
      } else if (i == 8) {
        ...
        double
            fuerza =
            (jugadores[jugadorDestino].distance(jugadores[i]) / 70) + (1 - sp.getMyPlayerPower(i));
