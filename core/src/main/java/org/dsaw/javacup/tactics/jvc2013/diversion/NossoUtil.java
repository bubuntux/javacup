...
public class NossoUtil {
  ...
  static double getAnguloNecesario(Position from, Position to) {
    return (Math.atan2(to.getY() - from.getY(), to.getX() - from.getX()) * 180.0)
           / Math.PI; // A gravidade era 0,12; mas para nos, vai ser 12
  }

  static double elevacao(double V) {
    double
        angle =
        (Math.asin(Math.sqrt(2 * Constants.G * Constants.ALTO_ARCO) / V) * 180.0) / Math.PI;
  ...
  static Command tocarAoProximo(int i, Position[] jogadores, Position[] rivais,
                                int[][] ordenAvaliacao) {
  ...
  static Command chutarGol(int i, double precisao, double velocidade, Position pos,
                           Position[] rivais) {
    int goleiro = getProximoJogador(Constants.centroArcoSup, rivais);
    double angulo, anguloZ;
  ...
  static int getProximoJogador(int orig, Position[] jogadores) {
    ...
    for (int i = 1; i < 11; i++) {
      if (i != orig) {
        double
            distTmp =
            NossoUtil.dist(jogadores[orig].getX(), jogadores[orig].getY(), jogadores[i].getX(),
                           jogadores[i].getY());
        if (distTmp < distMin) {
          distMin = distTmp;
          idxDistMin = i;
