...
import org.dsaw.javacup.tactics.jvc2012.iria.util.TacticUtils.Attitude;
...
public class Pass {
  ...
  private double calculateFitness(Position position, double z, int iter, Position last,
    ...
    int
        oppIterToBall =
        calculateIterToBall(position, z, opponents, sp.rivalPlayersDetail(),
  ...
  public int calculateIterToBall(Position position, double z, Position[] players,
    ...
    for (int i = 0; i < players.length; i++) {
      if ((details[i].isGoalKeeper() ? Constants.ALTO_ARCO : Constants.ALTURA_CONTROL_BALON) >= z) {
        it =
            (int) Math.ceil(Math.max(0, players[i].distance(position) - (details[i].isGoalKeeper()
                                                                         ? Constants.DISTANCIA_CONTROL_BALON_PORTERO
                                                                         : Constants.DISTANCIA_CONTROL_BALON))
                            / Constants.getVelocidad(details[i].getSpeed()));
