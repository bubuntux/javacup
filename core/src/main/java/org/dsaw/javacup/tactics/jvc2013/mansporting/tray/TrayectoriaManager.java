...
public final class TrayectoriaManager {
    ...
    private TrayectoriaManager() {
        ...
        try {
            ...
            this.golpeosByDistancia = new HashMap<Double, List<TrayectoriaPunto>>();
            this.pasesByDistancia = new HashMap<Double, TrayectoriaPunto>();
            this.disparosByDistancia = new HashMap<Double, TrayectoriaPunto>();
            ...
            for (Trayectoria trayectoria : this.trayectorias.getTrayectoria()) {
                ...
                for (int i = 0; i < trayectoria.getPuntos().size(); i++) {
                    ...
                    if (candidatos == null) {
                        candidatos = new LinkedList<TrayectoriaPunto>();
                    ...
                    if (!paseAdded && desp.getProbabilidadControl() >= PASE_FACTOR && desp.getDesplazamientoV() <= Constants.ALTURA_CONTROL_BALON) {
                        paseAdded = true;
                ...
                if (MathUtils.round(trayectoria.getVelocidad(), ROUND_VELOCIDAD) >= Constants.REMATE_VELOCIDAD_MAX && trayectoria.getAnguloVertical() >= 1.0D) {
                    ...
                    for (int i = trayectoria.getPuntos().size() - 1; i > 1; i--) {
                        ...
                        if (punto.getVelocidadH() >= velocidadTiro && punto.getDesplazamientoV() <= Constants.ALTO_ARCO) {
                            iteraciones = i;
                            break;
                        }
                    }
    ...
    private Trayectorias generateTrayectorias() {
        ...
        for (double angulo = 0; angulo <= Constants.ANGULO_VERTICAL_MAX; angulo++) {
            for (double velocidad = Constants.REMATE_VELOCIDAD_MIN; MathUtils.round(velocidad, ROUND_VELOCIDAD) <= Constants.REMATE_VELOCIDAD_MAX; velocidad += 0.01D) {
                ...
                AirTrajectory
                    trayectoriaAire = new AirTrajectory(Math.cos(anguloReal) * velocidad, Math.sin(anguloReal) * velocidad, 0, 0);
                for (int iteraciones = 0;; iteraciones++) {
                    ...
                    desplazamiento.setDesplazamientoH(trayectoriaAire.getX(time) * Constants.AMPLIFICA_VEL_TRAYECTORIA);
                    desplazamiento.setDesplazamientoV(trayectoriaAire.getY(time) * Constants.AMPLIFICA_VEL_TRAYECTORIA * 2);

                    double velocidadH = desplazamiento.getDesplazamientoH() - desplazamiento0.getDesplazamientoH();
                    double velocidadV = desplazamiento.getDesplazamientoV() - desplazamiento0.getDesplazamientoV();
                    ...
                    if (iteraciones > 0 && vel <= 0) {
                        ...
                        desplazamiento.setProbabilidadControl(iteraciones == 0 ? 0 : (probabilidad < 0 ? 0 : probabilidad));
    ...
    private <E> E getValue(double distancia, Map<Double, E> map) {
        ...
        List<Double> keys = new LinkedList<Double>(map.keySet());
