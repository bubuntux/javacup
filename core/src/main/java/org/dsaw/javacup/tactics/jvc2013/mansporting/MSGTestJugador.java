...
public class MSGTestJugador extends MSGAbstractJugadorPropio {
    ...
    int interacionesEnd;
    Map<Integer, Punto3D> puntoCreenciaMax = new HashMap<Integer, Punto3D>();
    Map<Integer, Punto3D> puntoCreenciaMin = new HashMap<Integer, Punto3D>();
    Map<Integer, Punto3D> puntoCreencia = new HashMap<Integer, Punto3D>();
    Map<Integer, Punto3D> puntoReal = new HashMap<Integer, Punto3D>();
    ...
    public List<Command> ejecuta() {
        Punto3D puntoBalon = new Punto3D(context.getSituacionPartido().ballPosition().getX(), context.getSituacionPartido().ballPosition().getY(), context.getSituacionPartido().ballAltitude());
        if (stop) {
           return Arrays.<Command>asList(new CommandMoveTo(index, new Position(puntoBalon.getX(), puntoBalon.getY())));
        }
        ...
        if (calcular && context.getSituacionPartido().iteration() > interacionesEnd) {
            ...
            try {
                ...
                for (int i = interacionesStart; i <= interacionesEnd; i++) {
                    if (!puntoCreencia.containsKey(i) && !puntoCreenciaMax.containsKey(i) && !puntoCreenciaMin.containsKey(i) && !puntoReal.containsKey(i)) {
                        break;
                    }
                    writer.write(
                            String.format("REAL{%s,%s,%s}, MAX{%s,%s,%s}, MED{%s,%s,%s}, MIN{%s,%s,%s}\n",
                                          ...
                                          puntoCreenciaMin.containsKey(i) ? puntoCreenciaMin.get(i).getZ() : null));
                }
                writer.flush();
                writer.close();
        ...
        if (getPuedeRematar()) {
            if(puntoBalon.getY() > -3 * Constants.LARGO_CAMPO_JUEGO / 8)
            {
                return Arrays.<Command>asList(
                        new CommandMoveTo(index, new Position(puntoBalon.getX(), -3 * Constants.LARGO_CAMPO_JUEGO / 8)),
            ...
            MSGTrayectoria.DisparoTrayectoria
                disparo = MSGTrayectoria.getInstance(candidato, this, puntoBalon, new Punto3D(Constants.centroArcoSup.getX(), Constants.centroArcoSup.getY(), 0));
            ...
            for (int i = 0; i < disparo.getIteracionMax(); i++) {
                puntoCreencia.put(context.getSituacionPartido().iteration() + i, disparo.getPosicion(i).getPunto());
                puntoCreenciaMax.put(context.getSituacionPartido().iteration() + i, disparo.getPosicion(i).getPuntoMax());
                puntoCreenciaMin.put(context.getSituacionPartido().iteration() + i, disparo.getPosicion(i).getPuntoMin());
            }
