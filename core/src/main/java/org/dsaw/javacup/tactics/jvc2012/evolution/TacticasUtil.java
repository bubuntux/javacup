...
public class TacticasUtil {
  ...
  public static Jugador calcularMasCercanoSinMarca(Position posicion, Jugador[] jugadores,
                                                   int rol) {
...
//		}
  ...
  public static int rivalesEntreLineas(Rival[] rivales, double lineaBaja, double lineaAlta) {
    int numRivales = 0;
    for (int i = 1; i < rivales.length; i++) {
      if (rivales[i].getPosicion().getY() <= lineaAlta
          && rivales[i].getPosicion().getY() >= lineaBaja) {
        numRivales++;
      }
    }
    return numRivales;
  ...
  public static Position calcularParada(GameSituations sp) {
    ...
    for (int i = 0; i < Constants.ITERACIONES - sp.iteration(); i++) {
      ...
      if (sp.getTrajectory(i)[1] <= Constants.centroArcoInf.getY()
          && sp.getTrajectory(i)[1] > Constants.centroArcoInf.getY() - 3
  ...
  public static Position calcularPosicionCortePase(Jugador jugador, GameSituations sp,
                                                   Jugador[] jugadores, Rival[] rivales) {
    Position resultado = null;
    Position aux;
    ...
    for (int i = 0; i < Constants.ITERACIONES
                    && (new Position(sp.getTrajectory(i)[0], sp.getTrajectory(i)[1]).distance(
        new Position(sp.getTrajectory(i + 1)[0], sp.getTrajectory(i + 1)[1])) != 0); i++) {
      ...
      distanciaAux =
          jugador.getPosicion()
              .distance(new Position(sp.getTrajectory(i)[0], sp.getTrajectory(i)[1]));
  ...
  public static Position calcularCortePaseMarca(Jugador jugador, GameSituations sp) {
    ...
    return jugador.getRivalMarcado().getPosicion()
        .moveAngle(jugador.getRivalMarcado().getPosicion().angle(sp.ballPosition()), 1);
...
//		}
  ...
  public static Pase calcularPosicionPase(Jugador jugador, Jugador[] jugadores, Rival[] rivales,
                                          GameSituations sp) {
    Pase resultado = null;
    Position pos = null;
          ...
          if (jugadores[i].getRol() == Jugador.DELANTERO) {
            if (i == jugadores.length - 1) {
              ...
              if (jugadores[i].getRivalesZona() == null
                  || jugadores[i].getRivalesZona().length == 0) {
                return new Pase(jugadores[i], jugadores[i].getPosicion().movePosition(+5, +5));
              }
            }
          }
    ...
    for (int i = 0; i < jugadores.length; i++) {
      if (jugador.getId() != jugadores[i].getId()) {
        if ((jugadores[i].getRivalesZona() == null
             || (jugadores[i].getRivalesZona() != null
                 && jugadores[i].getRivalesZona().length == 0))
            && comprobarRival(jugador.getPosicion(), jugadores[i].getPosicion().movePosition(0, +5),
                              rivales)) {
    ...
    for (int i = 0; i < jugadoresSolos.size(); i++) {
      ...
      if (dis < distancia
          ...
          && comprobarRival(jugador.getPosicion(),
                            jugadoresSolos.get(i).getPosicion().movePosition(0, +5), rivales)) {
        distancia = dis;
  ...
  public static CommandHitBall calcularTiro(Jugador jugador, Jugador[] jugadores, Rival[] rivales,
                                            GameSituations sp) {
    ...
    if (masAdelante.getId() == jugador.getId()
        && jugador.getPosicion().getY()
           < Constants.centroArcoSup.getY() - Constants.ANCHO_AREA_GRANDE - 20) {
      resultado =
          new CommandHitBall(jugador.getId(), Constants.centroArcoSup.movePosition(0, 0), 1, 12);
    }

    if (jugador.getRol() == Jugador.DELANTERO) {
      if (jugador.getPosicion().getY()
          > Constants.centroArcoSup.getY() - Constants.ANCHO_AREA_GRANDE - 15
          && !(jugador.getPosicion().getX() < -35) && !(jugador.getPosicion().getX() > 35)) {

        if (jugador.getPosicion().getX() < -10) {

          if (jugador.getPosicion().getY()
              < Constants.centroArcoSup.getY() - Constants.ANCHO_AREA_GRANDE) {
            movePos = -2;
            angulo = 13;
          } else if (jugador.getPosicion().getY() < Constants.centroArcoSup.getY() - (
              Constants.ANCHO_AREA_GRANDE / 2)
                     && jugador.getPosicion().getY()
                        > Constants.centroArcoSup.getY() - Constants.ANCHO_AREA_GRANDE) {
            movePos = -1.5;
            angulo = 10;
          ...
          resultado =
              new CommandHitBall(jugador.getId(),
                                 Constants.posteDerArcoSup.movePosition(movePos, 0), 1, angulo);
          ...
          resultado =
              new CommandHitBall(jugador.getId(),
                                 Constants.posteIzqArcoSup.movePosition(movePos, 0), 1, angulo);
        } else {
      ...
      if (jugador.getPosicion().getY()
          > Constants.centroArcoSup.getY() - Constants.ANCHO_AREA_GRANDE - 5) {
        Rival portero = getPortero(rivales, sp);
  ...
  public static Rival[] obtenerRivalesZona(Jugador jugador, Rival[] rivales) {
    ...
    Zona zona = new Zona(0);
    int RADIO_ZONA = 13;
    zona.setAbajoDerecha(new Position(jugador.getPosicion().getX() + RADIO_ZONA,
                                      jugador.getPosicion().getY() - RADIO_ZONA));
    zona.setAbajoIzquierda(new Position(jugador.getPosicion().getX() - RADIO_ZONA,
                                        jugador.getPosicion().getY() - RADIO_ZONA));
    zona.setArribaDerecha(new Position(jugador.getPosicion().getX() + RADIO_ZONA,
                                       jugador.getPosicion().getY() + RADIO_ZONA));
    zona.setArrivaIzquierda(new Position(jugador.getPosicion().getX() - RADIO_ZONA,
                                         jugador.getPosicion().getY() + RADIO_ZONA));
  ...
  public static Position puedoConducir(Jugador jugador, Rival[] rivales) {
    ...
    int RADIO_Y = 13;
    int ATRAS = 3;
    zona.setAbajoDerecha(
        new Position(jugador.getPosicion().getX() + RADIO_X, jugador.getPosicion().getY() - ATRAS));
    zona.setAbajoIzquierda(
        new Position(jugador.getPosicion().getX() - RADIO_X, jugador.getPosicion().getY() - ATRAS));
    zona.setArribaDerecha(new Position(jugador.getPosicion().getX() + RADIO_X,
                                       jugador.getPosicion().getY() + RADIO_Y));
    zona.setArrivaIzquierda(new Position(jugador.getPosicion().getX() - RADIO_X,
                                         jugador.getPosicion().getY() + RADIO_Y));
