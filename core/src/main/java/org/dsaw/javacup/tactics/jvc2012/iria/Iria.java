...
public class Iria implements Tactic {
  ...
  public List<Command> execute(GameSituations sp) {
    ...
    for (int i = 0; i < sp.rivalPlayers().length; i++) {
      for (int j = 0; j < sp.myPlayers().length; j++) {
        if ((i > sp.rivalPlayers().length + 1) || (j > sp.myPlayers().length + 1)) {
    ...
    if (myAttitudeInIteration == Attitude.Offensive) {
      ...
      if (sp.ballPosition().getY() > -(Constants.LARGO_CAMPO_JUEGO / 4)) {
        ...
        int
            ...
            new Position(0, (Constants.LARGO_CAMPO_JUEGO / 2))
                .nearestIndexes(sp.myPlayers(), oGK.indexGK());
        int e = 1;
        if (sp.myGoals() - sp.rivalGoals() <= 0) {
          e = 0;
        }
        for (int i = 0; i < 2 - e; i++) {
          if (!exclude.contains(candidates[i]) && (indexAtack != candidates[i]) && (
              sp.myPlayersDetail()[candidates[i]].getPower() >= 0.8d)) {
            comandos.add(new CommandMoveTo(candidates[i], getNormalPositions(sp)[candidates[i]]));
          }
        }
      }
  ...
  public Position[] getStartPositions(GameSituations sp) {
    Position[] aTacticDefault = new Position[]{new Position(0.0, -51),
                                               ...
                                               new Position(-9.0, -17.0),
                                               new Position(-25.0, -17.0),
  ...
  public Position[] getNoStartPositions(GameSituations sp) {
    Position[] aTacticDefault = new Position[]{new Position(0.0, -51),
                                               new Position(-13.0, -32.0),
                                               new Position(-0.0, -36.0),
  ...
  public Position[] getNormalPositions(GameSituations sp) {
    ...
    Position[] aTacticDefault = new Position[]{new Position(0.0, -50.4),
                                               ...
                                               new Position(this.control(0 + coenfAdX, -10, 10),
                                                            this.control(42 + coenfAdY, 31, 42)),
                                               new Position(this.control(12 + coenfAdX, 0, 20),
                                                            this.control(27 + coenfAdY, 0, 42)),
                                               new Position(this.control(-12 + coenfAdX, -20, 0),
                                                            this.control(27 + coenfAdY, 0, 42))};
    return aTacticDefault;
  }
}