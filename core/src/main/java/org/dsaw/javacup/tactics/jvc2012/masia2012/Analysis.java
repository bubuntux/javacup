...
public class Analysis {
  ...
  public void update(GameSituations sp) {
    ...
    CalculateIterationsResults
        calculateIterationsResults =
        calculateIterationsToBall(myPlayers, myPlayersDetail);
  ...
  private boolean attacking() {
    return sp.isStarts() || (!sp.isRivalStarts() && iterationToBall >= 0 && (
        rivalIterationToBall < 0 || iterationToBall < rivalIterationToBall));
  }
  ...
  private CalculateIterationsResults calculateIterationsToBall(Position[] players,
    ...
    while (iteration < ballPositions.length) {
      ...
      if (!ballPosition.isInsideGameField(2)) {
        return new CalculateIterationsResults(ballPositions.length,
                                              ballPosition.nearestIndex(players),
                                              ballPositions[iteration].setInsideGameField());
      }
      if (ballZ[iteration] <= Constants.ALTO_ARCO) {
        for (int i = 0; i < players.length; i++) {
          if (details[i].isGoalKeeper()
              && Math.abs(ballPosition.getX()) <= Constants.LARGO_AREA_GRANDE / 2
              && Math.abs(ballPosition.getY())
          ...
          if (ballZ[iteration] <= height) {
            double
                runDistance =
                (double) iteration * Constants.getVelocidad(details[i].getSpeed())
                + ballControlDistance;
            double distanceToBall = players[i].distance(ballPosition);
            if (runDistance >= distanceToBall) {
              return new CalculateIterationsResults(iteration, i,
                                                    ballPositions[iteration].setInsideGameField());
            }
          }
        }
    ...
    return new CalculateIterationsResults(ballPositions.length - 1,
                                          ballPositions[ballPositions.length - 1]
                                              .nearestIndex(players),
                                          ballPositions[ballPositions.length - 1]
                                              .setInsideGameField());
  }
  ...
  public int calculateIterationsToBall(Position position, double z, Position[] players,
                                       PlayerDetail[] details, int[] iterToShoot) {
    int it, best = Constants.ITERACIONES;
    double height;
    double ballControlDistance;
    for (int i = 0; i < players.length; i++) {
      if (details[i].isGoalKeeper() && Math.abs(position.getX()) <= Constants.LARGO_AREA_GRANDE / 2
      ...
      if (height >= z) {
        it =
            (int) Math.ceil(
                Math.max(0, players[i].distance(position) - ballControlDistance) / Constants
                    .getVelocidad(details[i].getSpeed()));
  ...
  public int calculateRivalIterationsToBall(Position position, double z) {
    return calculateIterationsToBall(position, z, rivalPlayers, rivalPlayersDetail,
                                     rivalIterationsToShoot);
  }
