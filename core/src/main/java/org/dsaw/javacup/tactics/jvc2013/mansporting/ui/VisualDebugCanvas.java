...
/**
 * <br> Canvas para las operaciones. <br> <br>Copyright Fi2net S.A. 10-ene-2011
 */
public class VisualDebugCanvas extends Canvas {
  ...
  public VisualDebugCanvas(double scale, double width, double height, double altura) {
    ...
    this.drawablesXY = new LinkedList<>();
    this.drawablesXZ = new LinkedList<>();
    this.drawablesZY = new LinkedList<>();
    this.scale = scale;
    this.drawingDimension =
        new Rectangulo(new Punto(0, 0), width + altura + 1, height + altura + 1);
  }

  /**
   * Paints this canvas. <p/> Most applications that subclass <code>Canvas</code> should override
   * this method in order to perform some useful operation (typically, custom painting of the
   * canvas). The default operation is simply to clear the canvas. Applications that override this
  ...
  public void paint(Graphics g) {
    ...
    drawMarco(new DesplazadoVisualDebugGraphics(ig, new Punto(0, marcoXZ.getAlto() / 2)), marcoXZ,
              drawablesXZ);
    drawMarco(new DesplazadoVisualDebugGraphics(ig, new Punto(-marcoZY.getAncho() / 2, 0)), marcoZY,
              drawablesZY);
