    ...
    public static void main(String[] args) {
        SwingUtilities.invokeLater(new Runnable() {
            ...
            public void run() {
                ...
                frame.setSize((int) (scale * (Constants.ANCHO_CAMPO + Z_MAX)), (int) (scale * (Constants.LARGO_CAMPO + Z_MAX)));
                frame.setResizable(false);
                frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
                frame.initComponents(scale, Constants.ANCHO_CAMPO_JUEGO, Constants.LARGO_CAMPO_JUEGO, Z_MAX);
                ...
                Punto puntoMayorTrasladado = new Punto(puntoMayor.getX() + punto.getX(), puntoMayor.getY() + punto.getY());
                Punto puntoMenorTrasladado = new Punto(puntoMenor.getX() + punto.getX(), puntoMenor.getY() + punto.getY());


                frame.getCanvas().addDrawableXY(new VectorDrawable(new Segmento(punto, punto2), Color.GREEN));
                frame.getCanvas().addDrawableXY(new VectorDrawable(new Segmento(new Punto(0, 0), puntoOrigen), Color.BLUE));
                frame.getCanvas().addDrawableXY(new VectorDrawable(new Segmento(new Punto(0, 0), puntoMayor), Color.CYAN));
                frame.getCanvas().addDrawableXY(new VectorDrawable(new Segmento(new Punto(0, 0), puntoMenor), Color.CYAN));
                frame.getCanvas().addDrawableXY(new VectorDrawable(new Segmento(punto, puntoMayorTrasladado), Color.GRAY));
                frame.getCanvas().addDrawableXY(new VectorDrawable(new Segmento(punto, puntoMenorTrasladado), Color.GRAY));

                frame.getCanvas().addDrawableXY(new RectaDrawable(new Recta(new Punto(-1, 0), new Punto(1, 0)), Color.BLACK));
                frame.getCanvas().addDrawableXY(new RectaDrawable(new Recta(new Punto(0, -1), new Punto(0, 1)), Color.BLACK));
