    ...
    while (!found) {
      ...
      if (getZBall(it) <= Constants.ALTO_ARCO) {
        for (int i = 0; i < rivales.length; i++) {
          if (getZBall(it) <= (detalles[i].isGoalKeeper() ? Constants.ALTO_ARCO
                                                          : Constants.ALTURA_CONTROL_BALON)) {
            pJug = rivales[i];
            dist0 =
                (double) it * Constants.getVelocidad(detalles[i].getSpeed()) + (
                    detalles[i].isGoalKeeper() ? Constants.DISTANCIA_CONTROL_BALON_PORTERO
