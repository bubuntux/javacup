    ...
    public Command irAPosicionDestino() {
        ...
        if (sp.isRivalStarts() ||
            ...
            if (!this.estaBalonEnCampoRival()) {
                double x = (this.getPosicionBalon().getX() * (Constants.LARGO_ARCO / 2) - 0.5) / (Constants.ANCHO_CAMPO_JUEGO / 2);
...
//            return new CommandHitBall(this.getIndice(), jugadores[idx].getPosicionActual(), 1, Constants.ANGULO_VERTICAL_MAX);
            ...
            if (rnd.nextBoolean()) {
                return new CommandHitBall(this.getIndice(), Constants.centroCampoJuego, .8 + Math.random() * .2, true);
            }
            else if (rnd.nextBoolean()) {
                return new CommandHitBall(this.getIndice(), Constants.cornerSupIzq, .8 + Math.random() * .2, true);
            }
            else {
                return new CommandHitBall(this.getIndice(), Constants.cornerSupDer, .8 + Math.random() * .2, true);
            }
