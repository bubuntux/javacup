...
public class Helper {
  ...
  public static double getDistanciaMarca(GameSituations sp, int idRival) {
    ...
    double
        diferenciaLimitesRemate =
        Constants.REMATE_VELOCIDAD_MAX - Constants.REMATE_VELOCIDAD_MIN;
  ...
  public static CommandMoveTo getMovimientoGoleador(GameSituations sp, int idxJugador,
                                                    Util23 util23) {
    ...
    Position pResult;
    if (idxJugador == 10
        && sp.ballPosition().distance(Constants.centroArcoSup) <= distanciaMarca2) {
  ...
  public static CommandMoveTo marcar(int idxMiJugador, int idxRival, int rivalMasCercano,
                                     boolean balonAMiArco, HistorialMarcas historialMarcas,
                                     GameSituations sp, Util23 util23, int rivalMasPeligro) {
    Position pMarca;
    ...
    if (idxRival != rivalMasPeligro
        && (idxRival != rivalMasCercano || (
        pMio.distance(sp.ballPosition()) < pRival.distance(sp.ballPosition())
  ...
  public static Position getPosicionArquero(Position balonPos, Position pRivalMasCercano,
                                            Position pMiArquero, double alturaBalon,
                                            Position... balonposAntA) {
    if (pMiArquero.distance(balonPos) < pRivalMasCercano.distance(balonPos)
        && alturaBalon < Constants.ALTO_ARCO) {
      return balonPos;
    }
    Position
        balonposAnt =
        (balonposAntA == null || balonposAntA.length < 1) ? Constants.centroArcoInf
                                                          : balonposAntA[0];
    Position[] rectaIntersecArquero = getLineaInterseccionArquero(balonPos);
    Position
        posIdeal =
        Position
            .Intersection(rectaIntersecArquero[0], rectaIntersecArquero[1], balonPos, balonposAnt);
    // posIdeal = posIdeal.setPosicion(posIdeal.getX() * -1, posIdeal.getY()
    // * -1);
    if (posIdeal == null) {
      posIdeal = Constants.cornerInfIzq;
    }
    if ((posIdeal.getX() > rectaIntersecArquero[0].getX()
         || posIdeal.getX() < rectaIntersecArquero[1].getX())) {
      posIdeal =
          Position.Intersection(rectaIntersecArquero[0], rectaIntersecArquero[1], balonPos,
      ...
      if ((posIdeal.getX() > rectaIntersecArquero[0].getX()
        ...
        switch (getCuadranteBalon(balonPos)) {
          case I:
            posIdeal =
                new Position(Constants.posteIzqArcoInf.getX(),
                             Constants.posteIzqArcoInf.getY() + (Constants.ANCHO_AREA_CHICA / 3));
            break;
          case IV:
            posIdeal =
                new Position(Constants.posteDerArcoInf.getX(),
                             Constants.posteDerArcoInf.getY() + (Constants.ANCHO_AREA_CHICA / 3));
            break;
          default:
            posIdeal = Constants.centroArcoInf;
            break;
  ...
  public static Position[] getLineaInterseccionArquero(Position balonPos) {
    ...
    Position[]
        ret =
        new Position[]{new Position(rectaIntersecArqueroDefault[0].getX(),
                                    Constants.centroArcoInf.getY() + (Constants.ANCHO_AREA_CHICA
                                                                      * nuevoY)),
                       new Position(rectaIntersecArqueroDefault[1].getX(),
                                    Constants.centroArcoInf.getY() + (Constants.ANCHO_AREA_CHICA
                                                                      * nuevoY)),};
    return ret;
  }
  ...
  private static Position[] rectaIntersecArqueroDefault = new Position[]{
      new Position(Constants.centroArcoInf.getX() + (Constants.LARGO_ARCO / 2),
                   Constants.centroArcoInf.getY() + Constants.ANCHO_AREA_CHICA),
      new Position(Constants.centroArcoInf.getX() - (Constants.LARGO_ARCO / 2),
                   Constants.centroArcoInf.getY() + Constants.ANCHO_AREA_CHICA)};
    ...
    if (posBalon.getY() > 0) {
      ...
      Position res[] = new Position[]{
          ...
          new Position(Constants.centroArcoInf.getX() - (Constants.LARGO_AREA_CHICA / 2),
                       ...
                       + (Constants.ANCHO_AREA_CHICA) * multiplicador)};
      return res;
    }
  }
