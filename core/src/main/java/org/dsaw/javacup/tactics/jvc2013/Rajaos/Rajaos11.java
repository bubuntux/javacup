...
public class Rajaos11 implements Tactic {
  ...
  private boolean enAreaChicaBuenos(Position p) {
    boolean
        ...
        (p.getY() > -Constants.LARGO_CAMPO_JUEGO / 2
         && p.getY() < Constants.LARGO_AREA_CHICA - Constants.LARGO_CAMPO_JUEGO / 2);
    boolean
        bx =
        (p.getX() > 0 - Constants.ANCHO_AREA_CHICA / 2
         && p.getY() < Constants.ANCHO_AREA_CHICA / 2);
    return (bx && by);
  }
  ...
  private void colocarPortero() {
    ...
    double
        rel =
        Constants.ANCHO_CAMPO / (Constants.posteDerArcoInf.getX() - Constants.posteIzqArcoInf
            .getX());
    comandos.add(
        new CommandMoveTo(0, new Position(balon.getX() / rel, Constants.posteDerArcoInf.getY())));
    //Miramos si el portero sale a por el ballPosition
    if (cercaBuenoBalon(0, balon.nearestIndex(malos)) == true
        && balonFin.getY() >= Constants.centroArcoInf.getY()) {
      comandos.add(new CommandMoveTo(0, balonFin));
    } else {
      Position
          p =
          Position
              .Intersection(balonAnt, balon, Constants.posteIzqArcoInf, Constants.posteDerArcoInf);
      if (p == null) {
        if (enAreaChicaBuenos(balonFin) == true) {
          ...
          //colocamos portero en situacion relativa al ballPosition
          rel =
              Constants.ANCHO_CAMPO / (Constants.posteIzqArcoInf.getX() - Constants.posteDerArcoInf
                  .getX());
          comandos.add(new CommandMoveTo(0, new Position(
              Constants.posteDerArcoInf.getX() + balon.getX() / rel,
              Constants.posteDerArcoInf.getY())));
          return;
        }
      }
      if (p.getX() > Constants.posteIzqArcoInf.getX() && p.getX() < Constants.posteDerArcoInf
          .getX()) {
    ...
    if (malosR.length == 0 && buenosR.length == 0) {
      ...
      for (int i = 0; i < iterMalosBalon.length; i++) {
        if (iterMalosBalon[i] < iterRecup) {
          if (iteracionesPosicion(buenos[jugRecup], posRecup, detBuenos[jugRecup].getSpeed())
              > iterMalosBalon[i])
          //return true;
          {
            return false;
  ...
  private void chutarPuerta(int jug) {
    ...
    if (distPI > distPD) {
      if (distPI / distPD > 1.5) {
        direccion =
            new Position(Constants.posteIzqArcoSup.getX() + distPorteroI / 2,
    ...
    if (distPorteroI > distPorteroD) {
      ...
      direccion =
          new Position(Constants.posteDerArcoSup.getX() - distPorteroD / 2,
                       Constants.centroArcoSup.getY());
    }
  ...
  private void realizarPase(int jug1, int jug2) {
    ...
    if (malosEnMedio(buenos[jug2], Constants.centroArcoSup, 40, 20) == false
        && distPuerta < distMaxDisparo / 1.5) {
      //pasamos en direccion a porteria
      pasePos =
          buenos[jug2].moveAngle(buenos[jug2].angle(Constants.centroArcoSup), distPase / 5 + 1);
    } else if (distM < 5) {
    ...
    if (malosEnMedio(buenos[jug1], pasePos, angDesv, 20) == true
        || malosEnMedio(pasePos, balon, angDesv, 20) == true
        || distM <= Constants.DISTANCIA_CONTROL_BALON) {
  ...
  int[] jugadoresParalelo(int jug, int dist) {
    ...
    for (int i = 0; i < 11; i++) {
      if (buenos[i].getY() > buenos[jug].getY() - dist
          && buenos[i].getY() < buenos[jug].getY() + dist) {
        temp1[n] = i;
        n++;
      }
    }
  ...
  private void buscaMejorAccion(int jug) {
    ...
    if (jug == 0 || esSaquePuerta()) {
      ...
      if (jug == 0 && esSaquePuerta() && contIterSaque < Constants.ITERACIONES_SAQUE / 2) {
        return;
      }
  ...
  double valorarPase(int jug) {
    ...
    if (malosEnMedio(balon, buenos[jug], 25)
        == true)//|| distMalo <= Constants.DISTANCIA_CONTROL_BALON)
    {
    ...
    if (malosEnMedio(buenos[jug], Constants.centroArcoSup, distMaxDisparo / 4) == false
        || distMalo <= Constants.DISTANCIA_CONTROL_BALON) {
      if (distPuerta < distMaxDisparo / 2) {
        valor = valor + 4;
      }
      if (distPuerta < distMaxDisparo) {
  ...
  private double valorarDespeje(int jug) {
    ...
    if (distPorteria >= 0 && distPorteria < 10) {
      ptos += -2;
    } else if (distPorteria >= 10 && distPorteria < 20) {
      ptos += -1;
    } else if (distPorteria >= 20 && distPorteria < 30) {
      ptos += 0;
    } else if (distPorteria >= 30 && distPorteria < 40) {
      ptos += 1;
    } else if (distPorteria >= 40) {
      ptos += 2;
    } else if (distPorteria >= 50) {
      ptos += -2;
    }
  ...
  private void marcajeDelanteros() {
    if ((zonaCampo > 9 && predecirPosesion() == true)
        || esSaquePuerta() == true)// || iterTrasSaque < numIterTrasSaque)
    {
  ...
  private void setAlineacion() {
    ...
    if (p.getY() > largo4) {
      ...
      if (posesion == true) {
        if (p.getX() > ancho3) {
          alinActual = alineacion12;
          zonaCampo = 12;
        } else if (p.getX() > ancho2) {
          alinActual = alineacion11;
          zonaCampo = 11;
        } else if (p.getX() > ancho1) {
          alinActual = alineacion10;
          zonaCampo = 10;
        }
      } else {
        if (p.getX() > ancho3) {
      ...
      if (posesion == true) {
        if (p.getX() > ancho3) {
          alinActual = alineacion9;
          zonaCampo = 9;
        } else if (p.getX() > ancho2) {
          alinActual = alineacion8;
          zonaCampo = 8;
        } else if (p.getX() > ancho1) {
          alinActual = alineacion7;
          zonaCampo = 7;
        }
      } else {
        if (p.getX() > ancho3) {
          alinActual = alineacion6;
          zonaCampo = 6;
        } else if (p.getX() > ancho2) {
          alinActual = alineacion5;
          zonaCampo = 5;
        } else if (p.getX() > ancho1) {
          alinActual = alineacion4;
          zonaCampo = 4;
        }
      }
      ...
      if (posesion == true) {
        ...
        if (p.getX() > ancho3) {
          alinActual = alineacion3;
          zonaCampo = 3;
        } else if (p.getX() > ancho2) {
          alinActual = alineacion2;
          zonaCampo = 2;
        } else if (p.getX() > ancho1) {
          alinActual = alineacion1;
          zonaCampo = 1;
        }
      }
