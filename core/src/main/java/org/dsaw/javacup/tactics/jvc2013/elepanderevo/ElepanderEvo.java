...
public class ElepanderEvo implements Tactic {
    ...
    public PlayerI[] getPlayers() {
      return new PlayerI[]{
          ...
          new JugadorImpl("Alvaro", 2, new Color(255, 200, 150), new Color(50, 0, 0), 0.9d, 0.89d,
                          0.89d, false),
          new JugadorImpl("Alberto", 3, new Color(255, 200, 150), new Color(50, 0, 0), 0.84d, 0.85d,
                          0.85d, false),
          new JugadorImpl("FranFerri", 4, new Color(255, 200, 150), new Color(50, 0, 0), 0.75d,
                          0.74d, 0.74d, false),
          new JugadorImpl("Jose", 5, new Color(255, 200, 150), new Color(50, 0, 0), 0.78d, 0.78d,
                          0.79d, false),
          new JugadorImpl("Roberto", 6, new Color(255, 200, 150), new Color(50, 0, 0), 0.82d, 0.83d,
                          0.84d, false),
          new JugadorImpl("Alex", 7, new Color(255, 200, 150), new Color(50, 0, 0), 0.78d, 0.77d,
                          0.76d, false),
          new JugadorImpl("Miguel", 8, new Color(255, 200, 150), new Color(50, 0, 0), 0.81d, 0.8d,
                          0.81d, false),
          new JugadorImpl("Paco", 9, new Color(255, 200, 150), new Color(50, 0, 0), 0.82d, 0.83d,
                          0.85d, false),
          new JugadorImpl("Ruben", 10, new Color(255, 200, 150), new Color(50, 0, 0), 0.85d, 0.85d,
                          0.85d, false),
          new JugadorImpl("Angel", 11, new Color(255, 200, 150), new Color(50, 0, 0), 0.84d, 0.84d,
                          0.94d, false)
      };
    }
  }
  ...
  public Position posicionRivalMasNivelCercania(Position miPosicion, GameSituations sp, int nivel) {
    ...
    for (Position posicionRival : posicionRivales) {
      ...
      if (minimaDistancia > distancia) {
        minimaDistancia = distancia;
        posicionRivalMasCercano2
            .setPosition(posicionRivalMasCercano.getX(), posicionRivalMasCercano.getY());
        posicionRivalMasCercano = posicionRival;
  ...
  public int[] obtenerDelanterosRivalesCubiertos(GameSituations sp) {
    ...
    for (PlayerI detalleJugadorRival : detalleJugadoresRivales) {
      ...
      } else if (numero > 0 && numero < 11 && rivalEstaCubierto(posicionRivales[numero - 1],
                                                                posicionmisJugadores)) {
        listaJugadoresCubiertos[numero - 1]++;//Error detalleJugadorRival.getNumber() puede ser 20
      }
    }
  ...
  public boolean rivalEstaCubiertoSoloPorMi(Position posicionRival, Position[] misPosiciones,
                                            int i) {
  ...
  public boolean rivalEstaCubiertoPorVariosIncluidoYo(Position posicionRival,
                                                      Position[] misPosiciones, int i) {
    int jugadoresCubriendo = 0;
    boolean yoCubro = false;
    for (int jugador = 0; jugador < misPosiciones.length; jugador++) {
      if (rivalEstaCubiertoPorMi(posicionRival, misPosiciones[i])) {
      ...
      if (recuperadores.length > 1) {
        ...
        for (int i = 1; i < recuperadores.length; i++) {
          comandos.add(new CommandMoveTo(recuperadores[1],
                                         //TODO: abarranco: va solo 1 de los jugadores, el mas cercano
    ...
    for (int i : sp.canKick()) {
      ...
      if (estoyEnDisposicionDeRematar(posicionJugadores[i], sp)) {
        ...
        if (estoyCubierto(posicionJugadores[i], sp) || i == 0) {
          ...
          Position
              trayectoriaPorteriaContraria =
              getTrayectoriaPorteriaContraria(posicionJugadores[i], sp);
