...
public class Chuts {
  ...
  private CommandHitBall getMejorPase(int indexBalon) {
    ...
    double
        factor =
        factorPrincipal - (((jugOrigen.getY() + 52.5) / Constants.LARGO_CAMPO_JUEGO) * 8.0);
    for (int i = 10; i > 0; i--) {
      if (i == indexBalon) {
        continue;
      }
  ...
  private Position getDestinoTiro() {
    if (this.porteroRival.getX() > 0 || (this.porteroRival.getX() == 0
                                         && this.sp.ballPosition().getX() < 0)) {
  ...
  /**
   * Comprueba si el jugador de mi equipo est� m�s adelantado que todos los rivales (excepto el
   * portero)
   */
