 ...
 * @author MaN
 */
public abstract class MSGAbstractJugadorPropio extends MSGAbstractJugadorDetalle
    implements MSGJugadorPropio {
  ...
  public MSGAbstractJugadorPropio(int index, Color colorPelo, Color colorPiel, String nombre,
                                  int numero, MSGEstadisticas estadisticas) {
  ...
  protected Command irABalon() {
    ...
    if (context.getJugadoresCercanosBalonData() != null) {
      ...
      posicionBalon =
          context.isAtaque() ? balonData.getPropio().getPunto().getPuntoXY()
                             : balonData.getContrario().getPunto().getPuntoXY();
    } else {
  ...
  public boolean isJugadorMasCercano() {
    if (context.getSituacionPartido().isStarts()) {
      ...
      return iteraciones != null && iteraciones.getPropio() != null
             && iteraciones.getPropio().getJugador() == this;
    }
  }
  ...
  protected final PaseData getMejorPase() {
    ...
    List<PaseData> totalPases = new LinkedList<>();
    for (Map.Entry<MSGJugadorPropio, PaseData> pase : pases.entrySet()) {
      totalPases.add(pase.getValue());
    }
    List<Comparator<PaseData>>
        comparators =
        Arrays.<Comparator<PaseData>>asList(new MSGSituacionPartidoContext.AlturaComparator());
    ...
    for (PaseData pase : totalPases) {
      MSGSituacionPartidoUIListener.getInstance().segmentosEvaluarPase
          .add(new Segmento(this.posicion, pase.getPuntoDestino().getPuntoXY()));
    }
  ...
  protected CommandHitBall tiroAPuerta() {
    double
        distancia =
        TrigonometriaUtils.getDistancia(posicion, new Punto(Constants.centroArcoSup));
  ...
  protected Command despeja() {
    List<MSGJugadorPropio>
        jugadoresADespejar =
        MSGUtils.getJugadorADespejar(this, context.getJugadoresPropios());
  ...
  protected Command desmarque() {
    ...
    return desmarque == null ? null : new CommandMoveTo(index, new Position(desmarque.getX(),
                                                                            desmarque.getY()));
  }
    ...
    if (!getAlineacion().getPosicion().equals(posicionBalon.getPuntoXY())) {
      ...
      for (Punto p : TrigonometriaUtils.getInterseccion(segmento, getAlineacion().getArea())) {
        if (puntoIra == null) {
          ...
          double
              d1 =
              TrigonometriaUtils.getDistancia(puntoIra.getPuntoXY(), posicionBalon.getPuntoXY());
  ...
  protected Command cubreContrario() {
    ...
    if (jugadorAMarcar.getPosicion().getY() <= context.getBalon().getPosicion().getY()) {
      Segmento
          tiroAPuerta =
          new Segmento(jugadorAMarcar.getPosicion(), new Punto(Constants.centroArcoInf));
      Punto[]
          puntoMarcaje =
          TrigonometriaUtils.getInterseccion(tiroAPuerta, jugadorAMarcar.getAreaCubierta(5));
      ...
      MSGSituacionPartidoUIListener.getInstance().segmentosMarcaje
          .add(new Segmento(getPosicion(), punto));
    }
    return comando;
  }
}
