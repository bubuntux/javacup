...
public class AccionRealizada {
	...
	*/
	...
	public AccionRealizada () {
		...
		tablaSectores = new Hashtable<String, Hashtable<String, Integer[]>>();
	...
	public void actualizaInformacion (String[] informacion, int contador, int acierto, int peso) {
		...
		if (informacion != null && informacion.length == NUM_INDICES_TRAD) {
			...
			if (sector != null && accion != null) {
				...
				tablaSectores.get(sector).get(accion)[IND_CONTADOR] = tablaSectores.get(sector).get(accion)[IND_CONTADOR] + contador;
				tablaSectores.get(sector).get(accion)[IND_ACIERTOS] = tablaSectores.get(sector).get(accion)[IND_ACIERTOS] + acierto;
				tablaSectores.get(sector).get(accion)[IND_PESO] = tablaSectores.get(sector).get(accion)[IND_PESO] + peso;
			}
			else {
	...
	public Integer decide (Accion[] acciones, GameSituations sp, char sectorActivo) {
		...
		try {
			if (acciones == null) { 
				...
				Integer[] informacionFinal = dameInformacion(traduceAccion(acciones[0], sp.ballPosition(), sectorActivo));
				if (acciones[0] != null) {
				...
				for (int i=1; i<acciones.length; i++) {
					...
					Integer[] informacionAux = dameInformacion(traduceAccion(acciones[i], sp.ballPosition(), sectorActivo));
					...
					if (informacionFinal == null && informacionAux != null) {
						...
						if ( Math.abs(iFinal_Contador - iAux_Contador) < diferenciaEjecuciones) {
							...
							// Caso en el que no ganamos ampliamente.
								...
								if ((porcentAciertos1 >= porcentajeAciertos) && (porcentAciertos2 >= porcentajeAciertos)) {
