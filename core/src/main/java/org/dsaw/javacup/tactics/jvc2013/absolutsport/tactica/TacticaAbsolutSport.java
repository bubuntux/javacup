	        	...
	        	// Si el destino del pase se encuentra dentro del area y tiene tiro a puerta vac�a se la pasamos.
	        		...
	        		if (sp.ballPosition().getY() > ((Constants.LARGO_CAMPO_JUEGO /2) - Constants.ANCHO_AREA_GRANDE) &&
		                ...
		                */
	                	...
	                	Integer eleccion = acciones.decide(new Accion[]{pasar, regatear, paseAlHueco}, sp, sectorActivo);
        ...
        if (tirar.tiroSeguro()) {
        	...
        	// No tenemos un �ngulo suficiente para realizar el tiro.
    			...
    			*/
    			...
    			AccionPasarAlHueco pasarAlHueco = gen.pasarAlHueco(sp, jugador, arrayJugadores, arrayRivales, Constants.posteDerArcoSup.getX() + 1.5);
    			if (pasarAlHueco != null && !pasarAlHueco.paseHaciaElArea()) {
    				pasarAlHueco = null;
    			}
    				
    			Integer eleccion = acciones.decide(new Accion[]{tirar, regatear, pasarAlHueco}, sp, sectorActivo);
    			if (eleccion != null && eleccion < 3) {
    				if (eleccion == 0) {
    					realizaTiro(tirar, sp);
    				}
        ...
        if (jugador.getIndice() != 0) {
            ...
            // a realizar.
				...
				if (rivalCercanoDestino != null && rivalCercanoOrigen != null) {
					...
					double distOrigen = sp.rivalPlayers()[rivalCercanoOrigen].distance(sp.myPlayers()[jugador.getIndice()]);
					double distDestino = sp.rivalPlayers()[rivalCercanoDestino].distance(sp.myPlayers()[pasar.jugadorDestino().getIndice()]);
