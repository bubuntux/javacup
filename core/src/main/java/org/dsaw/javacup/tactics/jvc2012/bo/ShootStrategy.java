...
public class ShootStrategy implements SoccerStrategy {
  ...
  private boolean isShootSituation(double power) {
    ...
    return currentSituation.ballPosition().distance(Constants.centroArcoSup)
           <= THRESHOLD * factor * Constants.getVelocidadRemate(power)
  ...
  private ShootInfo evaluateShoot(double power, double angle, double angleVert) {
    TrajectoryMasia
        trajectory =
        new TrajectoryMasia(currentSituation.ballPosition(), Constants.getVelocidadRemate(power),
                            angle, angleVert);
    double fitness = 0, f;
  ...
  private double calculateFitness(Position position, double z, int iter, Position last,
    ...
    int
        oppIterToBall =
        ChimueloTacticUtils.calculateIterToBall(position, z, currentSituation.rivalPlayers(),
  ...
  public List<Command> makeStrategy() {
    ...
    if (isShootSituation(power)) {
      ...
      for (int player : canShoot) {
        commands.add(new CommandHitBall(player, shoot.angle * 180 / Math.PI, 1,
