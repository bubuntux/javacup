package org.dsaw.javacup.tactics.jvc2012.hortalezablamers.utils.saques;
...
public class SaquesABalonParado {
  ...
  public static List<Command> sacarABalonParado(GameSituations gs) {
    ...
    if (rematan.length > 0) {
      ...
      if (gs.ballPosition().equals(HBConstants.POSICION_SAQUE_PUERTA_IZDA) || gs.ballPosition()
          .equals(HBConstants.POSICION_SAQUE_PUERTA_DCHA)) {
        comandos.add(new CommandHitBall(rematan[0], Constants.centroCampoJuego,
                                        HBConstants.FUERZA_GOLPEO_MAXIMA,
                                        HBConstants.ANGULO_GOLPEO_DESPEJE));
      }
      // para el saque de centro, saca al centro del campo propio
      else if (gs.ballPosition().equals(Constants.centroCampoJuego)) {
        comandos.add(new CommandHitBall(rematan[0], HBConstants.POSICION_CENTRO_CAMPO_PROPIO,
                                        HBConstants.FUERZA_GOLPEO_MEDIA,
                                        HBConstants.ANGULO_GOLPEO_PASE));
      }
      // para el saque de corner, saca al punto de penalty
      else if (gs.ballPosition().equals(Constants.cornerSupIzq) || gs.ballPosition()
          .equals(Constants.cornerSupDer)) {
        comandos.add(
            new CommandHitBall(rematan[0], Constants.penalSup, HBConstants.FUERZA_GOLPEO_MAXIMA,
                               HBConstants.ANGULO_GOLPEO_CUELGUE));
      } else {
