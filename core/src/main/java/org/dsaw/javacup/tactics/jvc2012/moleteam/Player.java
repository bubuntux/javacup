...
public class Player {
    ...
    if (recoveryPosition != null && this.canKick == false) {
      ...
      if (Variables.ballIsOurs) {
        if (this.playerIndex == Variables.ballInPlayerIndex && vars.gs.canKick().length > 0) {
          Position3D
              ballPosition3D =
              new Position3D(vars.gs.ballPosition(), vars.gs.ballAltitude());
          vars.AddCommands(new Shoot(this.playerIndex,
                                     Constants.centroArcoSup,
                                     1,
                                     25));
          if (this.playerIndex != 0) {
            if (ballPosition3D.y > 47
                && Math.abs(ballPosition3D.x) > Constants.ANCHO_AREA_GRANDE / 2) {
              CheckForPassAdvance(vars, MConstants.ShootTrayectory.backward);
            } else if (
                CountPlayersGreaterThanY(vars.myPlayers, this.playerPosition.y, this.playerIndex)
                == 0) {
  ...
  public Position getGoalKeeperPosition(Variables vars) {
    ...
    if (goalKeeperPosition == null) {
      goalKeeperPosition =
          Helpers.IntersectionOfTwoSegments(Constants.posteIzqArcoInf.movePosition(0, 4),
                                            Constants.posteDerArcoInf.movePosition(0, 4),
                                            Constants.centroArcoInf, vars.gs.ballPosition());
    }
    if (goalKeeperPosition == null) {
      goalKeeperPosition =
          Helpers.IntersectionOfTwoSegments(Constants.posteDerArcoInf,
                                            Constants.posteDerArcoInf.movePosition(0, 4),
                                            Constants.centroArcoInf, vars.gs.ballPosition());
    }
    if (goalKeeperPosition == null) {
      goalKeeperPosition =
          Helpers.IntersectionOfTwoSegments(Constants.posteIzqArcoInf,
                                            Constants.posteIzqArcoInf.movePosition(0, 4),
                                            Constants.centroArcoInf, vars.gs.ballPosition());
    }
    if (goalKeeperPosition == null || !vars.gs.ballPosition().isInsideGameField(0)) {
      goalKeeperPosition =
          Constants.centroArcoInf
              .moveAngle(Constants.centroArcoInf.angle(vars.gs.ballPosition()), 5);
    }
  ...
  public Position getBasicDefencePosition(Variables vars) {
    ...
    Position
        rivalPosition =
        vars.rivalPlayers.GetPlayerByIndex(this.rivalToDefenceIndex).playerPosition.to2D();
    return rivalPosition
        .moveAngle(rivalPosition.angle(Constants.centroArcoInf), Constants.JUGADORES_SEPARACION);
  ...
  public Position getBasicAttackPosition(Variables vars) {
    if (this.playerIndex == Variables.ballToPlayerIndex
        && Variables.ballToEstimatedPosition3D != null) {
    ...
    switch (this.playerRole) {
      ...
      case attackLine: {
        if (Variables.playerAloneEnabled) {
          ...
          Player
              nearestRival =
              vars.rivalPlayers.GetPlayerByIndex(
                  this.playerPosition.to2D().nearestIndex(vars.rivalPlayers.positions));
          if (this.playerPosition.to2D().distance(nearestRival.playerPosition.to2D()) < 3
              && this.playerPosition.to2D().distance(defaultPosition) < 1) {
  ...
  public boolean rivalsAreInGoAheadTrayectory(Variables vars, double angle, double power,
                                              double angleZ) {
    ArrayList<Position3D>
        expectedTrayectory =
        getTrayectoryExpectedFirstTimeInFloor(vars, angle, power, angleZ, false);
    ...
    for (int iteration = 0; iteration < expectedTrayectory.size(); iteration++) {
      ...
      if (estimated3DBallPosition.z <= Constants.ALTURA_CONTROL_BALON) {
        ...
        int[]
            nearestRivalIndexes =
            estimatedBallPosition.nearestIndexes(vars.rivalPlayers.positions);
        ...
        for (int i = 0; i < nearestIndexes.length; i++) {
          ...
          if (this.playerPosition.to2D().distance(myPlayer.playerPosition.to2D())
              < Constants.JUGADORES_SEPARACION + Constants.DISTANCIA_CONTROL_BALON
              &&
              Math.toDegrees(this.playerPosition.to2D().angle(myPlayer.playerPosition.to2D())) + 5
              > angle
              &&
              angle
              > Math.toDegrees(this.playerPosition.to2D().angle(myPlayer.playerPosition.to2D())) - 5
          ...
          double
              iterationsToEstimatedBallPosition =
              Math.round(((this.playerPosition.to2D().distance(estimatedBallPosition)
                           - Constants.DISTANCIA_CONTROL_BALON) / (this.getRealPlayerSpeed(vars)
                                                                   * (iteration))));
          double
              rivalIterationsToEstimatedBallPosition =
              Math.round(((rivalPlayer.playerPosition.to2D().distance(estimatedBallPosition)
                           - Constants.DISTANCIA_CONTROL_BALON) / (
                              rivalPlayer.getRealPlayerSpeed(vars) * (iteration))));
          if (iterationsToEstimatedBallPosition != 0
              && Math.abs(iterationsToEstimatedBallPosition) >= Math
              .abs(rivalIterationsToEstimatedBallPosition)
              && Math.abs(rivalPlayer.playerPosition.to2D().distance(estimatedBallPosition)) < Math
              .abs(this.playerPosition.to2D().distance(estimatedBallPosition))) {
            return true;
          }
        }
      }
  ...
  public Position3D getTrayectoryExpectedPosition(Variables vars, double shootAngle,
                                                  double shootPower, double shootAngleZ,
                                                  boolean returnNullIfOutOfPlayField) {
    ArrayList<Position3D>
        coords =
        getTrayectoryExpected(vars, shootAngle, shootPower, shootAngleZ,
                              returnNullIfOutOfPlayField);
    return coords.get(coords.size() - 1);
  }

  public Position3D getTrayectoryExpectedPositionFirstTimeInFloor(Variables vars, double shootAngle,
                                                                  double shootPower,
                                                                  double shootAngleZ,
                                                                  boolean returnNullIfOutOfPlayField) {
    ArrayList<Position3D>
  ...
  public ArrayList<Position3D> getTrayectoryExpectedFirstTimeInFloor(Variables vars,
                                                                     double shootAngle,
                                                                     double shootPower,
                                                                     double shootAngleZ,
                                                                     boolean returnNullIfOutOfPlayField) {
  ...
  public ArrayList<Position3D> getTrayectoryExpected(Variables vars, double shootAngle,
                                                     double shootPower, double shootAngleZ,
                                                     boolean returnNullIfOutOfPlayField) {
    ...
    double
        angleZ =
        Math.max(0, Math.min(shootAngleZ, Constants.ANGULO_VERTICAL_MAX)) * (Math.PI / 180d);
    int maxIterations = 1000;

    AbstractTrajectory
        trajectory =
        new AirTrajectory(Math.cos(angleZ) * speed, Math.sin(angleZ) * speed, 0, 0);
    ...
    for (int step = 1; step < maxIterations; ++step) {
      ...
      Position3D
          nextBall =
          new Position3D(vars.gs.ballPosition().getX() + trX * Math.cos(angle),
                         vars.gs.ballPosition().getY() + trX * Math.sin(angle), trY * 2, step,
                         trY > oldtrY);
  ...
  public boolean rivalsAreInPassTrayectory(Variables vars, Player destinationPlayer,
                                           double shootAngle, double shootPower,
                                           double shootAngleZ) {
    double
        passDistance =
        vars.gs.ballPosition().distance(destinationPlayer.playerPosition.to2D())
        - Constants.DISTANCIA_CONTROL_BALON;
    ArrayList<Position3D>
        expectedTrayectory =
        getTrayectoryExpected(vars, shootAngle, shootPower, shootAngleZ, false);
    for (int iteration = 1; iteration < expectedTrayectory.size(); iteration++) {
      ...
      if (estimated3DBallPosition.z <= Constants.ALTURA_CONTROL_BALON) {
        ...
        int[]
            nearestRivalIndexes =
            estimated3DBallPosition.to2D().nearestIndexes(vars.rivalPlayers.positions);
        for (int i = 0; i < nearestRivalIndexes.length; i++) {
          Player rivalPlayer = vars.rivalPlayers.get(nearestRivalIndexes[i]);
          double
              iterationsToEstimatedBallPosition =
              Math.abs(Math.round(
                  (destinationPlayer.playerPosition.to2D().distance(estimated3DBallPosition.to2D()))
                  / (destinationPlayer.getRealPlayerSpeed(vars) * (iteration))));
          double
              rivalIterationsToEstimatedBallPosition =
              Math.abs(Math.round(
                  (rivalPlayer.playerPosition.to2D().distance(estimated3DBallPosition.to2D())) / (
                      rivalPlayer.getRealPlayerSpeed(vars) * (iteration))));
          if (iterationsToEstimatedBallPosition != 0
      ...
      if (Variables.ballIsOurs) {
        ...
        for (int i = 1; i < recoveryBallProvability.length; i++) {
          ...
          if (recoveryBallProvability.length != 2
              && this.playerIndex == 0
              && vars.gs.ballPosition()
                     .distance(vars.myPlayers.GetPlayerByIndex(0).playerPosition.to2D()) > 10) {
            return null;
          }
  ...
  public void CheckForShoot(Variables vars) {
    ...
    if (vars.rivalPlayers.positions[0].distance(Constants.centroArcoSup) < 0) {
      if (vars.rivalPlayers.positions[0].distance(Constants.posteIzqArcoSup)
          < vars.rivalPlayers.positions[0].distance(Constants.posteDerArcoSup)) {
      ...
      angleZloop:
      for (double angleZ = 0; angleZ <= 8; angleZ = angleZ + 1) {
        powerloop:
        for (double power = 1; power > 0; power = power - 0.1) {
          ArrayList<Position3D>
              expectedTrayectory =
              getTrayectoryExpected(vars, angle, power, angleZ, false);
          if (expectedTrayectory == null) {
            break powerloop;
          }
          coordsloop:
          for (Position3D c3d : expectedTrayectory) {
            ...
            Players
                nearestRivalPlayersToExpectedTrayectory =
                c3d.nearestPlayers(vars, vars.rivalPlayers);
            for (Player nearestRivalPlayerToExpectedTrayectory : nearestRivalPlayersToExpectedTrayectory) {
              boolean
                  nearestRivalPlayerCanHandleBall =
                  (nearestRivalPlayerToExpectedTrayectory.getRealPlayerSpeed(vars) * c3d.iteration)
                  + Constants.DISTANCIA_CONTROL_BALON_PORTERO >= Math.abs(
                      nearestRivalPlayerToExpectedTrayectory.playerPosition.to2D()
                          .distance(c3d.to2D()));
...
//               
  ...
  public void CheckForPassAdvance(Variables vars,
    ...
    angleloop:
    for (int angle = minAngle; angle < maxAngle; angle = angle + 5) {
      ...
      angleZloop:
      for (double angleZ = 0; angleZ <= 45; angleZ = angleZ + 5) {
        powerloop:
        for (double power = 1; power > 0.4; power = power - 0.1) {
          ArrayList<Position3D>
              expectedTrayectory =
              getTrayectoryExpected(vars, angle, power, angleZ, false);
          if (expectedTrayectory == null) {
          ...
          ArrayList<Position3D>
              expectedTrayectoryErrorMin =
              getTrayectoryExpected(vars, angleErrorMin, power, angleZ, false);
          ArrayList<Position3D>
              expectedTrayectoryErrorMax =
              getTrayectoryExpected(vars, angleErrorMax, power, angleZ, false);
          coordsloop:
          for (Position3D c3d : expectedTrayectory) {
            ...
            Players
                nearestRivalPlayersToExpectedTrayectory =
                c3d.nearestPlayers(vars, vars.rivalPlayers);
            for (int rp = 0; rp < Math.min(1, nearestRivalPlayersToExpectedTrayectory.size());
                 rp++) {
              Player
                  nearestRivalPlayerToExpectedTrayectory =
                  nearestRivalPlayersToExpectedTrayectory.get(rp);
              double
                  nearestRivalMettersInC3DIteration =
                  nearestRivalPlayerToExpectedTrayectory.getRealPlayerSpeed(vars) * c3d.iteration;
              double
                  nearestRivalDistanceToTrayectory =
                  Math.abs(nearestRivalPlayerToExpectedTrayectory.playerPosition.to2D()
                               .distance(c3d.to2D()));
              boolean nearestRivalPlayerCanHandleBallErrorMin = false;
              boolean nearestRivalPlayerCanHandleBallErrorMax = false;
              boolean
                  nearestRivalPlayerCanHandleBall =
                  nearestRivalMettersInC3DIteration + Constants.DISTANCIA_CONTROL_BALON_PORTERO
                  >= nearestRivalDistanceToTrayectory;
              if (MConstants.ErrorCalculationEnabled
                  && expectedTrayectoryErrorMin.size() <= c3d.iteration) {
                double
                    nearestRivalDistanceToTrayectoryErrorMin =
                    Math.abs(nearestPlayerToExpectedTrayectory.playerPosition.to2D()
                                 .distance(expectedTrayectoryErrorMin.get(c3d.iteration).to2D()));
                nearestRivalPlayerCanHandleBallErrorMin =
                    nearestRivalMettersInC3DIteration + Constants.DISTANCIA_CONTROL_BALON_PORTERO
                    >= nearestRivalDistanceToTrayectoryErrorMin;
              }
              if (MConstants.ErrorCalculationEnabled
                  && expectedTrayectoryErrorMax.size() <= c3d.iteration) {
                double
                    nearestRivalDistanceToTrayectoryErrorMax =
                    Math.abs(nearestPlayerToExpectedTrayectory.playerPosition.to2D()
                                 .distance(expectedTrayectoryErrorMax.get(c3d.iteration).to2D()));
                nearestRivalPlayerCanHandleBallErrorMax =
                    nearestRivalMettersInC3DIteration + Constants.DISTANCIA_CONTROL_BALON_PORTERO
                    >= nearestRivalDistanceToTrayectoryErrorMax;
              }
              if (nearestRivalPlayerCanHandleBall || (MConstants.ErrorCalculationEnabled && (
                  nearestRivalPlayerCanHandleBallErrorMin
                  || nearestRivalPlayerCanHandleBallErrorMax))) {
                break coordsloop;
              }
            }
            nearestPlayerToExpectedTrayectory =
                c3d.nearestPlayer(vars, vars.myPlayers, this.playerIndex);
            if (nearestPlayerToExpectedTrayectory != null) {
              double
                  nearestPlayerMettersInC3DIteration =
                  nearestPlayerToExpectedTrayectory.getRealPlayerSpeed(vars) * c3d.iteration;
              double
                  nearestPlayerDistanceToTrayectory =
                  Math.abs(
                      nearestPlayerToExpectedTrayectory.playerPosition.to2D().distance(c3d.to2D()));
              boolean nearestPlayerCanHandleBallErrorMin = false;
              boolean nearestPlayerCanHandleBallErrorMax = false;
              boolean
                  nearestPlayerCanHandleBall =
                  nearestPlayerMettersInC3DIteration + Constants.DISTANCIA_CONTROL_BALON_PORTERO
                  >= nearestPlayerDistanceToTrayectory;
              if (MConstants.ErrorCalculationEnabled
                  && expectedTrayectoryErrorMin.size() <= c3d.iteration) {
                double
                    nearestPlayerDistanceToTrayectoryErrorMin =
                    Math.abs(nearestPlayerToExpectedTrayectory.playerPosition.to2D()
                                 .distance(expectedTrayectoryErrorMin.get(c3d.iteration).to2D()));
                nearestPlayerCanHandleBallErrorMin =
                    nearestPlayerMettersInC3DIteration + Constants.DISTANCIA_CONTROL_BALON_PORTERO
                    >= nearestPlayerDistanceToTrayectoryErrorMin;
              }
              if (MConstants.ErrorCalculationEnabled
                  && expectedTrayectoryErrorMax.size() <= c3d.iteration) {
                double
                    nearestPlayerDistanceToTrayectoryErrorMax =
                    Math.abs(nearestPlayerToExpectedTrayectory.playerPosition.to2D()
                                 .distance(expectedTrayectoryErrorMax.get(c3d.iteration).to2D()));
                nearestPlayerCanHandleBallErrorMax =
                    nearestPlayerMettersInC3DIteration + Constants.DISTANCIA_CONTROL_BALON_PORTERO
                    >= nearestPlayerDistanceToTrayectoryErrorMax;
              }
              if (nearestPlayerCanHandleBall || (nearestPlayerCanHandleBall
                                                 && MConstants.ErrorCalculationEnabled && (
                  nearestPlayerCanHandleBallErrorMin || nearestPlayerCanHandleBallErrorMax))) {
                isGoodPass = true;
      ...
      if (isGoodPass && nearestPlayerToExpectedTrayectory != null) {
        if (bestCommand == null
            ||
            (vars.myPlayers.GetPlayerByIndex(bestCommand.destinationPlayerIndex).playerPosition.y
             < nearestPlayerToExpectedTrayectory.playerPosition.y)) {
          bestCommand =
              new Pass(this.playerIndex, nearestPlayerToExpectedTrayectory.playerIndex, finalAngle,
                       finalPower, finalAngleZ);
  ...
  public void CheckForPass(Variables vars,
    ...
    for (int i = 0; i < nearestIndexes.length; i++) {
      ...
      double
          angleToNearestPosition =
          Math.toDegrees(currentPlayerPosition.angle(nearestPlayer.playerPosition.to2D()));
      double
          distanceToNearestPosition =
          currentPlayerPosition.distance(nearestPlayer.playerPosition.to2D());
      ...
      if ((!this
          .rivalsAreInPassTrayectory(vars, nearestPlayer, angleToNearestPosition, hitPower, angleZ)
           && nearestIndex != 0
...
class Pass extends MCommand {
  ...
  public Pass(int playerIndex, int destinationPlayerIndex, double angle, double power,
              double angleZ) {
    this.playerIndex = playerIndex;
