        ...
        for (int i = 0; i < jugadores.length; i++) {
          if (i != jugadorQueSaca.getId()) {
            if (sp.ballPosition().getY() > 15) {
              ...
              comandos.add(new CommandMoveTo(i, new Position(jugadores[i].getPosicionBase().getX(),
                                                             rival.getPosicion().getY())));
            } else {
...
//    			}
//    			else 
          if (sp.ballPosition().getY() <= Constants.ANCHO_AREA_GRANDE + Constants.centroArcoInf
              .getY()) {
            comandos.add(
                new CommandHitBall(jugadorQueSaca.getId(), jugadores[10].getPosicion(), 1, 35));
          } else if ((jugadorQueSaca.getRivalesZona() == null
                      || (jugadorQueSaca.getRivalesZona() != null
                          && jugadorQueSaca.getRivalesZona().length == 0))
                     && (rival.getPosicion().distance(jugadorQueSaca.getPosicion()) > 20)) {

            comandos.add(new CommandHitBall(jugadorQueSaca.getId(),
                                            new Position(-jugadorQueSaca.getPosicion().getX(),
                                                         jugadorQueSaca.getPosicion().getY() + 5),
                                            0.20, false));
            ...
            if (pase != null && pase.getJugadorDestino() != null) {
              comandos.add(new CommandHitBall(jugadorQueSaca.getId(), pase.getPosicionPase(),
                                              TacticasUtil
                                                  .getPotenciaPase(jugadorQueSaca.getPosicion(),
                                                                   pase.getPosicionPase()), 30));
              comandos
                  .add(new CommandMoveTo(pase.getJugadorDestino().getId(), pase.getPosicionPase()));
            } else {
    ...
    if (jugadorQueSaca != null && sp.getRecoveryBall().length > 1
        && sp.getRecoveryBall()[1] != portero && jugadorQueSaca.getId() != sp
        .getRecoveryBall()[1]) {
  ...
  private static LinkedList<Command> tacticaDefensa(GameSituations sp, Jugador[] jugadores,
    ...
    ArrayList<Rival>
        rivalesDetras =
        TacticasUtil.calcularRivalesDetrasLinea(rivales, MovimientosDefensa.LINEA_CENTROCAMPO_BASE);
    Jugador jugadorAux;
    ...
    for (int i = 0; i < rivalesDetras.size(); i++) {
      ...
      if (!rivalesDetras.get(i).isMarcado()) {
        jugadorAux =
            TacticasUtil.calcularMasCercanoSinMarca(rivalesDetras.get(i).getPosicion(), jugadores,
                                                    Jugador.DEFENSA);
        if (jugadorAux == null) {
          jugadorAux =
              TacticasUtil.calcularMasCercanoSinMarca(rivalesDetras.get(i).getPosicion(), jugadores,
    ...
    double[]
        cortesVerticalesDefensa =
        JugadoresManager.calcularCortes(defensas, Constants.ANCHO_CAMPO_JUEGO / 2);
    double[]
        cortesVerticalesCentro =
        JugadoresManager.calcularCortes(centro, Constants.ANCHO_CAMPO_JUEGO / 2);
    double[]
        cortesVerticalesDelantera =
        JugadoresManager.calcularCortes(delanteros, Constants.ANCHO_CAMPO_JUEGO / 2);
    int j = 0, x = 0, y = 0;
    ...
    for (int i = 0; i < jugadores.length; i++) {
      ...
      if (jugadores[i].getRol() == Jugador.DEFENSA) {
        comandos.add(MovimientosDefensa
                         .moverDefensa(jugadores[i], posicionBalon, jugadores, rivales, sp,
                                       new Position(cortesVerticalesDefensa[j++],
                                                    MovimientosDefensa.getLineaDefensaActual())));
      } else if (jugadores[i].getRol() == Jugador.CENTROCAMPISTA) {
        comandos.add(MovimientosDefensa
                         .moverCentrocampo(jugadores[i], posicionBalon, jugadores, rivales, sp,
                                           new Position(cortesVerticalesCentro[x++],
                                                        MovimientosDefensa
                                                            .getLineaCentroActual())));
      } else if (jugadores[i].getRol() == Jugador.DELANTERO) {
        comandos.add(MovimientosDefensa
                         .moverDelantera(jugadores[i], posicionBalon, jugadores, rivales, sp,
                                         new Position(cortesVerticalesDelantera[y++],
                                                      MovimientosDefensa
                                                          .getLineaDelanteraActual())));
      }
    ...
    if (!TacticasUtil.tengoBalon(masCercaBalon.getId(), sp.canKick())
        && sp.ballAltitude() <= Constants.ALTURA_CONTROL_BALON) {
  ...
  private static LinkedList<Command> tacticaAtaque(GameSituations sp, Jugador[] jugadores,
    ...
    if (sp.canKick().length > 0) {
      ...
      if (tiro != null) {
      ...
      } else if (conBalon.getRivalesZona() == null || (conBalon.getRivalesZona() != null
                                                       && conBalon.getRivalesZona().length == 0)) {
        ...
        comandos.add(new CommandHitBall(conBalon.getId(), pase.getPosicionPase(),
                                        ...
                                        TacticasUtil.getAlturaPase(conBalon.getPosicion(),
                                                                   pase.getPosicionPase())));

        comandos.add(new CommandMoveTo(pase.getJugadorDestino().getId(),
                                       pase.getJugadorDestino().getPosicion().movePosition(0, 10)));
        ...
        comandos.add(new CommandHitBall(conBalon.getId(), pos.movePosition(0, 0),
                                        TacticasUtil.getPotenciaPase(conBalon.getPosicion(),
                                                                     pos),
                                        TacticasUtil.getAlturaPase(conBalon.getPosicion(), pos)));
      }
      ...
      comandos.add(
          new CommandMoveTo(conBalon.getId(), TacticasUtil.puedoRecuperar(conBalon, rivales, sp)));
    }
    ...
    for (int i = 0; i < jugadores.length; i++) {
      ...
      if (jugadores[i].getRol() == Jugador.DEFENSA && !colocados.contains(jugadores[i])) {
        comandos.add(MovimientosAtaque
                         .moverDefensa(jugadores[i], posicionBalon, jugadores, rivales, sp,
                                       new Position(cortesVerticalesDefensa[j++],
                                                    MovimientosAtaque.getLineaDefensaActual())));
      } else if (jugadores[i].getRol() == Jugador.CENTROCAMPISTA && !colocados
          .contains(jugadores[i])) {
        comandos.add(MovimientosAtaque
                         .moverCentrocampo(jugadores[i], posicionBalon, jugadores, rivales, sp,
                                           new Position(cortesVerticalesCentro[x++],
                                                        MovimientosAtaque.getLineaCentroActual())));
    ...
    if (saque && sp.canKick().length > 0) {
      comandos.add(
          new CommandHitBall(sp.canKick()[0], jugadores[10].getPosicion().movePosition(0, 5), 1,
                             40));
      saque = false;
    }
  ...
  private static Command movimientoDelPortero(Jugador[] jugadores, Rival[] rivales,
                                              GameSituations sp) {
    /*************** COLOCACION PORTERO *****************************/
...
//    		}
      ...
      if (rival.getPosicion().distance(posRecuperacion)
          <= jugadores[portero].getPosicion().distance(posRecuperacion) - 6) {
...
//			}
  ...
  public static Jugador[] calcularCambioTactica(Jugador[] jugadores, GameSituations sp) {
    ...
    if ((sp.rivalGoals() - sp.myGoals() > 1 || (sp.myGoals() < sp.rivalGoals()
                                                && (Constants.ITERACIONES - sp.iteration())
                                                   < 1200))) {
      if (!tactica433) {
        tactica433 = true;
        tactica442 = false;
