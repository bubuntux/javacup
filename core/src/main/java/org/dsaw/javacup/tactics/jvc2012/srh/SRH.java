  ...
  private void no_saca_el_rival() {
    ...
    for (int delantero : getRivalDelanteros()) {
      ...
      if (i < 8) {
        ...
        comandos.add(new CommandMoveTo(indiceDefensaMasCerca,
                                       getPositionDondeCubrirAlRival(rivales[delantero])));
      }
    }
    ...
    double
        posXFinal =
        sitAct.ballPosition().getX() / (Constants.ANCHO_CAMPO_JUEGO / Constants.LARGO_ARCO);
    comandos.add(new CommandMoveTo(0, new Position(posXFinal, Constants.centroArcoInf.getY()
                                                              + Constants.ANCHO_AREA_CHICA / 2)));
  ...
  private Command getComandoSegunEstrategia(int jugador) {
    ...
    switch (jugador) {
      ...
      case 4:
        ...
        //Pase
        comandoSalida =
            new CommandHitBall(jugador, jugadores[jugadorMasSolo], potenciaPase, angulo);
        break;
      case 5:
      case 6:
      case 7:
        ...
        //Pase
        comandoSalida =
            new CommandHitBall(jugador, jugadores[jugadorMasSolo], potenciaPase,
                               anguloPaseDelantero);
        break;
      case 8:
      case 9:
...
//				comandoSalida = new CommandHitBall(jugador, jugadores[jugadorMasSolo], potenciaPase, anguloPaseDelantero);
  ...
  private Position getPositionDondeCubrirAlRival(Position rival) {
    ...
    if (sitAct.ballAltitude() == 0) {
      pos_final =
          pos_final.moveAngle(pos_final.angle(pelota),
                              Constants.DISTANCIA_CONTROL_BALON - Constants.VELOCIDAD_MAX);
    } else {
      pos_final =
          pos_final.moveAngle(Constants.centroArcoInf.angle(),
                              -Constants.DISTANCIA_CONTROL_BALON + Constants.VELOCIDAD_MAX);
    }
    return pos_final;
  }
