...
public class MessiLaSacaHastaConLaCho {
  ...
  public MessiLaSacaHastaConLaCho(List<Command> comandos, Datines data) {
    super();
    ...
    while (i < iterBallOpponent) {
      if (data.getPosBall(i).isInsideGameField(0)) {
        ...
        if (dist <= i * Constants.VELOCIDAD_MAX + Constants.DISTANCIA_CONTROL_BALON_PORTERO
            && data.getZBall(i) <= (inArea ? Constants.ALTO_ARCO
                                           : Constants.ALTURA_CONTROL_BALON)) {
          return data.getPosBall(
              i);//.moverAngulo(data.getPosBall(i).angulo(Constants.centroArcoInf), inArea?Constants.DISTANCIA_CONTROL_BALON_PORTERO:Constants.DISTANCIA_CONTROL_BALON, data.getPosBall(i).distancia(Constants.centroArcoInf));
        }
      } else {
  ...
  private Position move(Position posBall, int iter) {
    ...
    int
        iter1 =
        (int) Math.ceil(
            (sp.myPlayers()[index].distance(pos) - Constants.DISTANCIA_CONTROL_BALON_PORTERO)
