...
public class BasicCommandGenerator implements CommandGenerator {
   ...
   *
   * @throws IllegalArgumentException si las dos posiciones del balón son la misma en ambas
   *                                  iteraciones
   * @return trayectoria
   */
  ...
  protected boolean isInsideRivalSmallArea(Position pos) {
    boolean res = (Math.abs(pos.getX()) < Constants.LARGO_AREA_CHICA / 2 + 1
                   && Math.abs(pos.getY())
                      > Constants.LARGO_CAMPO / 2 - Constants.ANCHO_AREA_CHICA - 1
                   && pos.getY() > 0);
    return res;
  }
  ...
  protected boolean isPlayerInsideLargeArea(int playerIdx) {
    ...
    return (sp.myPlayers()[playerIdx].getX() <= (Constants.ANCHO_CAMPO_JUEGO / -2) + (
        Constants.ANCHO_AREA_GRANDE / 2) + 2
            && sp.myPlayers()[playerIdx].getX() <= (Constants.LARGO_CAMPO_JUEGO / -2) + (
        Constants.LARGO_AREA_GRANDE / 2) + 2);
  }
  ...
  protected boolean isInsideOwnSmallArea(Position pos) {
    boolean res = (Math.abs(pos.getX()) < Constants.LARGO_AREA_CHICA / 2 + 2
                   && Math.abs(pos.getY())
                      > Constants.LARGO_CAMPO / 2 - Constants.ANCHO_AREA_CHICA - 2
                   && pos.getY() < 0);
    return res;
  }
