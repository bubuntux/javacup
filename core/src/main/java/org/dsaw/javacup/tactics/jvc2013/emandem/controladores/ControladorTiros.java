...
public class ControladorTiros {
	...
	public Command getTiro(final int jugador, final GameSituations situacionPartido) {
		...
		List<Integer> todosLosJugadoresAPase = new ArrayList<Integer>();
		List<Integer> jugadoresMediosTracerosPase = new ArrayList<Integer>();
		List<Integer> jugadoresMediosDelanterosPase = new ArrayList<Integer>();
		List<Integer> jugadoresDelanterosPase = new ArrayList<Integer>();
	...
	private void getTiroDefensas(final int jugador,
		...
		for (CUADRANTE_CANCHA cuadranteCancha : CUADRANTE_CANCHA.values()) {
			if (CUADRANTE_CANCHA.CUADRANTE_2_SUP.equals(cuadranteCancha) || CUADRANTE_CANCHA.CUADRANTE_3_SUP.equals(cuadranteCancha)) {
				jugadoresPorCuadranteTmp = ControladorEstadoPartido.getInstance().getJugadoresPorCuadrantes().get(cuadranteCancha.name());
			...
			} else if (CUADRANTE_CANCHA.CUADRANTE_2_INF.equals(cuadranteCancha) || CUADRANTE_CANCHA.CUADRANTE_3_INF.equals(cuadranteCancha)) {
				jugadoresPorCuadranteTmp = ControladorEstadoPartido.getInstance().getJugadoresPorCuadrantes().get(cuadranteCancha.name());
				if (jugadoresPorCuadranteTmp != null) {
					todosLosJugadoresAPase.addAll(jugadoresPorCuadranteTmp);
					jugadoresMediosDelanterosPase.addAll(jugadoresPorCuadranteTmp);
				}
			} else if (CUADRANTE_CANCHA.CUADRANTE_1_INF.equals(cuadranteCancha) || CUADRANTE_CANCHA.CUADRANTE_4_INF.equals(cuadranteCancha)) {
				jugadoresPorCuadranteTmp = ControladorEstadoPartido.getInstance().getJugadoresPorCuadrantes().get(cuadranteCancha.name());
				if (jugadoresPorCuadranteTmp != null) {
					todosLosJugadoresAPase.addAll(jugadoresPorCuadranteTmp);
		...
		if(todosLosJugadoresAPase.size() > 0) {
			...
			while (!todosLosJugadoresAPase.isEmpty()) {
				...
				if (ControladorEstadoPartido.getInstance().getJugadoresPuedenRematar().contains(jugadorIntentaReciber)) {
			...
			if (existeJugadorParaRematar) {
				if (jugadoresMediosTracerosPase.contains(jugadorIntentaReciber)) {
					...
					this.tiro = new CommandHitBall(jugador, destinoBola, fuerzaRemateDelanteros, random.nextInt(40));
				} else if (jugadoresMediosDelanterosPase.contains(jugadorIntentaReciber)) {
					destinoBola = situacionPartido.myPlayers()[jugadorIntentaReciber];
					destinoBola.setPosition(destinoBola.getX(), destinoBola.getY() + 10);
					this.tiro = new CommandHitBall(jugador, destinoBola, fuerzaRemateDelanteros, random.nextInt(42));
	...
	private void getTiroMediosTraseros(final int jugador,
		...
		// Obtener los jugadores probables a darles pase
		for (CUADRANTE_CANCHA cuadranteCancha : CUADRANTE_CANCHA.values()) {
			if (CUADRANTE_CANCHA.CUADRANTE_2_INF.equals(cuadranteCancha) || CUADRANTE_CANCHA.CUADRANTE_3_INF.equals(cuadranteCancha)) {
				jugadoresPorCuadranteTmp = ControladorEstadoPartido.getInstance().getJugadoresPorCuadrantes().get(cuadranteCancha.name());
				if (jugadoresPorCuadranteTmp != null) {
					todosLosJugadoresAPase.addAll(jugadoresPorCuadranteTmp);
					jugadoresMediosDelanterosPase.addAll(jugadoresPorCuadranteTmp);
		...
		if(todosLosJugadoresAPase.size() > 0) {
			...
			if (existeJugadorParaRematar) {
				if (jugadoresMediosDelanterosPase.contains(jugadorIntentaReciber)) {
					destinoBola = situacionPartido.myPlayers()[jugadorIntentaReciber];
					destinoBola.setPosition(destinoBola.getX(), destinoBola.getY() + 10);
					this.tiro = new CommandHitBall(jugador, destinoBola, fuerzaRemateDefensas, ControladorUtils.getInstance().generaNumeroDobleAleatorioEntre(15, 40));
				} else if (jugadoresDelanterosPase.contains(jugadorIntentaReciber)) {
					destinoBola = situacionPartido.myPlayers()[jugadorIntentaReciber];
					destinoBola.setPosition(destinoBola.getX(), destinoBola.getY() + 10);
					this.tiro = new CommandHitBall(jugador, destinoBola, fuerzaRemateDelanteros, ControladorUtils.getInstance().generaNumeroDobleAleatorioEntre(15, 40));
				} else {
					destinoBola = Constants.centroArcoSup;
					if (tiraLadoDerecho) {
						destinoBola.setPosition((ejeXDestino * -1), destinoBola.getY());
		...
		if(todosLosJugadoresAPase.size() > 0) {
			...
			if (existeJugadorParaRematar) {
				if (jugadoresMediosDelanterosPase.contains(jugadorIntentaReciber)) {
					destinoBola = situacionPartido.myPlayers()[jugadorIntentaReciber];
					destinoBola.setPosition(destinoBola.getX(), destinoBola.getY() + 10);
					this.tiro = new CommandHitBall(jugador, destinoBola, fuerzaRemateDefensas, random.nextInt(20));
				} else if (jugadoresDelanterosPase.contains(jugadorIntentaReciber)) {
					destinoBola = situacionPartido.myPlayers()[jugadorIntentaReciber];
					destinoBola.setPosition(destinoBola.getX(), destinoBola.getY() + 10);
					this.tiro = new CommandHitBall(jugador, destinoBola, fuerzaRemateDefensas, random.nextInt(20));
					...
					this.tiro = new CommandHitBall(jugador, destinoBola, fuerzaRemateDefensas, random.nextInt(35));
				}
			} else {
				destinoBola = Constants.centroArcoSup;
				if (tiraLadoDerecho) {
				...
				this.tiro = new CommandHitBall(jugador, destinoBola, fuerzaRemateDefensas, random.nextInt(37));
			}
			...
			this.tiro = new CommandHitBall(jugador, destinoBola, fuerzaRemateDefensas, random.nextInt(57));
		}
	}
	...
	private void getTiroDelanteros(final int jugador,
		...
		if(todosLosJugadoresAPase.size() > 0) {
			...
			if (existeJugadorParaRematar) {
				if (jugadoresMediosDelanterosPase.contains(jugadorIntentaReciber)) {
					destinoBola = situacionPartido.myPlayers()[jugadorIntentaReciber];
					this.tiro = new CommandHitBall(jugador, destinoBola, fuerzaRemateDefensas, random.nextInt(60));
				} else {
					...
					if (ControladorEstadoPartido.getInstance().getJugadoresMarcados().containsKey(jugador) && 
						...
						if (ControladorEstadoPartido.getInstance().isEstaBalonEnElAreaGrandeInf()) {
							destinoBola = Constants.centroArcoSup;
							ejeXDestino = ControladorUtils.getInstance().generaNumeroDobleAleatorioEntre(0, ((Constants.LARGO_ARCO / 2) - 0.5));
...
//									new Recta(new Posicion(x, y), new Posicion(x, y)));
							...
							this.tiro = new CommandHitBall(jugador, destinoBola, fuerzaRemateDelanteros, random.nextInt(anguloTiro));
						} else { // da pase
							destinoBola = situacionPartido.myPlayers()[jugadorIntentaReciber];
							this.tiro = new CommandHitBall(jugador, destinoBola, fuerzaRemateDefensas, random.nextInt(60));
						}
