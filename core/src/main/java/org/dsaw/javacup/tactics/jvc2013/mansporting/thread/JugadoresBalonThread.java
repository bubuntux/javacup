...
public class JugadoresBalonThread<I> implements IdentificableCallable<I, JugadoresBalonData> {
  ...
  public JugadoresBalonData call() throws Exception {
    ...
    for (int i = 0; i <= trayectoria.getIteracionMax(); i++) {
      ...
      if (x != null || y != null) {
        Punto3D
            puntoFueraCampo =
            new Punto3D(x == null ? punto.getPunto().getX() : x,
                        y == null ? punto.getPunto().getY() : y, 0);
  ...
  private void handleMasCercano(JuadoresBalonContainer valores, boolean fueraCampo, Punto3D punto,
                                int i) {
    ...
    if (valores.contrario == null) {
      int
          iteracionesSumaRival =
          (int) Math.ceil(distanciaRival / jugadorDetalleRival.getAreaCubierta(1).getRadio());
      valores.contrario =
          new JugadorBalonData(jugadorDetalleRival, fueraCampo, iteracionesSumaRival, punto);
    }

    if (valores.propio == null) {
      int
          iteracionesSumPropio =
          (int) Math.ceil(distanciaPropio / jugadorDetallePropio.getAreaCubierta(1).getRadio());
      valores.propio =
          new JugadorBalonData(jugadorDetallePropio, fueraCampo, iteracionesSumPropio, punto);
    }
  }
