...
public class ElTurboVargas {

  private GameSituations sp;
  private List<Command> comandos;
  private Datines data;

  public ElTurboVargas(List<Command> comandos, Datines data) {
    this.comandos = comandos;
    this.data = data;
  }

  private boolean isShootSituation(double power, Mentalidad mentality) {
    ...
    return sp.ballPosition().distance(Constants.centroArcoSup)
           <= getThreshold(mentality) * factor * Constants.getVelocidadRemate(power)
  ...
  void execute(GameSituations sp, int[] canShoot, Mentalidad mentality) {
    this.sp = sp;
    double power = 1;
    ...
    if (isShootSituation(power, mentality)) {
      ...
      angle =
          Math.min(angle + 0.5 * Constants.getErrorAngular(1.0) / 2 * Math.PI,
                   sp.ballPosition().angle(Constants.centroArcoSup));
      double
          maxAngle =
          sp.ballPosition().angle(Constants.posteIzqArcoSup.movePosition(Constants.RADIO_BALON, 0));
      maxAngle =
          Math.max(maxAngle - 0.5 * Constants.getErrorAngular(1.0) / 2 * Math.PI,
                   sp.ballPosition().angle(Constants.centroArcoSup));
      ...
      for (int player : canShoot) {
        comandos.add(new CommandHitBall(player, shoot.angle * 180 / Math.PI, 1,
                                        shoot.angleVert * 180 / Math.PI));
      }
    }
  }
  ...
  private ShootInfo evaluateShoot(double power, double angle, double angleVert) {
    Leandorffffffff
        trajectory =
        new Leandorffffffff(sp.ballPosition(), Constants.getVelocidadRemate(power), angle,
                            angleVert);
    double fitness = 0, f;
    for (int i = 0; i < trajectory.length; i++) {
      if (i == 0) {
        ...
        f =
            calculateFitness(trajectory.positions[i], trajectory.z[i], i + 1,
  ...
  private double calculateFitness(Position position, double z, int iter, Position last,
    ...
    int
        oppIterToBall =
        data.calculateIterToBall(position, z, sp.rivalPlayers(), sp.rivalPlayersDetail(),
