...
public class TacticUtils {
  ...
  public static Attitude getAttitude(GameSituations sp) {
    ...
    if ((!sp.isStarts()) && (sp.isRivalStarts() || (iMPIterationsToBall
                                                    > iRivalIterationsToBall))) {
      return Attitude.Deffensive;
    }
    ...
    while (!bFound) {
      ...
      if (dTrajectoryBall[2] <= Constants.ALTO_ARCO) {
        for (int i = 0; i < apPositionPlayers.length; i++) {
          if (dTrajectoryBall[2] <= (aoPlayersDetail[i].isGoalKeeper() ? Constants.ALTO_ARCO
                                                                       : Constants.ALTURA_CONTROL_BALON)) {
            double
                dDistanceDetail =
                (double) iStep * Constants.getVelocidad(aoPlayersDetail[i].getSpeed()) + (
                    aoPlayersDetail[i].isGoalKeeper() ? Constants.DISTANCIA_CONTROL_BALON_PORTERO
                                                      : Constants.DISTANCIA_CONTROL_BALON);
            double
                dDistanceReal =
                apPositionPlayers[i].distance(new Position(dTrajectoryBall[0], dTrajectoryBall[1]));
