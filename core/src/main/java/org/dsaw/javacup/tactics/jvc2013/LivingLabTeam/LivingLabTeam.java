...
public class LivingLabTeam implements Tactic {
    ...
    class TacticaDetalleImpl implements TacticDetail {
        ...
        public PlayerDetail[] getPlayers() {
            return new PlayerDetail[]{
                new JugadorImpl("jplazaro", 1, new Color(255,200,150), new Color(50,0,0),1.0d,1.0d,1.0d, true),
                new JugadorImpl("mllorente", 2, new Color(255,200,150), new Color(50,0,0),1.0d,0.66d,0.5d, false),
                new JugadorImpl("anmarca", 3, new Color(255,200,150), new Color(50,0,0),1.0d,1.0d,0.34d, false),
                new JugadorImpl("alfiva", 4, new Color(255,200,150), new Color(50,0,0),0.72d,0.73d,0.38d, false),
                new JugadorImpl("aaracil", 5, new Color(255,200,150), new Color(50,0,0),0.72d,0.72d,0.36d, false),
                new JugadorImpl("vribes", 6, new Color(255,200,150), new Color(50,0,0),0.5d,0.34d,0.79d, false),
                new JugadorImpl("serdula", 7, new Color(255,200,150), new Color(50,0,0),1.0d,1.0d,0.94d, false),
                new JugadorImpl("johercu", 8, new Color(255,200,150), new Color(50,0,0),0.98d,1.0d,0.82d, false),
                new JugadorImpl("jobabluc", 9, new Color(255,200,150), new Color(50,0,0),1.0d,1.0d,1.0d, false),
                new JugadorImpl("fraperod", 10, new Color(255,200,150), new Color(50,0,0),1.0d,1.0d,1.0d, false),
                new JugadorImpl("ccacho", 11, new Color(255,200,150), new Color(50,0,0),1.0d,1.0d,1.0d, false)
            };
        }
        ...
        if ((!sp.isRivalStarts())&&(!sp.isStarts())) {
            if(balonRecuperable) {            	            	
                ...
                if(posibilidadRecuperacion.length == 2) {
                	comandos.add(new CommandMoveTo(posibilidadRecuperacion[1],new Position(coordenadasBalonRecuperable[0],coordenadasBalonRecuperable[1])));
                } else {
                	comandos.add(new CommandMoveTo(posibilidadRecuperacion[1],new Position(coordenadasBalonRecuperable[0],coordenadasBalonRecuperable[1])));
                	comandos.add(new CommandMoveTo(posibilidadRecuperacion[2],new Position(coordenadasBalonRecuperable[0],coordenadasBalonRecuperable[1])));
                }         	                       
            }            	
        } else if(sp.isStarts()) {
        	...
        	if(balonRecuperable) {        			
        		...
        		// probabilidad                   
        		comandos.add(new CommandMoveTo(posibilidadRecuperacion[1],new Position(coordenadasBalonRecuperable[0],coordenadasBalonRecuperable[1])));
        		jugadorDestino = buscarJugadorDesmarcado(sp,true,posibilidadRecuperacion[1],jugadores,jugadoresRivales);
        		comandos.add(new CommandHitBall(posibilidadRecuperacion[1],setPaseEntreLineas(jugadorDestino,jugadores[posibilidadRecuperacion[1]],jugadores[jugadorDestino],jugadoresRivales),setFuerzaRemate(jugadores[posibilidadRecuperacion[1]],jugadores[jugadorDestino]),false));
        	}        	
        }    
    ...
    /** Calcula el numero de jugadores rivalPlayers que estan en nuestro campo para asignarles una marca */
    ...
    /** Distribuye a nuestros jugadores en funcion del marcador, del tiempo que quede para terminar el partido y 
     * la distribucion de los jugadores rivalPlayers */
    			...
    			case 7:
    				for(int j=jugadoresRivalesPeligrosos.length-1; j>0; j--) {
    					if(jugadoresRivalesPeligrosos[j] == 1) {        				
    						comandos.add(new CommandMoveTo(i,new Position(rivales[j].getX() + 0.7,rivales[j].getY() + 0.7)));
    						jugadoresRivalesPeligrosos[j] = 0;
    						break;
    					} else {
    						if((sp.myGoals() >= sp.rivalGoals())&&((sp.iteration()*100)/(Constants.ITERACIONES)<= 75)) {
    							// Estamos empatando o ganando       				
    							if (i == 10) {
    								comandos.add(new CommandMoveTo(i,new Position(Constants.penalSup.getX() + 2.0,Constants.penalSup.getY())));
    							} else {
    								comandos.add(new CommandMoveTo(i,alineacion1[i]));
    							}
    						} else if(sp.myGoals() < sp.rivalGoals()) {
    						...
    						} else if((sp.myGoals() > sp.rivalGoals())&&((sp.iteration()*100)/(Constants.ITERACIONES)> 75)) {
    							// Estamos ganando
    							if (i == 10) {
    								chutarGol = true;    								
    								comandos.add(new CommandMoveTo(i,new Position(-0.23776223776223776,32.782805429864254)));
    							} else {
    ...
    /** Devuelve un entero con el dorsal del jugador que se encuentra en las mejores condiciones para recibir el ballPosition */
    public int buscarJugadorDesmarcado(GameSituations sp,boolean salirJugando,int limiteInferior,Position[] propios,Position[] rivales) {
    	int retorno = -1; 
    	int retorno_temp = -1;
    	...
    	if(!salirJugando) {
			...
			for(int i=1; i<indicesJugadoresPropiosProximos.length; i++) {
					if(!jugadorMarcado(indicesJugadoresPropiosProximos[i],propios,rivales)) {						
						if(propios[indicesJugadoresPropiosProximos[i]].distance(Constants.centroArcoSup)<(propios[indicesJugadoresPropiosProximos[0]].distance(Constants.centroArcoSup))) {
    	...
    	if(retorno == -1) {
    		...
    		if(retorno_temp == -1) {
    			...
    			if(propios[retorno].distance(Constants.centroArcoSup)<(propios[retorno_temp].distance(Constants.centroArcoSup))) {
    				return retorno;
    			} else {
    				return retorno_temp;
    			}
    ...
    public boolean porteroRivalFueraAreaGrande(Position porteroRival) {
    	boolean retorno = false;
    	if((porteroRival.distance(Constants.centroArcoSup)>Constants.ANCHO_AREA_GRANDE)||(porteroRival.getX()>Constants.LARGO_AREA_GRANDE)) {
    		retorno = true;    		
    	}
    	return retorno;
    }
    ...
    public boolean porteroRivalFueraAreaPequena(Position porteroRival) {
    	boolean retorno = false;
    	if((porteroRival.distance(Constants.centroArcoSup)>Constants.ANCHO_AREA_CHICA)||(porteroRival.getX()>Constants.LARGO_AREA_CHICA)) {
    		retorno = true;    		
    	}
    	return retorno;
    }    
    ...
    public Tiro setDireccionDisparo(int golpeador, GameSituations sp) {
    	...
    	auxpos0.setPosition(Constants.centroArcoSup.getX()+(Math.pow(-1, r.nextInt(3))*1.2d), Constants.centroArcoSup.getY());
    ...
    public Position setPaseEntreLineas(int dorsalJugadorDestino,Position jugadorOrigen,Position jugadorDestino, Position[] jugadoresRivales) {
    	Position retorno = null;
    	// Compruebo si hay rivalPlayers en la linea de pase
    	for(int i=0;i<jugadoresRivales.length;i++) {
    		if((jugadoresRivales[i].getX() >=  jugadorDestino.getX() - margenSeguridadPase)&&(jugadoresRivales[i].getX() <=  jugadorDestino.getX() + margenSeguridadPase)) {    	
    			// Defino el pase al hueco
    			if((jugadoresRivales[i].distance(new Position(jugadorDestino.getX() - margenSeguridadPase,jugadorDestino.getY())))<jugadoresRivales[i].distance(new Position(jugadorDestino.getX() + margenSeguridadPase,jugadorDestino.getY()))) {
    				// Debo hacer el desmarque hacia la derecha (menor probabilidad de perder el ballPosition)
        			retorno = new Position(jugadorDestino.getX()+margenSeguridadPase,jugadorDestino.getY());
        			comandos.add(new CommandMoveTo(dorsalJugadorDestino,new Position(jugadorDestino.getX()+margenSeguridadPase,jugadorDestino.getY())));
        		} else {
    ...
    public void setRematadores(GameSituations sp,Position[] jugadores,Position[] jugadoresRivales) {
    	for(int i:sp.canKick()) {
        	...
        	int jugadorDestino = -1;        	
        	if(i == 0) {   
        		if(((sp.iteration()*100)/(Constants.ITERACIONES)<= 50)&&(sp.myGoals()>=sp.rivalGoals())) {
        			jugadorDestino = buscarJugadorDesmarcado(sp,true,i,jugadores,jugadoresRivales);
        			comandos.add(new CommandHitBall(i,jugadores[jugadorDestino],setFuerzaRemate(jugadores[i],jugadores[jugadorDestino]),45));
        		} else {
        			jugadorDestino = buscarJugadorDesmarcado(sp,false,i,jugadores,jugadoresRivales);
        			comandos.add(new CommandHitBall(i,setPaseEntreLineas(jugadorDestino,jugadores[i],jugadores[jugadorDestino],jugadoresRivales),setFuerzaRemate(jugadores[i],jugadores[jugadorDestino]),45));
        		}        		
        		...
        		if(sp.iteration() - iteracionBalonJugadoDelanteros >= limiteDelanterosSinJugar) {
        			...
        			if(r.nextBoolean()) {
        				jugadorDestino = buscarJugadorDesmarcado(sp,true,i,jugadores,jugadoresRivales);
        				comandos.add(new CommandHitBall(i,setPaseEntreLineas(jugadorDestino,jugadores[i],jugadores[jugadorDestino],jugadoresRivales),setFuerzaRemate(jugadores[i],jugadores[jugadorDestino]),false));
        			} else {
        				jugadorDestino = buscarJugadorDesmarcado(sp,false,i,jugadores,jugadoresRivales);
        				comandos.add(new CommandHitBall(i,setPaseEntreLineas(jugadorDestino,jugadores[i],jugadores[jugadorDestino],jugadoresRivales),setFuerzaRemate(jugadores[i],jugadores[jugadorDestino]),false));
        			}
        		...
        		if((jugadores[i].distance(Constants.centroArcoSup)<= distanciaTiro)) {
        			if((porteroRivalFueraAreaGrande(jugadoresRivales[0]))||(porteroRivalFueraAreaPequena(jugadoresRivales[0]))) {
    ...
    public void buscarDesmarque(GameSituations sp,Position[] jugadores) {
    	for(int i=8;i<jugadores.length;i++) {
    		if((i==8)||(i==10)) {
    			if((jugadores[i].distance(Constants.centroArcoSup)<= distanciaTiro)) {
    				comandos.add(new CommandMoveTo(i,new Position(jugadores[i].getX(),jugadores[i].getY()+r.nextInt(2))));
    			} else {
    				comandos.add(new CommandMoveTo(i,new Position(jugadores[i].getX(),jugadores[i].getY()-r.nextInt(2))));
    			}    			
    		}
