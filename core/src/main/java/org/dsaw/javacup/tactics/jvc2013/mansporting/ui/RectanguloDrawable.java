...
/**
 * <br> Drawable para un rectangulo. <br> <br>Copyright Fi2net S.A. 10-ene-2011
 */
public class RectanguloDrawable extends AbstractDrawable<Rectangulo> {
  ...
  public void paint(VisualDebugGraphics g, Rectangulo canvas) {
    ...
    g.drawRect(
        element.getCentro().getX() + (canvas.getCentro().getX() + canvas.getAncho() / 2)
        - element.getAncho() / 2,
        -element.getCentro().getY() + (canvas.getCentro().getY() + canvas.getAlto() / 2)
        - element.getAlto() / 2,
        element.getAncho(),
        element.getAlto());
