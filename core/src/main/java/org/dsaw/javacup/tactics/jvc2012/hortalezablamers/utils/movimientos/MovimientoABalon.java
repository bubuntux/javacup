...
public class MovimientoABalon {
  ...
  public List<Command> correrHaciaBalon(GameSituations gs) {
    ...
    if (recuperadores.length > 0 && (recuperadoresRival.length == 0
                                     || recuperadores[0] <= recuperadoresRival[0])) {
...
//			desplazarRival(gs, recuperadores, recuperadoresRival, posicionFuturaBalon, posicionDestino, comandos);
    ...
    //si el rival la va a controlar antes que yo, mejor intentar cubrirle
    else if (recuperadoresRival.length > 0 && (recuperadores.length == 0
                                               || recuperadores[0] > recuperadoresRival[0])) {
      ...
      // si no buscamos el jugador mas cercano a la posicion destino
        ...
        for (int i = 5; i < 11; i++) {
          if (indiceJugador == null || posicionDestino.distance(gs.myPlayers()[i]) < posicionDestino
              .distance(gs.myPlayers()[indiceJugador])) {
            indiceJugador = i;
          }
        }
  ...
  private void desplazarRival(GameSituations gs, int[] recuperadores, int[] recuperadoresRival,
                              Position posicionFuturaBalon, Position posicionDestino,
    ...
    try {
      if (intentandoDesplazarRival ||
          (gs.myPlayers()[recuperadores[1]].equals(posicionDestino) &&
           recuperadoresRival.length == 2
           && gs.rivalPlayers()[recuperadoresRival[1]].distance(posicionFuturaBalon)
              <= Constants.DISTANCIA_CONTROL_BALON * 1.1)) {
        Position
            nuevaPosicionDestino =
            Position.medium(posicionFuturaBalon, gs.rivalPlayers()[recuperadoresRival[1]]);
    ...
    while (!found) {
      ...
      if (posBalon[2] <= Constants.ALTO_ARCO) {
        for (int i = 0; i < gs.myPlayersDetail().length; i++) {
          if (posBalon[2] <= (
              gs.myPlayersDetail()[i].isGoalKeeper() && Common.dentroAreaPropia(posicionBalon)
    ...
    while (!found) {
      ...
      if (posBalon[2] <= Constants.ALTO_ARCO) {
        for (int i = 0; i < gs.rivalPlayersDetail().length; i++) {
          if (posBalon[2] <= (
              gs.rivalPlayersDetail()[i].isGoalKeeper() && Common.dentroAreaRival(posicionBalon)
              ? Constants.ALTO_ARCO : Constants.ALTURA_CONTROL_BALON)) {
            Position pJug = gs.rivalPlayers()[i];
            dist0 = (double) it * Constants.getVelocidad(gs.rivalPlayersDetail()[i].getSpeed());
            dist =
                pJug.distance(posicionBalon) - (gs.rivalPlayersDetail()[i].isGoalKeeper() && Common
                    .dentroAreaRival(posicionBalon) ? Constants.DISTANCIA_CONTROL_BALON_PORTERO
                                                    : Constants.DISTANCIA_CONTROL_BALON);
