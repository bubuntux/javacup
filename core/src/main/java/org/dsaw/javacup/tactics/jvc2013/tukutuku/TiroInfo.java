...
public class TiroInfo {
  ...
  private boolean balonEnPorteria(PosicionBalon ball, PosicionBalon lastBall) {
    ...
    if ((posBalon.y > Constants.posteIzqArcoSup.getY()) && (posBalonAnt.y
                                                            <= Constants.posteIzqArcoSup.getY())) {
      Position posMedia = Position
          .medium(posBalon.getPosicion(), posBalonAnt.getPosicion());

      if ((Math.abs(posMedia.getX()) < (Constants.LARGO_ARCO / 2) - Constants.RADIO_BALON)
          && posBalon.z < Constants.ALTO_ARCO) {
        return true;
      }
    }
