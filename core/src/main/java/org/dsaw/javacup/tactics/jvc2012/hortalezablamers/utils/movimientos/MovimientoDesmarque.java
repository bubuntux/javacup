...
public class MovimientoDesmarque {
  ...
  public static List<Command> mover(GameSituations gs) {
    ...
    if (iei.getIteracion() <= ITERACIONES_DE_DESMARQUE && iteracionRecuperacionRival >= iei
        .getIteracion()) {
      ...
      if (posBalon[2] <= Constants.ALTO_ARCO) {
        for (int i = 0; i < gs.myPlayers().length; i++) {
          if (posBalon[2] <= (
            ...
            double
                dist0 =
                (double) it * Constants.getVelocidad(gs.myPlayersDetail()[i]
                                                         .getSpeed()); //uso una iteracion menos, porque el jugador se puede mover en la misma iteracion en que se pasa
            double
                dist =
      ...
      if (posBalon[2] <= Constants.ALTO_ARCO) {
        for (int i = 0; i < gs.rivalPlayers().length; i++) {
          ...
          if (posBalon[2] <= (
            ...
            double
                dist0 =
                (double) it * Constants.getVelocidad(gs.rivalPlayersDetail()[i].getSpeed());
            double
                dist =
