...
public class Pass {
  ...
  public Pass(List<Command> comandos, Data data) {
    this.comandos = comandos;
    this.data = data;
    shoot = new Shoot(comandos, data);
  }
  ...
  private double getThreshold(Mentality mentality) {
    if (mentality == Mentality.Normal) {
      return .975;
    }
  ...
  private PassInfo calculatePass(double velocity, double angleVer, boolean forward) {
    ...
    for (int i = 0; i < n; i++) {
      trajectory[i] =
          new Trajectory(sp.ballPosition(), velocity, i * dAngle * Math.PI / 180, angleVer);
    }
