...
public class Zona {
  ...
  private Position calcularCentro() {
    return Position.medium(Position.medium(abajoDerecha, abajoIzquierda),
                           Position.medium(arribaDerecha, arribaIzquierda));
  }

  public boolean estaEnZona(Position posicion) {

    if (posicion.getX() <= getAbajoDerecha().getX() && posicion.getX() >= getAbajoIzquierda().getX()
        && posicion.getY() >= getAbajoDerecha().getY() && posicion.getY() <= getArribaDerecha()
        .getY()) {
      return true;
    } else {
      return false;
