...
public class MSGSituacionPartidoContext {
  ...
  public void update(MSGTactica tactica, Map<Integer, MSGAlineacionPosicion> alineacion,
    ...
    if (jugadoresRivales == null) {
      jugadoresRivales = new HashMap<>();
      jugadoresRivalesCercanosBalon = new ArrayList<>();
      jugadoresRivalesCercanosBalonFuturo = new ArrayList<>();
      for (int i = 0; i < situacionPartido.rivalPlayers().length; i++) {
        MSGJugadorDetalle
            jugador =
            new MSGJugadorRival(i, situacionPartido.rivalPlayersDetail()[i]);
    ...
    if (jugadoresPropios == null) {
      jugadoresPropios = new HashMap<>();
      jugadoresPropiosCercanosBalon = new ArrayList<>();
      jugadoresPropiosCercanosBalonFuturo = new ArrayList<>();
      jugadoresPropiosMarcajes = new HashMap<>();
      jugadoresPropiosDesmarques = new HashMap<>();
  ...
  private void updateJugadoresCercanosBalon() {
    ...
    try {
      List<MSGJugadorDetalle> jugadores = new LinkedList<>();
      jugadores.addAll(jugadoresRivales.values());
      jugadores.add(propioData.getJugador());
      JugadoresBalonThread
          thread =
          new JugadoresBalonThread(null, jugadores, MSGTrayectoria.getInstance(situacionPartido));
    ...
    if (this.jugadoresCercanosBalonData.getContrario() == null
        && this.jugadoresCercanosBalonData.getPropio() == null) {
      this.jugadoresCercanosBalonData = null;
    }

  }

  private JugadorBalonData handle(int[] puedenRematar,
                                  Map<Integer, ? extends MSGJugadorDetalle> jugadores) {
    if (puedenRematar.length > 1) {
      ...
      return new JugadorBalonData(jugador, false, iteraciones - situacionPartido.iteration(),
                                  punto);
    }
    return null;
  }
  ...
  private void updateListasOrdenadas() {
    ...
    Comparator<MSGJugadorDetalle> balonFuturoComparator;
    if (this.jugadoresCercanosBalonData == null) {
  ...
  private void updateMarcajes() {
    ...
    List<MSGJugadorDetalle>
        rivales =
        new LinkedList<>(this.jugadoresRivales.values());
    List<MSGJugadorPropio>
        propios =
        new LinkedList<>(this.jugadoresPropios.values());
  ...
  private void updateDesmarques() {
    ...
    if (this.jugadoresCercanosBalonData != null && this.jugadoresCercanosBalonData.isAlcanzable()
        || situacionPartido.isStarts()) {
      Punto3D posicionBalon = situacionPartido.isStarts()
                              ? new Punto3D(situacionPartido.ballPosition().getX(),
                                            situacionPartido.ballPosition().getY(),
                                            situacionPartido.ballAltitude())
                              : jugadoresCercanosBalonData.getPropio().getPunto();
      MSGJugadorPropio
          jugadorPasador =
          situacionPartido.isStarts() ? jugadoresPropiosCercanosBalon.get(0)
                                      : (MSGJugadorPropio) jugadoresCercanosBalonData.getPropio()
                                          .getJugador();
      List<MSGJugadorPropio>
          jugadoresADesmarcar =
          MSGUtils.getJugadoresADesmarcar(jugadorPasador, getJugadoresPropios());
      Map<MSGJugadorPropio, List<Punto3D>>
          puntosComprobacion =
          MSGUtils.getPuntosDesmarque(jugadoresADesmarcar);
      Map<MSGJugadorPropio, List<PaseData>>
          pases =
          getPases(posicionBalon, jugadorPasador, puntosComprobacion);
   ...
   * @return
   */
  public Map<MSGJugadorPropio, List<PaseData>> getPases(Punto3D posicionBalon,
                                                        MSGJugadorPropio jugadorPasador,
                                                        Map<MSGJugadorPropio, List<Punto3D>> areaJugadores) {
    List<PaseThread> desmarquesThreads = new LinkedList<>();
    for (Map.Entry<MSGJugadorPropio, List<Punto3D>> entry : areaJugadores.entrySet()) {
      ...
      for (Punto3D punto : puntosAComprobar) {
        desmarquesThreads
            .add(new PaseThread(jugadorPasador, posicionBalon, jugadorADesmarcar, punto, this));
      }
    }

    Map<MSGJugadorPropio, List<PaseData>> pases = new HashMap<>();
    for (IdentificableData<MSGJugadorPropio, PaseData> result : tactica.getThreadOperationHandler()
        .executeThreaded(desmarquesThreads)) {
      ...
      if (MSGUtils.isPaseValido(pase.getPuntoDestino().getPuntoXY())
          ...
          && pase.getJugadoresMasCercanos().getIteracionesVentaja() > jugador
          .getIteracionesVentajaPase()) {
        List<PaseData> pasesAlJugador = pases.get(jugador);
        if (pasesAlJugador == null) {
          pasesAlJugador = new LinkedList<>();
  ...
  public static class DistanciaJugadorComparator implements Comparator<PaseData> {
    ...
    public int compare(PaseData o1, PaseData o2) {
      Double
          d1 =
          TrigonometriaUtils
              .getDistancia(o1.getPuntoDestino().getPuntoXY(), jugadorPropio.getPosicion());
      Double
          d2 =
          TrigonometriaUtils
              .getDistancia(o1.getPuntoDestino().getPuntoXY(), jugadorPropio.getPosicion());
