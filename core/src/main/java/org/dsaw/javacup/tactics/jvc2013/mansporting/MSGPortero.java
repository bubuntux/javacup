...
public class MSGPortero extends MSGAbstractJugadorPropio {
  ...
  public MSGPortero(int index, Color colorPelo, Color colorPiel, String nombre, int numero,
  ...
  protected Boolean isInArea(Punto punto) {
    ...
    if (punto.getY() > (-Constants.ANCHO_CAMPO_JUEGO / 2 + Constants.ANCHO_AREA_GRANDE)
  ...
  protected Command cubrePorteria() {
    ...
    Punto puntoIrA = null;
    for (Recta bisectriz : TrigonometriaUtils
        .getBisectrices(posteIzq.getRecta(), posteDer.getRecta())) {
    ...
    return puntoIrA == null ? null : new CommandMoveTo(index, new Position(puntoIrA.getX(),
                                                                           puntoIrA.getY()));
  }
