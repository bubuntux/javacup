...
//				);
    ...
    CommandMoveTo
        cmt =
        new CommandMoveTo(chbsToReturn.getIndiceReceptor(),
                          chbsToReturn.getPosicionRecepcionMedia());
  ...
  private static CommandHitBallSimulated obtenerMejorPase(GameSituations gs, int indicePasador) {
    ...
    //PASES HACIA DELANTE
    Vector<CommandHitBallSimulated>
        comandosSimulados =
        simularComandosPase(sp, gs, indicePasador, angulosRelativos1);
  ...
  private static boolean paseDemasiadoAtras(GameSituations gs,
    ...
    return gs.ballPosition().distance(chbs.getPosicionRecepcionMedia())
           > Constants.LARGO_CAMPO_JUEGO / 3. ||
   ...
   */
  private static Vector<CommandHitBallSimulated> simularComandosPase(SimuladorPases sp,
                                                                     GameSituations gs,
                                                                     int indicePasador,
                                                                     Vector<Double> angulosRelativos) {
    ...
    for (double anguloRelativo : angulosRelativos) // indice de angulos
    {
      for (double potencia : potencias) // indice de potencias
      {
        for (double anguloVertical : angulosVerticales) { // indice de angulos verticales
          CommandHitBall
              chb =
              new CommandHitBall(indicePasador, anguloAPorteria + anguloRelativo, potencia,
                                 anguloVertical);
  ...
  /**
   * Obtiene el mejor pase ordenandolos segun el criterio de ordenacion de la clase
   * CommandHitBallSimulated. usa los pases mas seguros en cuanto a diferencia de iteraciones para
   * ser recuperados
   */
  private static CommandHitBallSimulated decidirMejorPase(GameSituations gs,
                                                          Vector<CommandHitBallSimulated> comandosSimulados) {
    ...
    return comandosSimuladosDeIgualDiferencia
        .get(random.nextInt(comandosSimuladosDeIgualDiferencia.size()));
  }
}
