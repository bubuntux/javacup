...
public class PaseInfo {
  ...
  private void calcularCalidad() {
    ...
    for (int iter = 1; iter < N_POSICIONES; ++iter) {
      if (posicionesBalon[iter].distancia(
          entrenador.Gs.rivalPlayers()[posicionesBalon[iter].getPosicion()
              .nearestIndex(entrenador.Gs.rivalPlayers())]) < 2) {
        calidad.libreContrarios = false;
      }
  ...
  protected void simularPase() {
    ...
    double
        vel =
        this.fuerza * Constants.getVelocidadRemate(entrenador.Gs.getMyPlayerPower(tirador));
    ...
    //Error Angular
    double errorAngular = Constants
        .getErrorAngular(entrenador.Gs.getMyPlayerError(tirador));
  ...
  public boolean puedeInterceptarBalon(PosicionBalon posBalon, boolean esRival, int iter) {
    ...
    int
        jugador =
        (esRival) ? posBalon.getPosicion().nearestIndex(Gs.rivalPlayers())
                  : posBalon.getPosicion().nearestIndex(Gs.myPlayers());

    int
        iteraccionesDisp =
        (esRival) ? Gs.rivalIterationsToKick()[jugador] : this.iterDisparo[jugador];

    //Si el jugador no puede rematar en esta iteraccion no puede recuperar el balon

    if (iteraccionesDisp > iter) {
      return false;
    }
    ...
    Position
        posJugador = (esRival) ? Gs.rivalPlayers()[jugador] : Gs.myPlayers()[jugador];
    ...
    if (detJugador.isGoalKeeper() && Math.abs(posJugador.getX()) <= Constants.LARGO_AREA_GRANDE / 2
        && Math.abs(posJugador.getY()) > Constants.ANCHO_AREA_GRANDE) {
