package org.dsaw.javacup.tactics.jvc2012.lgarza;
/*
 * pases interceptados
 * */
...
public class Tactica implements Tactic {
  ...
  class TacticDetailImpl implements TacticDetail {
    ...
    public PlayerDetail[] getPlayers() {
      return new PlayerDetail[]{
          new JugadorImpl("Oswaldo", 1, new Color(255, 200, 150), new Color(0, 0, 0), 1.0d, 0.7d,
                          1.0d, true),
          new JugadorImpl("Temerario", 2, new Color(176, 128, 84), new Color(0, 0, 0), 0.76d, 0.76d,
                          0.62d, false),
          new JugadorImpl("Noe", 3, new Color(227, 131, 44), new Color(0, 0, 0), 0.25d, 0.6d, 0.77d,
                          false),
          new JugadorImpl("Nestor", 4, new Color(249, 219, 191), new Color(0, 0, 0), 0.75d, 0.21d,
                          0.58d, false),
          new JugadorImpl("Valles", 5, new Color(255, 200, 150), new Color(0, 0, 0), 0.38d, 0.5d,
                          1.0d, false),
          new JugadorImpl("Luna", 20, new Color(255, 200, 150), new Color(0, 0, 0), 1.0d, 1.0d,
                          1.0d, false),
          new JugadorImpl("Piñas", 19, new Color(246, 171, 103), new Color(0, 0, 0), 1.0d, 0.75d,
                          1.0d, false),
          new JugadorImpl("Markin", 7, new Color(255, 200, 150), new Color(0, 0, 0), 1.0d, 0.86d,
                          1.0d, false),
          new JugadorImpl("Javier Hernández", 14, new Color(255, 200, 150), new Color(0, 0, 0),
                          1.0d, 1.0d, 1.0d, false),
          new JugadorImpl("Rocha", 10, new Color(226, 148, 77), new Color(0, 0, 0), 1.0d, 1.0d,
                          1.0d, false),
          new JugadorImpl("Gober", 11, new Color(255, 200, 150), new Color(0, 0, 0), 1.0d, 1.0d,
                          1.0d, false)
      };
    }
  }
  ...
  private void recuperarBalon(GameSituations sp) {
    ...
    if (recuperadores.length > 0) {
      ...
      for (int i = 1; i < recuperadores.length & i < 2; i++) {
        comandos.add(new CommandMoveTo(recuperadores[i], new Position(posicionRecuperacion[0],
  ...
  private void tirar(GameSituations sp) {
    Position
        posicionTiroAlPosteDerecho =
        new Position(Constants.posteDerArcoSup.getX() - 2, Constants.posteDerArcoSup.getY());
    Position
        posicionTiroAlPosteIzquierdo =
        new Position(Constants.posteIzqArcoSup.getX() + 2, Constants.posteIzqArcoSup.getY());
    ;
  ...
  private void paseAlPie(GameSituations sp, int pasador, int receptor) {
    ...
    if (distancia
        > distanciaParaPaseElevado)//si hay una distancia mayor a 30 ejecutar un pase largo
    {
  ...
  private boolean hayInterferencia(GameSituations sp, int pasador, int receptor) {
    ...
    for (int i = 0; i < rivales.length; i++) {
      if (rivales[i].getX() + cercania > Math.min(posicionPasador.getX(), posicionReceptor.getX())
          && rivales[i].getX() + cercania < Math
          .max(posicionPasador.getX(), posicionReceptor.getX())) {
        if (rivales[i].getY() + cercania > Math.min(posicionPasador.getY(), posicionReceptor.getY())
            && rivales[i].getY() + cercania < Math
            .max(posicionPasador.getY(), posicionReceptor.getY())) {
          return true;
        }
      }
    }
...
//                
        ...
        if (tiradores[i] == 10 && !estaMarcado(sp, 8)
            && sp.myPlayers()[8].distance(Constants.centroArcoSup) > sp.myPlayers()[i]
            .distance(Constants.centroArcoSup)) {
          paseAlPie(sp, tiradores[i], 8);
        } else if (tiradores[i] == 8 && !estaMarcado(sp, 10)
                   && sp.myPlayers()[i].distance(Constants.centroArcoSup) > sp.myPlayers()[8]
            .distance(Constants.centroArcoSup)) {
        ...
        else if ((tiradores[i] == 10 || tiradores[i] == 8)
                 && distanciaAlArco(sp) <= distanciaRemateAlArco
                 || distanciaAlArco(sp) <= distanciaRemateAlArco - 10) {
          //if(r.nextInt(2)==1)
          tirar(sp);
          //else
          //conducir(sp,tiradores[i]);
        } else if (tiradores[i] == 0)
