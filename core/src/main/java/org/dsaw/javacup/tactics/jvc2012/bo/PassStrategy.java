...
public class PassStrategy implements SoccerStrategy {
  ...
  private void pass(Position[] players, int index, PassInfo passInfo, List<Command> commands) {
    if (passInfo.exist) {
      double
          power =
          passInfo.velocity / Constants
              .getVelocidadRemate(currentSituation.myPlayersDetail()[index].getPower());
  ...
  private PassInfo calculatePass(double velocity, double angleVer, boolean forward) {
    ...
    for (int i = 0; i < n; i++) {
      trajectory[i] =
          new TrajectoryMasia(currentSituation.ballPosition(), velocity, i * dAngle * Math.PI / 180,
                              angleVer);
    }
        ...
        if (j == 0) {
          fitness =
              calculateFitness(trajectory[i].positions[j], trajectory[i].z[j], j + 1,
                               currentSituation.ballPosition(), 0);
        } else {
          fitness =
              calculateFitness(trajectory[i].positions[j], trajectory[i].z[j], j + 1,
  ...
  private double calculateFitness(Position position, double z, int iter, Position last,
    ...
    int
        iterToBall =
        ChimueloTacticUtils
            .calculateIterToBall(position, z, players, currentSituation.myPlayersDetail(),
                                 iterToShoot);
