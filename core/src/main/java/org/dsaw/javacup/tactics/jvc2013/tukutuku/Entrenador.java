...
/**
 * Clase encargada de realizar calculos generales de apoyo a los jugadores y de pensar en el partido
 * de forma general.
 *
...
public class Entrenador {
  ...
  public Position[] obtenerAlineacion() {
    ...
    if (variacionAtaque <= 3) {
      ...
      if (Gs.ballPosition().getY() > 0) {
        alineacion = tactica.alineacion3;
      }
      if (Gs.ballPosition().getY() > (Constants.LARGO_CAMPO_JUEGO / 6)) {
        alineacion = tactica.alineaciones.get(3 + variacionAtaque);
      }
      if (Gs.ballPosition().getY() > (Constants.LARGO_CAMPO_JUEGO / 3)) {
        alineacion = tactica.alineaciones.get(4 + (2 * variacionAtaque));
      }
    } else {
      if (Gs.ballPosition().getY() > -Constants.LARGO_CAMPO_JUEGO / 4) {
        alineacion = tactica.alineacion2;
      ...
      if (Gs.ballPosition().getY() > 0) {
        ...
        posiciones =
            (variacionAtaque == 4) ? calcularPosiiones(tactica.alineacion14)
                                   : calcularPosiiones(tactica.alineacion15);
    ...
    if (Gs.isRivalStarts() && Gs.ballPosition().getY() == 0) {
      alineacion = tactica.alineacion13;
    }
    if (Gs.isStarts() && Gs.ballPosition().getY() == 0) {
      alineacion = tactica.alineacion12;
    }
  ...
  private Position[] calcularPosiiones(Position[] positions) {
    double
        ...
        Math.max(Constants.LARGO_CAMPO_JUEGO / 3, 2 * Math
            .min(Math.abs(Constants.centroArcoInf.getY() - Gs.ballPosition().getY()),
                 Math.abs(Constants.centroArcoSup.getY() - Gs.ballPosition().getY())));
    double
        width =
        Math.max(Constants.ANCHO_CAMPO_JUEGO * 2 / 3, 2 * Math
            .min(Math.abs(Constants.cornerInfIzq.getX() - Gs.ballPosition().getX()),
                 Math.abs(Constants.cornerInfDer.getX() - Gs.ballPosition().getX())));
    ...
    for (int i = 0; i < newPositions.length; i++) {
      newPositions[i] =
          new Position(positions[i].getX() * width / Constants.ANCHO_CAMPO_JUEGO + center.getX(),
  ...
  public ArrayList<Command> procesarIteraccion(GameSituations Gs) {
    ...
    double
        distMisJugadores =
        Gs.ballPosition()
            .distance(Gs.myPlayers()[(Gs.ballPosition().nearestIndex(Gs.myPlayers()))]);
    double
        distRivales =
        Gs.ballPosition()
            .distance(Gs.rivalPlayers()[(Gs.ballPosition().nearestIndex(Gs.rivalPlayers()))]);
    ...
    if (!atacando && Gs.ballPosition().getY() < 20) {
      asignaMarcajes();
    }
    ...
    if (!Gs.isRivalStarts()) {
      ...
      if (recuperadores.length > 1) {
        ...
        Position
            posRecuperacion =
            new Position().setPosition(doubRecuperacion[0], doubRecuperacion[1]);
        ...
        for (int i = 1; i < (recuperadores.length < 3 ? recuperadores.length : 3); i++) {
          ...
          if ((recuperadores[i] == 0) && posRecuperacion.getY() <= Constants.posteDerArcoInf
              .getY()) {
            posRecuperacion =
                new Position(posRecuperacion.getX(), Constants.posteDerArcoInf.getY());
          }
  ...
  private void asignaMarcajes() {
    ...
    for (int rival : arrayRivalesCercanos) {
      ...
      for (int x = 0; x < jugMiosCercanos.length; ++x) {
        //El portero no marca
        if (jugMiosCercanos[x] == 0) {
          continue;
        }

        //El libero tampoco
        if (jugMiosCercanos[x] == 8) {
          continue;
        }
