...
public class Team2312 implements Tactic {
  ...
  public Team2312(final double distanciaMarca) {
  ...
  Position alineacion1[] = new Position[]{
      ...
      new Position(-31.384615384615387, -30.269230769230766),
  ...
  class TacticDetailImpl implements TacticDetail {
    ...
    class JugadorImpl implements PlayerDetail {
      ...
      public JugadorImpl(final String nombre, final int numero,
                         final Color piel, final Color pelo, final double velocidad,
                         final double remate, final double presicion,
    ...
    public PlayerDetail[] getPlayers() {
      return new PlayerDetail[]{
          new JugadorImpl("0", 1, new Color(255, 200, 150),
                          new Color(255, 255, 255), 1.0d, 0.72d, 0.5d, true),
          new JugadorImpl("1", 2, new Color(255, 200, 150),
                          new Color(255, 255, 0), 1.0d, 0.67d, 0.5d, false),
          new JugadorImpl("2", 3, new Color(255, 200, 150),
                          new Color(0, 0, 153), 1.0d, 0.67d, 0.5d, false),
          new JugadorImpl("3", 4, new Color(255, 200, 150),
                          new Color(255, 0, 0), 1.0d, 0.67d, 0.5d, false),
          new JugadorImpl("4", 5, new Color(255, 200, 150),
                          new Color(255, 255, 51), 1.0d, 0.67d, 0.5d, false),
          new JugadorImpl("5", 6, new Color(255, 200, 150),
                          new Color(0, 0, 204), 1.0d, 1.0d, 0.5d, false),
          new JugadorImpl("6", 7, new Color(255, 200, 150),
                          new Color(255, 0, 0), 1.0d, 1.0d, 0.5d, false),
          new JugadorImpl("7", 8, new Color(255, 200, 150),
                          new Color(255, 255, 0), 1.0d, 0.8d, 1.0d, false),
          new JugadorImpl("8", 9, new Color(255, 200, 150),
                          new Color(0, 0, 204), 1.0d, 0.8d, 1.0d, false),
          new JugadorImpl("23", 23, new Color(255, 204, 153),
                          new Color(255, 255, 255), 1.0d, 1.0d, 1.0d, false),
          new JugadorImpl("9", 11, new Color(255, 200, 150),
                          new Color(255, 0, 0), 1.0d, 1.0d, 1.0d, false)};
    }
  }
  ...
  public List<Command> execute(final GameSituations sp) {
    ...
    if (sp.iteration() > 0) {
      ...
      final int rivalMasCercano = sp.ballPosition().nearestIndex(
          sp.rivalPlayers());
      final int rivalMasPeligro = Constants.centroArcoInf.nearestIndex(
          sp.rivalPlayers());
      ...
      for (final Integer rival : rivales) {
        ...
        if (marcador != -1) {
          final CommandMoveTo marcar = Helper.marcar(marcador, rival,
                                                     rivalMasCercano, historialBalon.aMiArco(),
                                                     historialMarcas, sp, util23, rivalMasPeligro);
          com.add(marcar);
        }
      }

      for (final int i : sp.canKick()) {
        final CommandHitBall cg = mejorDisparoAlArco(i, 10,
                                                     Constants.ANGULO_VERTICAL_MAX, sp);
        final Position posKick = sp.myPlayers()[i];
        Integer rivalMarca = null;
        for (Iterator<Integer> iterator = util23.getMarcas().keySet()
        ...
        final double distanciaArco = posKick
            .distance(Constants.centroArcoSup);
        if (sp.isStarts()
            && iteracionesSaque >= Constants.ITERACIONES_SAQUE / 2) {
          if (cg != null
              && distanciaArco > Constants.ANCHO_AREA_GRANDE) {
            com.add(cg);
          } else {
            if (sp.ballPosition().distance(Constants.centroArcoSup) > 40) {
              Position posDisparo = Constants.centroArcoSup;
              com.add(new CommandHitBall(i, posDisparo, 1.0,
                                         Constants.ANGULO_VERTICAL));
            } else {
              com.add(new CommandHitBall(i, Helper
                  .obtenerPuntoMejorDisparoArco(sp), 1.0,
        ...
        if (!sp.isStarts()) {
          if (cg != null
            ...
            if (sp.ballPosition().distance(Constants.centroArcoSup) > 40) {
              ...
              if (posMiMarca != null
                  && posMiMarca.distance(miPosicion) < 1) {
//								if (posMiMarca.getX() < miPosicion.getX()) {
//									posDisparo = Constants.cornerSupDer;
//								} else {
//									posDisparo = Constants.cornerSupIzq;
//								}
      ...
      Position posArquero = Helper.getPosicionArquero(sp.ballPosition(),
                                                      pRivalMasCercano, pMiArquero,
                                                      sp.ballAltitude(),
                                                      balonPasado);
      com.add(new CommandMoveTo(0, posArquero));

      if (sp.isStarts()) {
        final int mj = sp.ballPosition()
            .nearestIndex(sp.myPlayers(), 0);
        if (mj == 0) {
      ...
      for (final Command c : com) {
        if (c instanceof CommandMoveTo) {
          ...
          if (cmd.getPlayerIndex() != 0
              && cmd.getMoveTo().equals(posArquero)) {
            listaBorrar.add(cmd);
          }
        }
      }
  ...
  private CommandHitBall mejorDisparoAlArco(final int idxPateador,
    ...
    for (final Position position : opciones) {
      ...
      for (double fuerza = 1; fuerza > 0.6; fuerza -= 0.1d) {
        ...
        for (double angZ = minAngZ; angZ <= maxAngZ; angZ += 0.5) {
          ...
          final double angRadZ = Math.max(0,
          ...
          if (iteraciones >= 0) {
            ...
            final double distRivAlPunto = sp.rivalPlayers()[idxRival]
                .distance(position);
            int iterRival = (int) Math.ceil(distRivAlPunto
                                            / Constants.getVelocidad(sp
                                                                         .rivalPlayersDetail()[idxRival]
                                                                         .getSpeed()));
            if (destino != null) {
              idxRival = destino.getIdxJugador();
            ...
            if (destino == null
                || (destino.getIterJugador() > iteraciones)
                || (angZ > 0 && destino.getIdxJugador() == idxPortRival)) {
              final double distJugadorAlPunto = position
                  .distance(sp.ballPosition());
              // si se trata de un disparo a meta
              return new CommandHitBall(idxPateador,
                                        trayectoriaBalon.getLast().getPosition(),
                                        fuerza, angZ);
            }
          }
        }
  ...
  private int obtenerTrayectoriaCalculada(
      final LinkedList<Ball> trayectoriaBalon, final Ball balon,
      final double potencia, final double angleXY, final double angleZ,
      final Position pFinal, final boolean completa,
      final int idxReceptor, final int maxPasos, final GameSituations sp) {
    trayectoriaBalon.clear();
    balon.setIteracion(0);
    trayectoriaBalon.add(balon);
    final AbstractTrajectory trajectory = new AirTrajectory(
        Math.cos(angleZ) * potencia, Math.sin(angleZ) * potencia, 0, 0);

    final double[] datosControl = new double[]{
        Constants.DISTANCIA_CONTROL_BALON_PORTERO, Constants.ALTO_ARCO};
    ...
    final boolean centro = sp.ballPosition().distance(
        Constants.centroCampoJuego) == 0;
    for (int step = 1; step <= maxPasos; step++) {

      final double time = step / 60d;
      final double dX = trajectory.getX(time)
                        * Constants.AMPLIFICA_VEL_TRAYECTORIA;
      final double dY = trajectory.getY(time)
                        * Constants.AMPLIFICA_VEL_TRAYECTORIA * 2;
      final double x = balon.getPosition().getX() + dX
                                                    * Math.cos(angleXY);
      ...
      distanciaAPunto = nuevoBalon.getPosition().setInsideGameField()
          .distance(pFinal);
      ...
      if (!elevando) {
        ...
        if (iteracionesAlcance == -1) {
          ...
          if (idxReceptor == -1 && !trayectoriaBalon.isEmpty()) {
            if (angleZ > 0) {
              final Ball balonAnterior = trayectoriaBalon
                  .getLast();
              // se verifica que el balon anterior ente en el
              // campo y el actual no
              final boolean antAdentro = balonAnterior
                                             .getAltura() > Constants.ALTO_ARCO
                                         && balonAnterior.getPosition().getY() <= (
                  Constants.LARGO_CAMPO_JUEGO / 2);
              final boolean actAfuera = nuevoBalon.getAltura() < Constants.ALTO_ARCO
                                        && nuevoBalon.getPosition().getY() > (
                  Constants.LARGO_CAMPO_JUEGO / 2);
              if (antAdentro && actAfuera) {

                final double deltaX = balonAnterior
                    .getPosition().distance(
                        nuevoBalon.getPosition());

                // se determina la altura en la linea de meta
                final double deltaY = balonAnterior.getAltura()
                                      - nuevoBalon.getAltura();
                final double h = Math.sqrt(deltaX * deltaX
                                           + deltaY * deltaY);
                // arc cos[(b� + c� - a�)/2bc]
                final double teta = Math.acos((deltaX * deltaX
                                               + h * h - deltaY * deltaY)
                                              / (2 * deltaX * h));
                // tan (teta)=opuesto/adyacente
                final double nuevoX = deltaX
                                      - ((Constants.LARGO_CAMPO_JUEGO / 2) - balonAnterior
                    .getPosition().getY());
                final double alturaMeta = nuevoBalon
                                              .getAltura()
                                          + (Math.tan(teta) * nuevoX);
                // si la altura en linea de meta es aceptable
                if (alturaMeta <= Constants.ALTO_ARCO
                    && (nuevoBalon.getAltura()
                        - (balonAnterior.getAltura() - b0
                    .getAltura()) <= Constants.ALTO_ARCO || centro)) {
          ...
          else if (iteracionesAlcance == -1
                   && distanciaAPunto <= datosControl[0]
                   && (angleZ == 0 || (dY <= datosControl[1]))) {
            iteracionesAlcance = Math.max(step,
                                          Constants.ITERACIONES_GOLPEAR_BALON);
      ...
      // rebote
      if (!completa
          && iteracionesAlcance == -1
          && (angleZ > 0 && ((elevando && !dzAntPosivivo) || (!elevando
                                                              && dY < datosControl[1])))) {
        return iteracionesAlcance;
      }
