    ...
    public void initRound(GameSituations sp) {
        ...
        for (int i = 1; i < minDestinations.length; ++i){
            if (attack[i]) {
                ...
                minDestinations[i] = new MyPosition(minX[i], Math.min(Constants.LARGO_CAMPO_JUEGO/2 - 10, (Math.max(50 - Math.abs(minX[i]) / 2, ballwithZ.y))));
    ...
    public class DecisionScore {
        ...
        public void computeSafe(MyPosition[] ball) {
            ...
            Mainloop : for (int k = 1; k < length; ++k){
                ...
                for (int i = 0; i < myPlayers.length; ++i){
                    if (defense[i] || myPlayersDetails[i].isGoalKeeper()) continue;
                    if (isReachable(ball[k], myPlayersDetails[i], myPlayers[i], myPlayersCantShoot[i], k)){
                        turn = k;
                        receiver = i;
                        forUs = true;
