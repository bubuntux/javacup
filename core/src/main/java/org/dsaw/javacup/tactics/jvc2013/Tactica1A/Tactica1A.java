package org.dsaw.javacup.tactics.jvc2013.Tactica1A;
...
public class Tactica1A implements Tactic {
  ...
  MyPosition align1[] = new MyPosition[]{
      new MyPosition(0, -52.41044776119403),
  ...
  Position alineacion2[] = new Position[]{
      new Position(0.2595419847328244, -52.41044776119403),
    ...
    public IPlayer[] getPlayers() {
      return new IPlayer[]{
          ...
          new JugadorImpl("Hamon", 2, new Color(255, 200, 150), new Color(50, 0, 0), 1d, 0.625d,
                          0.5d, false),
          new JugadorImpl("Viano", 3, new Color(255, 200, 150), new Color(50, 0, 0), 1d, 0.625d,
                          0.5d, false),
          new JugadorImpl("Weissert", 4, new Color(255, 200, 150), new Color(50, 0, 0), 1d, 0.625d,
                          0.5d, false),
          new JugadorImpl("Chérèque", 5, new Color(255, 200, 150), new Color(50, 0, 0), 1d, 0.625d,
                          0.5d, false),
          new JugadorImpl("Janin", 6, new Color(255, 200, 150), new Color(50, 0, 0), 1d, 0.625d,
                          0.5d, false),
          new JugadorImpl("Lacroix", 7, new Color(255, 200, 150), new Color(50, 0, 0), 1d, 0.625d,
                          0.5d, false),
          new JugadorImpl("Lextrait", 8, new Color(255, 200, 150), new Color(50, 0, 0), 1d, 0.625d,
                          0.5d, false),
          new JugadorImpl("Laburthe", 9, new Color(255, 200, 150), new Color(50, 0, 0), 1d, 1d, 1d,
                          false),
          new JugadorImpl("Niewen", 10, new Color(255, 200, 150), new Color(50, 0, 0), 1d, 1d, 1d,
                          false),
  ...
  int[] ableToShot;
  IPlayer[] myPlayersDetails;
  IPlayer[] enemiesDetails;
  ...
  public void initRound(GameSituations sp) {
    ...
    myGoal = new MyPosition(0, -Constants.LARGO_CAMPO_JUEGO / 2);
        /*
    ...
    for (int i = 1; i < minDestinations.length; ++i) {
      if (attack[i]) {
                /*minDestinations[i] = new Position(minX[i], Math.min(Constants.LARGO_CAMPO_JUEGO/2 - 10, (Math.max(0, ballwithZ.y))));
                maxDestinations[i] = new Position(maxX[i], (Math.max(0,Math.min(Constants.LARGO_CAMPO_JUEGO/2 - 10, ballwithZ.y + 30))));*/
        minDestinations[i] =
            new MyPosition(minX[i], Math.min(Constants.LARGO_CAMPO_JUEGO / 2 - 10,
                                             (Math.max(50 - Math.abs(minX[i]) / 2, ballwithZ.y))));
        maxDestinations[i] =
            new MyPosition(maxX[i], (Math.max(0, Math.min(Constants.LARGO_CAMPO_JUEGO / 2 - 10,
                                                          ballwithZ.y + 30))));
      } else if (middle[i]) {
        minDestinations[i] =
            new MyPosition(minX[i], Math.min(Constants.LARGO_CAMPO_JUEGO / 2 - 10,
                                             (Math.max(-Constants.LARGO_CAMPO_JUEGO / 2 + 10,
                                                       ballwithZ.y - 20))));
        maxDestinations[i] =
            new MyPosition(maxX[i], (Math.max(-Constants.LARGO_CAMPO_JUEGO / 2 + 10,
                                              Math.min(Constants.LARGO_CAMPO_JUEGO / 2 - 10,
                                                       ballwithZ.y + 20))));
      } else {
  ...
  public class DecisionScore {
    ...
    public void compute(MyPosition[] ball, boolean onlyMyPlayers) {
      ...
      for (int k = 1; k < length; ++k) {
        ...
        if (isGoal != 0 || isOut(ball[k])) {
          ...
          forUs = false;
          if (isGoal == 99) {
            if (isReachable(ball[k], enemiesDetails[enemyPortero], enemies[enemyPortero],
                            enemiesCantShoot[enemyPortero], k)) {
    ...
    public boolean isReachable(MyPosition ball, IPlayer jd, MyPosition currentPos,
                               int cantShoot, int turn) {
      if (cantShoot <= turn) {
        double maxHeight = Constants.ALTURA_CONTROL_BALON;
        double maxDist = Constants.DISTANCIA_CONTROL_BALON;
        if (jd.isGoalKeeper() && Math.abs(currentPos.getX()) <= Constants.LARGO_AREA_GRANDE / 2
            && Math.abs(currentPos.getY()) > Constants.ANCHO_AREA_GRANDE) {
  ...
  public class AverageDecisionScore extends DecisionScore {
    ...
    public AverageDecisionScore(DecisionScore ds, ArrayList<DecisionScore> dsList,
                                double errorAngle) {
      super(ds);
            /*
            count = 1;
      ...
      for (DecisionScore ds2 : dsList) {
        if (Math.abs(ds2.angle - ds.angle) <= errorAngle) {
          ...
          finalBall =
              new MyPosition((finalBall.getX() * count + ds2.finalBall.getX()) / (count + 1),
                             (finalBall.getY() * count + ds2.finalBall.getY()) / (count + 1),
                             ds.finalBall.z);
          if (ds2.forUs) {
            ++countForUs;
          }
  ...
  public AverageDecisionScore determineWhereToShot(int player, MyPosition ball) {
    ...
    double
        errorAngleDeg =
        Constants.getErrorAngular(myPlayersDetails[player].getPrecision()) * 180 / Math.PI;
    ...
    for (double strength = 0.1d; strength < 1.01; strength += 0.15d) {
      double
          speed =
          strength * Constants.getVelocidadRemate(myPlayersDetails[player].getPower()) * coefSaque;
      for (int angleZ = 0; angleZ <= 61; ++angleZ) {
        double
            radAngleZ =
            Math.max(0, Math.min(angleZ, Constants.ANGULO_VERTICAL_MAX)) * (Math.PI / 180d);
  ...
  public MyPosition[] predictShot(int player, MyPosition ball, double iStrength, double iAngle,
                                  double iAngleZ) {
    double angle = iAngle * (Math.PI / 180d);
    double speed = iStrength * Constants.getVelocidadRemate(myPlayersDetails[player].getPower());
    double
        angleZ =
        Math.max(0, Math.min(iAngleZ, Constants.ANGULO_VERTICAL_MAX)) * (Math.PI / 180d);
    ...
    AbstractTrajectory
        trajectory = new AirTrajectory(Math.cos(angleZ) * speed, Math.sin(angleZ) * speed, 0, 0);
    ...
    for (int step = 1; step < MAX_PREVISION; ++step) {
      ...
      MyPosition
          nextBall =
          new MyPosition(ballwithZ.x + trX * Math.cos(angle), ballwithZ.y + trX * Math.sin(angle),
                         trY * 2);
      res[step] = nextBall;
  ...
  public MyPosition[] predictBall(MyPosition ball, double iBallDx, double iBallDy, double iBallDz,
                                  int previsions) {
  ...
  public List<Command> execute(GameSituations sp) {
    //System.out.println("Turn: " + sp.iteration());
    // Initialize the round
    initRound(sp);
    ++nbturns;
    //System.out.println((sp.iteration())  + ": " + ballwithZ.toString());
    ...
    } else if (ds.finalBall.getY() < Constants.LARGO_ARCO / 2 - Constants.LARGO_CAMPO_JUEGO / 2
               && Math.abs(ds.finalBall.getX()) < Constants.LARGO_ARCO / 2) {
    ...
    for (int j : dangerousEnemies) {
      ...
      commands.add(new CommandMoveTo(iMin, enemies[j].getPosicion()
          .movePosition(0, -Constants.DISTANCIA_CONTROL_BALON / 2)));
    ...
    for (int i : availablePlayers) {
      ...
      Position
          diff =
          new Position(myPlayers[i].getX() - enemies[jMin].getX(),
                       myPlayers[i].getY() - enemies[jMin].getY());
      ...
      MyPosition
          tempDest =
          new MyPosition((minDestinations[i].x + maxDestinations[i].x) / 2,
                         (minDestinations[i].y + maxDestinations[i].y) / 2);
