...
public class Shoot {
  ...
  private boolean isShootSituation(double power) {
    ...
    return sp.ballPosition().distance(Constants.centroArcoSup)
           <= (30 - e) * factor * Constants.getVelocidadRemate(power)
  ...
  void execute(GameSituations sp, int[] canShoot) {
    ...
    if (isShootSituation(power)) {
      ...
      double
          angle =
          sp.ballPosition()
              .angle(Constants.posteDerArcoSup.movePosition(-Constants.RADIO_BALON * 2, 0));
      angle =
      ...
      double
          maxAngle =
          sp.ballPosition()
              .angle(Constants.posteIzqArcoSup.movePosition(Constants.RADIO_BALON * 2, 0));
      maxAngle =
  ...
  private double calculateFitness(Position position, double z, int iter, Position last,
    ...
    int
        oppIterToBall =
        calculateIterToBall(position, z, sp.rivalPlayers(), sp.rivalPlayersDetail(),
