...
public class Attackerl extends BasicCommandGenerator {
  ...
  /**
   * Los pases cuya línea de pase tenga alǵun rival a menos de esta distancia serán considerados
   * arriesgados
   */
  protected static final int PASS_SECURITY_MARGIN = 3;
  /**
   * Correr con el balón si hay algún rival a esta distancia, será considerado una mala idea
   */
    ...
    for (int i : sp.canKick()) {
      ...
      if (isBallInsideRivalLargeArea()) {
        ...
        //comprobamos si podemos centrar
                                            ...
                                            - MAX_DISTANCE_TO_SHOOT
                 && !isBallInsideRivalLargeArea()) {
        Position destino = getRandomPlayerInsideLargeArea();
        if (destino
            != null) {//Agregado para evitar java.lang.IndexOutOfBoundsException: Index: 0, Size: 0
          command = new CommandHitBall(i, destino, getPower(i, destino), true);
        }
      }
      ...
      if (command == null) {
        ...
        else if (isBallStopped() && sp.ballPosition().getY() < (Constants.LARGO_CAMPO / -2
                                                                + Constants.LARGO_AREA_GRANDE)) // patapúm p'arriba al jugador más adelantado
        {
          command = longPass(i);
        } else {
          if (!isBallStopped()) {
  ...
  protected Command shoot(int playerIdx) {
    ...
    proba_poste_Der =
        proba_poste_Der * probabilityToBeIntercepted(sp.myPlayers()[playerIdx],
                                                     Constants.posteDerArcoSup);
    proba_poste_Izq =
        proba_poste_Izq * probabilityToBeIntercepted(sp.myPlayers()[playerIdx],
  ...
  /**
   * Valora si un pase es bueno no malo. Mayor que 0 es un pase bueno y menor que 0 es malo.
   *
  ...
  /**
   * Calcula la probabilidad (de 0 a 1) de que un pase sea interceptado por el equipo rival.
   *
   ...
   * @return Probabilidad de que perdamos el balón (de 0 a 1, siendo 1 que nos van a quitar el
   * balón seguro)
   */
  ...
  protected Position getRandomPlayerInsideLargeArea() {
    ...
    if (posiblePlayers.size()
        > 0) {//Agregado para evitar java.lang.IndexOutOfBoundsException: Index: 0, Size: 0
      int idx;
      do {
