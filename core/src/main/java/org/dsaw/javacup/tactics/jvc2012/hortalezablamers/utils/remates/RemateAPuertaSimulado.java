...
public class RemateAPuertaSimulado {
  ...
  public static CommandHitBall rematarAPuerta(GameSituations gs, int indiceRematador) {
    ...
    if ((anguloAPosteIzdo - anguloAPosteDcho)
        < (anguloErrorIzda - anguloErrorDcha) * MAX_RELACION_ANGULOS) {
      return null;
    }
    ...
    while (true) {
      if (anguloIzda >= anguloAPosteIzdo + deltaAngulo
          && anguloDcha <= anguloAPosteDcho - deltaAngulo) {
        break;
      }
    ...
    for (Double angulo : angulos) { // indice de angulos
      CommandHitBall
          chb =
          new CommandHitBall(indiceRematador, angulo, HBConstants.FUERZA_GOLPEO_MAXIMA,
                             HBConstants.ANGULO_GOLPEO_TIRO);
