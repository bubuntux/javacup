...
public class Shoot {
  ...
  public Shoot(List<Command> comandos, Data data) {
    this.comandos = comandos;
    this.data = data;
  }

  private boolean isShootSituation(double power, Mentality mentality) {
  ...
  void execute(GameSituations sp, int[] canShoot, Mentality mentality) {
    this.sp = sp;
    double power = 1;
  ...
  private ShootInfo evaluateShoot(double power, double angle, double angleVert) {
    Trajectory
        trajectory =
        new Trajectory(sp.ballPosition(), Constants.getVelocidadRemate(power), angle, angleVert);
    double fitness = 0, f;
