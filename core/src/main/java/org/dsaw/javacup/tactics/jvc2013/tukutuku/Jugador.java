...
public abstract class Jugador {
  ...
  public ArrayList<Command> accionDefensa() {
    ...
    if ((entrenador.Gs.ballPosition().distance(alineacion[numero]) < 8) || (
      ...
      accion.add(new CommandMoveTo(numero, entrenador.Gs.ballPosition(),
                                   (entrenador.Gs.getMyPlayerEnergy(numero) > 0.8)));
    } else {
      Position
          jugRivalCercanoPos =
			...
			else*/
      {
        ...
        if ((entrenador.Gs.getMyPlayerEnergy(numero) > 0.7) && (
            entrenador.Gs.ballPosition().distance(this.posicionJugador) < 30)) {
  ...
  private posicionTiro[] generarArrayPuntuacionesGol(double[] arrayAngulos, PlayerI jug,
                                                     Position posBalon) {
  ...
  private double calcularValorXLineaDeFondo(Position posBalon, PlayerI jug, double angulo) {
    double nuevaX = -1000;
