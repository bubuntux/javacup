    ...
    public void ejecutar(Jugador jugador) {
        ...
        if (jugador.isBalonEnRangoDeControl()) {
            ...
            if (jugador.isAmenazado(Parametros.Jugador.AREA_SEGURA)
                ...
                Mensajero.getInstancia().entregarMensaje(Mensajero.INMEDIATO, jugador, receptor, Mensaje.RECIBIR_BALON, posicionPase);
            }
            ...
            Position posicionDisparo = jugador.getEquipo().puedeRematar(jugador, 1.0, Parametros.Jugador.DISTANCIA_REMATE_CUADRADA);
            if (posicionDisparo != null) {
                jugador.golpearBalon(posicionDisparo, false);
            }

            if (jugador.getRol() == Jugador.Rol.ATACANTE) {
                posicionDisparo = jugador.getEquipo().puedeRematar(jugador, 1.0, Parametros.Jugador.DISTANCIA_REMATE_CUADRADA * 1.5);
                if (posicionDisparo != null) {
                    jugador.golpearBalon(posicionDisparo, false);
                }
            }
