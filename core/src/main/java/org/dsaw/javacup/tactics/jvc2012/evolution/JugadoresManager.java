...
public class JugadoresManager {
  ...
  public static Jugador[] obtenerJugadores(Position[] posicionJugadores, int defensas, int centro,
                                           int delanteros) {
    ...
    double[]
        ...
        calcularCortes(delanteros, Constants.ANCHO_CAMPO_JUEGO / 2);
    int j = 0, x = 0, y = 0;
    ...
    for (int i = 0; i < posicionJugadores.length; i++) {
      if (i == SanMarinoEvolution.portero) {
        ...
        jugadores[i]
            .setPosicionAtaqueBase(new Position(cortesVerticalesDefensa[j++], LINEA_DEFENSA_BASE));
        defensas--;
      } else if (centro > 0) {
        jugadores[i] = new Jugador(i);
        jugadores[i].setRol(Jugador.CENTROCAMPISTA);
        jugadores[i]
            .setPosicionBase(new Position(cortesVerticalesCentro[x], LINEA_CENTROCAMPO_BASE));
        jugadores[i].setPosicionAtaqueBase(
            new Position(cortesVerticalesCentro[x++], LINEA_CENTROCAMPO_BASE + 25));
        centro--;
      } else if (delanteros > 0) {
        jugadores[i] = new Jugador(i);
        jugadores[i].setRol(Jugador.DELANTERO);
        jugadores[i]
            .setPosicionBase(new Position(cortesVerticalesDelantera[y], LINEA_DELANTERA_BASE));
        if (delanteros > 2) {
          jugadores[i].setPosicionAtaqueBase(
              new Position(cortesVerticalesDelantera[y++], LINEA_DELANTERA_BASE + 5));
        } else if (delanteros > 1) {
          jugadores[i].setPosicionBase(
              new Position(cortesVerticalesDelantera[y], LINEA_DELANTERA_BASE + 15));
          jugadores[i].setPosicionAtaqueBase(
              new Position(cortesVerticalesDelantera[y++], LINEA_DELANTERA_BASE + 15));
        } else {
          jugadores[i].setPosicionAtaqueBase(
              new Position(cortesVerticalesDelantera[y++], LINEA_DELANTERA_BASE));
        }
        delanteros--;
      }
    }
  ...
  public static Jugador[] cambiarTactica(Jugador[] jugadores, int defensas, int centro,
                                         int delanteros) {
  ...
  public static void actualizarRivalesEnZona(Jugador[] jugadores, GameSituations sp,
                                             Rival[] rivales) {
