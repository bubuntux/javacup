...
public class Simulador {
    ...
    public int calcularResultadoTiro(double fuerza,double angV,double pangH,Position posBalon,GameSituations sp){
        ...
        while(!encontrado&intento<limite){
            ...
            if(Math.abs(posActB[0])<=Constants.LARGO_ARCO&&posActB[1]>=Constants.LARGO_CAMPO_JUEGO&posActB[2]<=Constants.ALTO_ARCO) {return Gol;}
            if(Math.abs(posActB[0])>Constants.ANCHO_CAMPO_JUEGO/2.0||Math.abs(posActB[1])>Constants.LARGO_CAMPO_JUEGO/2.0){return Fuera;}
            ...
            if(cuantosAliados+cuantosEnemigos>0){
                if(cuantosEnemigos==0&cuantosAliados>0){return PaseBueno;}
                if(cuantosEnemigos==0&cuantosAliados>0){return PaseBueno;}
                if(cuantosAliados>0&&cuantosEnemigos>0){return PaseMediano;}
            }
    ...
    private int calcularCuantosLlegan(int intento, double[] posActB, boolean equipoPropio,GameSituations sp) {
        double x=posActB[0];
        double y=posActB[1];
        double z=posActB[2];
        ...
        for(int i=0;i<11;i++){
            if(z>Constants.ALTURA_CONTROL_BALON){
                if(detallesJugador[i].isGoalKeeper()){
                    ...
                    if(((distancia-Constants.DISTANCIA_CONTROL_BALON_PORTERO)/intento)<=detallesJugador[i].getSpeed()){
                        cuantos++;
                    }
                }
            }else{
    ...
    public datosTiro calcularPase(GameSituations sp,PlayerDetail jd,Position posAct,Position posObj){
    ...
    double[] calcularTiroCercano(double alturaBalon, double distancia, double pvelocidad) {
        ...
        while(iteraciones<500){
           distanciaRecorrida=trayectoriaAct.getX((double)iteraciones/60d)*Constants.AMPLIFICA_VEL_TRAYECTORIA;
           alturaAct=trayectoriaAct.getY((double)iteraciones/60d)*Constants.AMPLIFICA_VEL_TRAYECTORIA;
    ...
    public double[] distanciaHastaControlBalon(double alturaIni,double pvelocidad,double angulo,boolean portero){
        double[] res=new double[2];
        double alturaControl=Constants.ALTURA_CONTROL_BALON;
        if(portero){alturaControl=Constants.ALTO_ARCO;}
        ...
        while(vV>=0||aire){
            ...
            alturaAct+=vV;
             if(alturaAct>alturaControl&!aire){aire=true;}
    ...
    public double[] calcularPaseAire(double fuerzaJugador, double distancia,double alturaIni){
        ...
        for(double fuerza=1;fuerza>=0.1;fuerza=fuerza-0.05){
            for(double angulo=15;angulo<=60;angulo=angulo+2.5){
                double[] distAct=distanciaHastaControlBalon2(alturaIni, fuerza*fuerzaJugador, angulo, false);
...
public double[] distanciaHastaControlBalon2(double alturaIni,double pvelocidad,double angulo,boolean portero){
    double[] res=new double[2];
 double alturaControl=Constants.ALTURA_CONTROL_BALON;
        if(portero){alturaControl=Constants.ALTO_ARCO;}
            //direccion;
