...
//		}
	...
	private void MoverPortero(GameSituations sp) {
		...
		double y = 0;
		double a = 12;//15;
		double b = Constants.centroArcoInf.distance(formacionActual[0]) + 10; // Constants.centroArcoInf.distancia(formacionActual[0]);
		...
		if (!posecionBalon) {
			...
			if (anguloDestino >= anguloPosteIzq
				...
				if (sp.ballPosition().getX() > 0)
					comandos.add(new CommandMoveTo(0, Position.Intersection(sp.myPlayers()[0], Constants.posteDerArcoInf, sp.ballPosition(), destinoBalon)));
				else comandos.add(new CommandMoveTo(0, Position.Intersection(sp.myPlayers()[0], Constants.posteIzqArcoInf, sp.ballPosition(), destinoBalon)));
			}
		}
		//TODO mover portero
	}
	...
	/**
	 * Encuentra quien tiene el ballPosition en el momento asi el ballPosition no este en
	...
	/**
	 * Le dice al jugador que puede recuperar el ballPosition acercar a este. Se eligen
	 * jugadores segun la funcion sp.getTrajectory. Modifica comandos.
	 * 
	 * @param sp
	 */
				...
				if (j != 0) {
					...
					if (recuperadoresRival.length <= 1 || 
						...
						if (recuperadores[i] == 0) {
							if (jugadores[0].distance(sp.rivalPlayers()[jugadores[0].nearestIndex(sp.rivalPlayers())]) > 4) {
								if (sp.ballPosition().distance(jugadores[0]) - 4 < Constants.DISTANCIA_CONTROL_BALON_PORTERO) {
									double altura = sp.ballAltitude();
									if (altura > Constants.ALTO_ARCO - 1 ||
											velocidadBalon > 1.7) {
										Position des = jugadores[0].moveAngle((jugadores[0].angle(sp.ballPosition()) - Math.PI), 3);
	...
	/**
	 * Marcaje a los posibles destinos de pase cuando no se tiene el ballPosition.
	 * Modifica comandos.
	 * 
	 * @param sp
	 */
			...
			if (jugador != 0 && !jugadorMarcando[jugador]) {
				...
				if (distanciaAlRival < DISTANCIA_MAXIMA_BUSQUEDA_RIVAL) {
					destino = rivales[k].moveAngle(rivales[k].angle(sp
							.ballPosition()), DISTANCIA_MARCAJE_RIVAL);
					...
					if ((jugador == 1 
						...
						formacionActual[jugador] = formacionActual[jugador].moveAngle(formacionActual[jugador].angle(destino), 0.1);
					}
			...
			for (int j = 1; j < jugadores.length; j++) {
				if (i != j && 
					...
					actualDestino = PosicionBalonAlEspacio(sp, i, jugadores[i],
							j, jugadores[j], sp.rivalPlayers(), 0.9, 50, false);
	...
	/**
	 * Encuentra una posicion cercana al destino cuya presencia de rivalPlayers sea
	 * lo menor posible.
	 * 
	 * @param origen
	 * @param destino
	 ...
	 */
	private Position PosicionBalonAlEspacio(GameSituations sp, int i, Position origen, int j, Position destino,
			Position[] rivales, double radioAvanceEspacio, double maximoRadioAvance, boolean minimaCercania) {
	...
	/**
	 * Da un valor a la disputa entre dos jugadores rivalPlayers entre si para llegar
	...
	private double CercaniaAlArcoRival(Position posicion) {
		...
		cercaniaAlArcoRival = Constants.centroArcoSup.distance(posicion) * 50 / Constants.LARGO_CAMPO_JUEGO;
	 ...
	 * mayor valoracion (100) corresponde a una presencia muy alta de rivalPlayers en
	 * el camino.
	 * 
	 * @param jugadorOrigen
	...
	/**
	 * Cercania al destino de los rivalPlayers que depende de la distancia al origen
	...
	private double DispararAGol(GameSituations sp) {
		...
		for (int i : sp.canKick()) {
			...
			if (distanciaAArco < DISPARAR_DISTANCIA_MINIMA) {
				...
				valorDistanciaAArco = 50 - ((distanciaAArco - DISPARAR_DISTANCIA_LARGA) * 50 / (Constants.LARGO_CAMPO_JUEGO - DISPARAR_DISTANCIA_LARGA));
			}
			...
			objetivoEnArco = ObjetivoEnArco(porteroRival, sp.ballPosition(), sp.getMyPlayerError(i), distanciaAArco);
	...
	private double AnguloVertical(Double distanciaAArco, double remate) {
		...
		if (distanciaAArco > 60)
			return 30;
		double anguloVertical = 2 * (Constants.ALTO_ARCO + 1) / ((Constants.LARGO_CAMPO_JUEGO - distanciaAArco) / 2);
		anguloVertical = Math.atan(anguloVertical);
		anguloVertical = anguloVertical + anguloVertical * (1 - Constants.FACTOR_DISMINUCION_VEL_BALON_AIRE);
	 ...
	 */
	private Position ObjetivoEnArco(Position portero, Position balon, double error, Double distanciaAArco) {
		Position objetivoEnArco;
	...
	/**
	 * Acarrear el ballPosition. Le da un valor a la posibilidad de regate teniendo en
	 * cuenta la presencia de rivalPlayers.
	 * 
	 * @param sp
	 ...
	 * del ballPosition y quien este atacando.
	 * 
	 * @param sp
	 */
