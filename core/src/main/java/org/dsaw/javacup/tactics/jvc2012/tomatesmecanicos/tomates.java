  ...
  class TacticDetailImpl implements TacticDetail {
    ...
    public PlayerDetail[] getPlayers() {
      return new PlayerDetail[]{
          new JugadorImpl("Fabian", 1, new Color(255, 200, 150), new Color(50, 0, 0), 0.92d, 0.95d,
                          0.76d, true),
          new JugadorImpl("Camilo", 2, new Color(255, 200, 150), new Color(51, 51, 51), 0.76d,
                          0.81d, 0.77d, false),
          new JugadorImpl("Andres", 3, new Color(102, 0, 0), new Color(50, 0, 0), 0.8d, 0.72d,
                          0.74d, false),
          new JugadorImpl("Francisco", 4, new Color(255, 200, 150), new Color(204, 0, 0), 0.87d,
                          0.83d, 0.74d, false),
          new JugadorImpl("David", 5, new Color(255, 255, 204), new Color(51, 51, 51), 0.85d, 0.73d,
                          0.75d, false),
          new JugadorImpl("Alvaro", 23, new Color(255, 200, 150), new Color(50, 0, 0), 0.84d, 0.72d,
                          0.83d, false),
          new JugadorImpl("Juan", 7, new Color(255, 200, 150), new Color(50, 0, 0), 0.88d, 0.7d,
                          0.8d, false),
          new JugadorImpl("Gonzalo", 8, new Color(255, 200, 150), new Color(50, 0, 0), 0.83d, 0.8d,
                          0.85d, false),
          new JugadorImpl("Hernan", 9, new Color(255, 200, 150), new Color(50, 0, 0), 0.93d, 0.86d,
                          0.92d, false),
          new JugadorImpl("Miguel", 10, new Color(255, 255, 204), new Color(0, 0, 0), 0.96d, 0.84d,
                          0.92d, false),
          new JugadorImpl("Alejandro", 17, new Color(255, 200, 150), new Color(50, 0, 0), 0.97d,
                          0.92d, 0.93d, false)
      };
    }
  }
  ...
  public List<Command> execute(GameSituations sp) {
    ...
    if (rat < 4) {
      ...
      if (rat == 1) {
        ...
        comandos.add(
            new CommandMoveTo(1, new Position(pmediaizq.getX(), rivales[jugrat[0]].getY() - 3)));
        comandos.add(
            new CommandMoveTo(3, new Position(pmediader.getX(), rivales[jugrat[0]].getY() - 3)));
      }
      ...
      if (rat == 2) {
        ...
        if (rivales[jugrat[1]].getY() < rivales[jugrat[0]].getY()) {
          comandos.add(
              new CommandMoveTo(2, new Position(pmedia.getX(), rivales[jugrat[1]].getY() - 3)));
        } else {
          comandos.add(
              new CommandMoveTo(2, new Position(pmedia.getX(), rivales[jugrat[0]].getY() - 3)));
        }
      }
      ...
      if (rat == 4) {
        ...
        for (int i = 1; i < 5; i++) {
          ...
          if (jugadores[i].getY() > alineacion1[i].getY() - 6) {
            comandos.add(new CommandMoveTo(i, new Position(alineacion1[i].getX(),
                                                           alineacion1[i].getY() - 12)));
          } else {
    ...
    if (balon.getY() > sp.getTrajectory(sp.iteration() + 1)[1] && balon.getY() < -40) {
      ...
      if (jugadores[5].distance(rivales[min]) < 5) {
        comandos.add(
            new CommandMoveTo(5, new Position(jugadores[5].getX() + l, jugadores[5].getY() + l1)));
      }
    }
    ...
    for (int i = 6; i < jugadores.length; i++) {
      ...
      if (jugadores[i].distance(rivales[min]) < 5) {
        comandos.add(
            new CommandMoveTo(i, new Position(jugadores[i].getX() + l, jugadores[5].getY() + l1)));
      }
    }
    ...
    if (balon.getY() > 15) {
      if (rat < 4) {
        comandos.add(
            new CommandMoveTo(6, new Position(alineacion2[6].getX(), alineacion2[6].getY() + 12)));
        comandos.add(
            new CommandMoveTo(7, new Position(alineacion2[7].getX(), alineacion2[7].getY() + 12)));
      } else {
        comandos.add(
            new CommandMoveTo(6, new Position(alineacion1[6].getX(), alineacion1[6].getY() + 12)));
        comandos.add(
            new CommandMoveTo(7, new Position(alineacion1[7].getX(), alineacion1[7].getY() + 12)));
      }
    }
      ...
      if (sp.isStarts()) {
        ...
        if (balon.getY() == -47 && (balon.getX() == 9.16 || balon.getX() == -9.16)) {
          ...
          if (balon.getY() == 52.5 && (balon.getX() == 34 || balon.getX() == -34)) {
            ...
            if (balon.getX() == 34) {
              comandos
                  .add(new CommandMoveTo(7, new Position(posRecuperacion[0], posRecuperacion[1])));
            } else {
              comandos
                  .add(new CommandMoveTo(6, new Position(posRecuperacion[0], posRecuperacion[1])));
            }
            ...
            if (balon.getY() == 0 && balon.getX() == 0) {
              comandos
                  .add(new CommandMoveTo(9, new Position(posRecuperacion[0], posRecuperacion[1])));
              ...
              if (recuperadores.length > 1) {
                ...
                comandos.add(new CommandMoveTo(recuperadores[1], new Position(posRecuperacion[0],
                                                                              posRecuperacion[1])));
                ...
                rec[2] = 2;
                for (int i = 1; i < jugadores.length; i++) {
                  if (jugadores[i].distance(jugadores[recuperadores[1]]) < jugadores[i]
                      .distance(jugadores[rec[0]]) && i != recuperadores[1]) {
                ...
                for (int i = 0; i < rec.length; i++) {
                  for (int j = 1; j < jugadores.length; j++) {
                    if (jugadores[rec[i]].distance(rivales[j]) < jugadores[rec[i]]
                        .distance(rivales[recdr[i]])) {
                      recdr[i] = j;
                    }
                  }
                }
                ...
                for (int i = 0; i < rec.length; i++) {
                  for (int j = i + 1; j < recdr.length; j++) {
                    if (jugadores[rec[i]].distance(rivales[recdr[i]]) > jugadores[rec[j]]
                        .distance(rivales[recdr[j]])) {
                      mrec = j;
                    }
                  }
                  if (mrec == 0) {
        ...
        if (recuperadores.length > 1) {
          ...
          if (recuperadores[1] == 0) {
            if (recuperadores.length > 2 && recuperadores[0] > 2) {
              comandos.add(new CommandMoveTo(recuperadores[2],
                                             new Position(posRecuperacion[0], posRecuperacion[1])));
            }
          } else {
            comandos.add(new CommandMoveTo(recuperadores[1],
                                           new Position(posRecuperacion[0], posRecuperacion[1])));
          }
        }
      }
      ...
      if (sp.isStarts()) {
          ...
          if (k == 8 || k == 9 || k == 10) {
            if (balon.getY() < 35) {
              ...
              if (op == 0) {
                ...
                if (re == 0) {
                  comandos
                      .add(new CommandHitBall(k, Constants.posteDerArcoSup, 1, 10 + r.nextInt(12)));
                } else {
                  if (re == 1) {
                    comandos.add(
                        new CommandHitBall(k, Constants.posteIzqArcoSup, 1, 10 + r.nextInt(12)));
                  } else {
                    comandos
                        .add(new CommandHitBall(k, Constants.centroArcoSup, 1, 10 + r.nextInt(12)));
                  }
                }
              }
            }
        ...
        if (balon.getY() < 25 && balon.getY() > -20) {
          if (jugadores[k].distance(rivales[riv]) > 1) {
            ...
            int recp;
            if (k >= 6 && k
                          <= 10) {//Modificacion realizada por fabnun para evitar que se quede pegado el partido
              do {
                recp = r.nextInt(5) + 6;
              } while (recp != k);
              int t = 10;
