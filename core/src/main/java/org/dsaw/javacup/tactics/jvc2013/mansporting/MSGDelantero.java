...
public class MSGDelantero extends MSGMedioCentro {
  ...
  public MSGDelantero(int index, Color colorPelo, Color colorPiel, String nombre, int numero,
  ...
  protected CommandHitBall tira() {
    double
        posicionY =
        posicion.getY() < MSGConstants.DISTANCIA_NORMALIZACION_TIRO
        ? MSGConstants.DISTANCIA_NORMALIZACION_TIRO : posicion.getY();
    ...
    Punto
        destino =
        new Punto(
            context.getPorteroRival().getPosicion().getX() > 0 ? Constants.centroArcoSup.getX()
                                                                 - variacionX
                                                               : Constants.centroArcoSup.getX()
                                                                 + variacionX,
