...
public class TacticaMagallanes implements Tactic {
	...
	public abstract class  DSL{
		...
		public Position arcoRival(){return Constants.centroArcoSup;}
		public Position posicionBalon(){return sp.ballPosition();}
		public Position arcoPropio(){return Constants.centroArcoInf;}
		public Position media(Position p1, Position p2){return Position.medium(p1,p2);}
		public double getX(Position p){ return p.getX();}
		public double getY(Position p){ return p.getY();}
		public abstract Position resolve();
		public Position posicionJugador() {return sp.myPlayers()[player];}
		public Position propio(int i) {return sp.myPlayers()[i];}
		public Position rival(int i) {return sp.rivalPlayers()[i];}
	...
	public DSL instanciarDSL(TacticaMagallanes t,String dsl,GameSituations sp, Integer player){
		...
		try {
			instance=(DSL)Class.forName(
					"org.javahispano.javacup.tacticas.tacticas_aceptadas.TacticaMagallanes.TacticaMagallanes$"+dsl).getConstructor(TacticaMagallanes.class,GameSituations.class,Integer.class).newInstance(t,sp,player);
	...
	class Engine {
		...
		public Cmd bestPosicion(int i) {
			...
			if (inside(i,delanteros) && sp.ballPosition().getY() < 30 && sp.ballPosition().getY() > -60)
			{
		...
		public Cmd bestPase(int i) {
			...
			Position[] alternativas = null;
			List<Position> alt = new ArrayList<Position>();
