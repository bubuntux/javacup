...
public class MSGSituacionPartidoUIListener implements MSGSituacionPartidoListener {
   ...
   */
  public List<Segmento> segmentosEvaluarPase = new LinkedList<>();
  /**
   * Segmentos en los que evaluar los pases.
   */
  public List<Segmento> segmentosEvaluarDesmarcaje = new LinkedList<>();
  /**
   * Marcajes.
   */
  public List<Segmento> segmentosMarcaje = new LinkedList<>();
  /**
   * Tiro centrado.
   */
  ...
  public List<Punto> puntosPatetics = new LinkedList<>();
    ...
    if (MSGConstants.VISUAL_DEBUG) {
      SwingUtilities.invokeLater(new Runnable() {
        ...
        public void run() {
          ...
          // Porterías
          frame.getCanvas().addDrawableXY(new SegmentoDrawable(
              new Segmento(new Punto(Constants.posteDerArcoInf),
                           new Punto(Constants.posteIzqArcoInf)), Color.GREEN));
          frame.getCanvas().addDrawableXY(new SegmentoDrawable(
              new Segmento(new Punto(Constants.posteDerArcoSup),
                           new Punto(Constants.posteIzqArcoSup)), Color.GREEN));
          frame.getCanvas().addDrawableXZ(new RectanguloDrawable(
              new Rectangulo(new Punto(0, Constants.ALTO_ARCO / 2), Math.abs(
                  Constants.posteDerArcoSup.getX() - Constants.posteIzqArcoSup.getX()),
                             Constants.ALTO_ARCO), Color.GREEN));
          frame.getCanvas().addDrawableZY(new SegmentoDrawable(
              new Segmento(new Punto(0, Constants.posteDerArcoInf.getY()),
                           new Punto(Constants.ALTO_ARCO, Constants.posteIzqArcoInf.getY())),
              Color.GREEN));
          frame.getCanvas().addDrawableZY(new SegmentoDrawable(
              new Segmento(new Punto(0, Constants.posteDerArcoSup.getY()),
                           new Punto(Constants.ALTO_ARCO, Constants.posteDerArcoSup.getY())),
              Color.GREEN));

          // Jugadores propios
          for (MSGJugadorPropio situacion : context.getJugadoresPropios().values()) {
            frame.getCanvas().addDrawableXY(
                new RectanguloDrawable(situacion.getAlineacion().getArea(),
                                       new Color(0, 255, 255, 128)));
            frame.getCanvas().addDrawableXY(
                new CirculoDrawable(situacion.getAreaCubierta(1), new Color(255, 0, 0, 64)));
            if (situacion.isJugadorMasCercano()) {
              frame.getCanvas().addDrawableXY(new RectanguloDrawable(
                  new Rectangulo(situacion.getPosicion(), situacion.getAreaCubierta(5).getRadio(),
                                 situacion.getAreaCubierta(5).getRadio()),
          ...
          for (MSGJugadorDetalle situacion : context.getJugadoresRivales().values()) {
            frame.getCanvas().addDrawableXY(
                new CirculoDrawable(situacion.getAreaCubierta(1), new Color(0, 0, 255, 64)));
          }
          ...
          frame.getCanvas().addDrawableXY(new CirculoDrawable(
              new Circulo(balon.getPosicion().getPuntoXY(), Constants.RADIO_BALON), Color.BLACK));
          frame.getCanvas().addDrawableXZ(new CirculoDrawable(
              new Circulo(balon.getPosicion().getPuntoXZ(), Constants.RADIO_BALON), Color.BLACK));
          frame.getCanvas().addDrawableZY(new CirculoDrawable(
              new Circulo(balon.getPosicion().getPuntoZY(), Constants.RADIO_BALON), Color.BLACK));

          if (context.getJugadoresCercanosBalonData() != null) {
            frame.getCanvas()
                .addDrawableXY(new JugadorBalonDrawable(context.getJugadoresCercanosBalonData()));
          }
          ...
          for (Punto punto : puntosPatetics) {
            frame.getCanvas().addDrawableXY(
                new CirculoDrawable(new Circulo(punto, 5 * Constants.RADIO_BALON), Color.GREEN));
          }
