...
public class PlayerRender {
  ...
  public PlayerRender(TacticDetail impl, boolean uniformeAlternativo) throws SlickException {
    this();
    setImpl(impl);
    update(uniformeAlternativo);
  }

  public PlayerRender() throws SlickException {
    for (int j = 0; j < 7; j++) {
      for (int i = 1; i < 7; i++) {
    ...
    for (int i = 0; i < 11; i++) {
      ...
      for (int j = 0; j < 14; j++) {
        ...
        for (int x = 0; x < 20; x++) {
          for (int y = 0; y < 20; y++) {
            c0 =
                img[idx][(alternativa ? impl.getStyle2() : impl.getStyle()).getNumero() - 1]
                    .getColor(x, y);
            c1 = null;
            if (c0.equals(franja)) {
