...
public final class GameSituations {
  ...
  public double getMyPlayerAceleration(int jugIndex) {
    return miAcceleration[jugIndex]
        .getAccelerationGlobal(); //La aceleracion se compone de las dos aceleraciones en los dos ejes
  }
  ...
  public double getRivalAceleration(int jugIndex) {
    return rivalAcceleration[jugIndex].getAccelerationX() * rivalAcceleration[jugIndex]
        .getAccelerationY(); //La aceleracion se compone de las dos aceleraciones en los dos ejes
  }
  ...
  /**
   * Retorna el numero de iteraciones cursadas en el partido, el total de iteraciones esta dado por
   * Constants.ITERACIONES
   */
  ...
  /**
   * Retorna un array con las coordenadas x,y,z de la trayectoria del ballPosition en la iteration
   * indicada.
   */
  ...
  /**
   * Devuelve la distancia total recorrida por un jugador en las iteracciones que se le pasa como
   * parametro sin tener en cuenta el factor sprint*
   */
  ...
  /**
   * retorna un array donde el primer elemento es la iteration donde se puede recuperar el
   * ballPosition, los siguientes números corresponden a los indices de los jugadores que pueden
   * recuperar el ballPosition, ordenados desde el mas cercano al mas lejano del punto de
   * recuperación.
   */
  @Deprecated
      ...
      if (posBalon[2] <= Constants.ALTO_ARCO) {
        for (int i = 0; i < misJugadores.length; i++) {
          if (posBalon[2] <= (detalles[i].isGoalKeeper() ? Constants.ALTO_ARCO
            ...
            pJug = misJugadores[i];
            dist0 =
                distanceTotal(i, it); //Distancia que puede recorrer el jugador en it iteracciones
            dist =
                pJug.distance(
                    new Position(posBalon[0], posBalon[1])); //Distancia entre el jugador y el balon
            if (dist0 >= dist) {
              found = true;
              founds.add(dist);
              founds.add((double) i);
  ...
  /**
   * Retorna un array indicando verdadero o falso si el jugador del indice correspondiente esta
   * fuera de juego*
   */
   ...
   */
  protected void set(Position balon, double alturaBalon, int golesMios, int golesContrarios,
                     int iteracion, Position[] mios, Position[] contrarios,
                     Acceleration[] miAcceleration, Acceleration[] rivalAcceleration,
                     double[] miEnergia, double[] rivalEnergia, boolean saco, boolean sacaRival,
                     int[] iterGolpearBalonLocal, int[] iterGolpearBalonVisita,
                     AbstractTrajectory trayectoria, double x0, double y0, double t0, double a0,
                     int iterReal, boolean invert) {
    this.balon = balon;
   ...
   * informaci�n. Verdadero si es el equipo local el que ha rematado
   */
