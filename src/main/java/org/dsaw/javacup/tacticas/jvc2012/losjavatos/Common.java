...
public class Common {
    ...
    public void tAP() {
        ...
        for (int i : commonSp.canKick()) {
            if (commonSp.rivalPlayers()[pCo].getX() >= 0) {
                ...
                ang = calcAng(commonSp.myPlayers()[i].distance(Constants.posteIzqArcoSup),commonSp.getMyPlayerPower(i));
            } else {
    ...
    private void cub(int jug,Position pos){
        ...
        if (disX < -10 ) cubX -= 1.5;
        else if (disX < -5) cubX -= 0.75;
        else if (disX < 5);
        else if (disX < 10) cubX += 0.75;
        else if (ballX > 10) cubX += 1.5;
