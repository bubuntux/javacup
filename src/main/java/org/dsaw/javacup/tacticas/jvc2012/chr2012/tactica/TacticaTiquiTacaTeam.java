...
import org.dsaw.javacup.tacticas.jvc2012.chr2012.accion.Accion;
import org.dsaw.javacup.tacticas.jvc2012.chr2012.accion.AccionAvanzar;
import org.dsaw.javacup.tacticas.jvc2012.chr2012.accion.AccionDespejar;
import org.dsaw.javacup.tacticas.jvc2012.chr2012.accion.AccionPasar;
import org.dsaw.javacup.tacticas.jvc2012.chr2012.accion.AccionPasarAlArea;
import org.dsaw.javacup.tacticas.jvc2012.chr2012.accion.AccionPasarAlHueco;
import org.dsaw.javacup.tacticas.jvc2012.chr2012.accion.AccionRegatear;
import org.dsaw.javacup.tacticas.jvc2012.chr2012.accion.AccionTirar;
import org.dsaw.javacup.tacticas.jvc2012.chr2012.accion.AccionTirarTrallon;
import org.dsaw.javacup.tacticas.jvc2012.chr2012.accion.Generador;

import org.dsaw.javacup.tacticas.jvc2012.chr2012.tactica.utilidades.Balon;
import org.dsaw.javacup.tacticas.jvc2012.chr2012.tactica.utilidades.Entorno;
import org.dsaw.javacup.tacticas.jvc2012.chr2012.tactica.utilidades.Jugador;
import org.dsaw.javacup.tacticas.jvc2012.chr2012.tactica.utilidades.Rival;
 ...
 * 
 * @author Christian Onwuzor Martín (chr -> christian.onwuzor@gmail.com)
 */
public class TacticaTiquiTacaTeam implements Tactic {
    ...
    private void calculaPosesion(GameSituations sp) {
        ...
        if (sp.canKick().length > 0 || sp.isStarts()) {
        	...
        	// la pelota.
        		...
        		if (sp.rivalPlayers()[jugadorRivalPosesion].distance(sp.ballPosition()) > Constants.DISTANCIA_CONTROL_BALON * 3) {
        			jugadorRivalPosesion = -1;
        		}
        	}
        }
    ...
    private void movimientosJugadorPosesion(GameSituations sp, Jugador jugador) {
         ...
         if (contactoBalon(sp, jugador.getIndice())) {
             ...
             AccionTirar tirar = sp.isStarts() ? null : gen.tirar(sp, jugador, true, arrayJugadores, arrayRivales);
    ...
    private void calculaEejecucionTecnicaBarrido (GameSituations sp, Jugador jugador, AccionAvanzar avanzar) {
			...
			if (miJugadorLlegaAntes(sp, true)) {
				...
				if (jugador.getMarcaje() != null) {
					...
					realizaIrA(jugador, 
							   jugador.getMarcaje().getPosicion().moveAngle(
									   	jugador.getMarcaje().getPosicion().angle(sp.ballPosition()), Entorno.DISTANCIA_MARCAJE_DEFENSA));
				...
				// No tengo ning�n marcaje asignado.
					...
					if (conjuntoDelanteros.contains(jugador)) {
						...
						if (sp.ballPosition().getY() > Entorno.alineacionNormal[jugador.getIndice()].getY()) {
							...
							if ((sp.ballPosition().getX() <0 && sp.myPlayers()[jugador.getIndice()].getX() < 0) ||
								(sp.ballPosition().getX() >0 && sp.myPlayers()[jugador.getIndice()].getX() > 0)) {
								yDelantero = -3;
							}
							else {
								yDelantero = 2;
					...
					// Los extremos no bajan m�s del l�mite de la defensa.
						...
						realizaIrA(jugador, new Position(Entorno.alineacionNormal[jugador.getIndice()].getX(), yMedio));
					}
					...
					// dirigirse hacia la Y del bal�n.
						...
						double yMedio = Math.min(Entorno.alineacionNormal[jugador.getIndice()].getY(), sp.ballPosition().getY());
    ...
    private void mueveMedioCentrosSinBalon(GameSituations sp, Jugador medioCentro) {
    	...
    	// En caso contrario nos movemos normalmente.
	    	...
	    	if (sp.ballPosition().getY() > y) {
	    		...
	    		if ((Entorno.alineacionNormal[medioCentro.getIndice()].getX() < 0 && sp.ballPosition().getX() < 0) ||
	    			(Entorno.alineacionNormal[medioCentro.getIndice()].getX() > 0 && sp.ballPosition().getX() > 0)) {
	    			variacionY = 10;
	    		}
	    	...
	    	Position posInicial = new Position(Entorno.alineacionNormal[medioCentro.getIndice()].getX(), y);
	    	Position posDefecto = new Position(Entorno.alineacionNormal[medioCentro.getIndice()].getX(), y);
   ...
   private void mueveExtremoSinBalon(GameSituations sp, Jugador extremo) {
	   	...
	   	// En caso contrario nos movemos normalmente.
			...
			if ((Entorno.alineacionNormal[extremo.getIndice()].getX() < 0 && sp.ballPosition().getX() < 0) ||
				(Entorno.alineacionNormal[extremo.getIndice()].getX() > 0 && sp.ballPosition().getX() > 0)) {
				variacionY = 14;
			}
			// El bal�n se encuentra en el lado contrario del extremo.
			else {
    ...
    private void mueveDelanteroSinBalon(GameSituations sp, Jugador delantero) {
    	...
    	if (y > limiteY) {
    		...
    		if (limiteY - sp.ballPosition().getY() > 10) {
    			...
    			if (sp.ballPosition().getX() > 0 && Entorno.alineacionNormal[delantero.getIndice()].getX() < 0 ||
    				sp.ballPosition().getX() < 0 && Entorno.alineacionNormal[delantero.getIndice()].getX() > 0) {
    ...
    private Position buscaHueco(GameSituations sp,
    	...
    	boolean rivalInterceptaPase = false;
    	boolean salir = false;
		...
		do {
			...
			rivalInterceptaPase = gen.rivalEnTrayectoria(sp, sp.ballPosition(), pos, Entorno.DISTANCIA_CORTE_PASE);
    ...
    private void mueveTacticaBarridoSinBalon (GameSituations sp, Jugador jugador) {
		...
		if (Math.abs(diffBalonX) < distancia) {
			...
			if (diffBalonX < 0) {
				...
				// En este caso nos salimos del campo por lo que nos dirigimos hacia la izquierda pero hacia arriba tambien.
					...
					realizaIrA(jugador, new Position(sp.ballPosition().getX() - distancia,
							 						 Math.min((sp.myPlayers()[jugador.getIndice()].getY() + 20), Constants.LARGO_CAMPO_JUEGO)));
				}
			}
		...
		// de pase, tiro, etc.
		else if (sp.ballPosition().getY() < ((Constants.LARGO_CAMPO_JUEGO/2) - Constants.ANCHO_AREA_GRANDE)){
    ...
    private void decideP (GameSituations sp, Jugador jugador, AccionPasar pasar) {
        ...
        // El pase es hacia atras.
        	...
        	// Si el destino del pase se encuentra dentro del area y tiene tiro a puerta vac�a se la pasamos.
        	else if ((sp.myPlayers()[pasar.jugadorDestino().getIndice()].getY() > ((Constants.LARGO_CAMPO_JUEGO/2) - Constants.ANCHO_AREA_GRANDE)) &&
        			 (Math.abs(sp.myPlayers()[pasar.jugadorDestino().getIndice()].getX()) < Constants.LARGO_AREA_GRANDE/2) &&
        		...
        		if (sp.ballPosition().getY() > ((Constants.LARGO_CAMPO_JUEGO /2) - Constants.ANCHO_AREA_GRANDE) &&
    ...
    private void decideT (GameSituations sp, Jugador jugador, AccionTirar tirar) {
    	...
    	//System.out.println("decideT");
    	
        // Si es a puerta vac�a o tiro seguro.
        if (tirar.tiroSeguro() || tirar.aPuertaVacia()) {
        	...
        	if (anguloTiro > (0 + variacion) && (anguloTiro < (180 - variacion))) {
        		...
        		if (tirar.dentroDeForzado()) {
        			...
        			// Si tenemos una variacion considerable realizamos el tiro.
     ...
     private void decideAP (GameSituations sp, Jugador jugador,
    	...
    	//System.out.println("decideAP");
    	 
        // Es un delantero o es un medio
        if (conjuntoDelanteros.contains(jugador) || conjuntoMedios.contains(jugador)) {
            ...
            // a realizar.
				...
				// Recuperemos los �ndices de los rivales mas cercanos que esten m�s adelantados sobre el eje Y
	            ...
	            // Si tenemos rivales por encima del eje Y.
				if (rivalCercanoDestino != null && rivalCercanoOrigen != null) {

					// Si el rival del origen esta m�s alejado del jugador origen.
					if (sp.rivalPlayers()[rivalCercanoOrigen].distance(sp.myPlayers()[jugador.getIndice()]) > 
						sp.rivalPlayers()[rivalCercanoDestino].distance(sp.myPlayers()[pasar.jugadorDestino().getIndice()])) {
					...
					// Si el rival del destino sobre Y se encuentra m�s alejado que el rival del origen le 
					// pasamos la pelota.
					else {
						realizaPase(pasar, sp);
					}
				}
				// Si el jugador con la posesi�n no tiene rivales por encima del eje Y avanza.
				else if (rivalCercanoOrigen == null && demarcacion == 'D') {
					realizaAvance(avanzar, sp);
				}
				// Si el jugador destino no tiene rivales por encima del eje Y le pasamos la pelota. 
				else {
					realizaPase(pasar, sp);
				}
			}
        ...
        // Es un defensa
        else if (conjuntoDefensas.contains(jugador)) {
            // El defensa directamente pasa la pelota.
            realizaPase(pasar, sp);
        }
        // Es un portero
        else {
         ...
         if (conjuntoDelanteros.contains(jugador) || conjuntoMedios.contains(jugador)) {
             ...
             // Si no es un tiro a puerta vacia ni seguro
             	...
             	// Dentro del �rea de forzado de tiro tiramos
             		...
             		Accion tirarDestino = gen.tirar(sp, pasar.jugadorDestino(), false, arrayJugadores, arrayRivales);
             		...
             		// Si podemos regatear cuanto m�s grande sea la distancia de tiro m�s regatearemos. 
             			...
             			Double distanciaTiro = sp.myPlayers()[jugador.getIndice()].distance(Constants.centroArcoSup);
             			AccionRegatear regatear = gen.regatear(sp, jugador, arrayRivales);
     					if (rand.nextInt(distanciaTiro.intValue()) > 10 && regatear != null) {
     						realizaRegate(regatear, sp);
     					}
     					else {
     						realizaTiroTrallon(sp, jugador);
    ...
    private void decide(GameSituations sp, Jugador jugador) {
    	...
    	if (sp.isStarts() && iteracionesParaSacar > 0) {
        	...
        	// Si estamos para forzar el tiro lo forzamos.
        	if (forzarTiro(sp)) {
        		realizaTiroTrallon(sp, jugador);
        	}
    		
            // No es el portero.
        	else if (jugador.getIndice() != 0) {
		        	...
		        	if (paseAlHueco != null && paseAlHueco.paseHaciaElArea()) {
			            ...
			            if (conjuntoDelanteros.contains(jugador) ||
			            	...
			            	if (regatear != null && regatear.regateHaciaPorteria()) {
			                	...
			                	if (gen.destinoDentroArea(sp, sp.ballPosition())) {
			                		realizaTiroTrallon(sp, jugador);
			                	}
			                	else {
					...
					if (distanciaPorteria <= Constants.DISTANCIA_PENAL) {
			        	...
			        	if (paseAlHueco != null) {
				            ...
				            if (conjuntoDelanteros.contains(jugador) ||
				                ...
				                AccionRegatear regatear = gen.regatear(sp, jugador, arrayRivales);
				            	if (regatear != null && contRegate < 10) {
				            		realizaRegate(regatear, sp);
				            	}
				                else {
    ...
    private void generaListaTrayectoria (GameSituations sp) {
		...
		if (listaTrayectoria.size() == 0) {
			listaTrayectoria.add(new Balon(sp.ballPosition().getX(), sp.ballPosition().getY(), sp.ballAltitude()));
		}    	
    }
    ...
    private void generaTiempoAlcanceBalonJugador (GameSituations sp, Jugador jugador) {
		...
		if (listaTrayectoria.size() > 0) {
			...
			PlayerDetail jd = sp.myPlayersDetail()[jugador.getIndice()];
			double alcanceXY = jd.isGoalKeeper() ? Constants.DISTANCIA_CONTROL_BALON_PORTERO : Constants.DISTANCIA_CONTROL_BALON;
    ...
    private void generaTiempoAlcanceBalonRival (GameSituations sp, Rival rival) {
		...
		if (listaTrayectoria.size() > 0) {
			...
			PlayerDetail rd = sp.rivalPlayersDetail()[rival.getIndice()];
    ...
    private boolean balonLlegaAPorteria(GameSituations sp) {
    	...
    	if (avanzaEnYAPorteria(sp)) {
    		...
    		if (listaTrayectoria.size() > 0) {
				...
				if (ultimaPosicion.getY() <= Constants.centroArcoInf.getY() + Constants.DISTANCIA_CONTROL_BALON_PORTERO) {
    ...
    private boolean forzarTiro (GameSituations sp) {
    	
    	boolean forzar = sp.ballPosition().distance(Constants.centroArcoSup) < Entorno.DISTANCIA_FORZAR_TIRO;
    	...
    	if (forzar && Math.abs(sp.ballPosition().getX()) > Constants.posteDerArcoSup.getX() + 1.5) {
    		
    		double incremento = 30;
    		double anguloTiro = Math.toDegrees(sp.ballPosition().angle(Constants.centroArcoSup));
    		forzar = (anguloTiro > (0 + incremento)) && (anguloTiro < (180 - incremento));
    	}    	
    ...
    private void realizaIrAPorBalon (GameSituations sp, Jugador jugador) {
    	...
    	if (jugador.getInformacionAlcanceBalon() != null) {
    		...
    		//CHR: Arreglarlo en alguna versi�n ya que si mando el bal�n fuera vamos a ir detr�s de �l.
    ...
    private void realizaTiroTrallon(GameSituations sp, Jugador jugador) {
        ...
        comandos.add(new CommandMoveTo(tiroTrallon.comando().getPlayerIndex(), Constants.centroArcoSup));
        
        contAvance = 0;
        contRegate = 0;
        
        //System.out.println("Tiro trallon");
    }
