...
public class AirTrajectory extends AbstractTrajectory {
  ...
  final AbstractTrajectory nextTrajectory;
  ...
  public AirTrajectory(double vx0, double vy0, double x0, double y0) {
    ...
    if (vy0 > 0.1) {
      nextTrajectory = new AirTrajectory(vx0, vy0, x0, y0);
    } else {
      nextTrajectory = new FloorTrajectory(Math.sqrt(vx0 * vx0 + vy0 * vy0) * 2, x0);
    }
  }
