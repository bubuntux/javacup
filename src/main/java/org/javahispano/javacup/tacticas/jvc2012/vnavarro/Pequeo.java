package org.javahispano.javacup.tacticas.jvc2012.vnavarro;
...
public class Pequeo implements org.javahispano.javacup.model.Tactic {
	...
	private void pase(int jug, GameSituations sp, LinkedList<Command> comandos, boolean defensivo) {
		...
		if (defensivo && sp.ballPosition().getY()<-9){
		...
		else if ((((desmarcados.contains(jugadores[jug]) || desmarcados.isEmpty()) && jug != 0)||solo(sp,jugador))&&
			...
			if (first){ cond = true;first = false;}
			else cond =r.nextBoolean();
	...
	private void MarcarJugadores(GameSituations sp,
		...
		if (Defensiva(sp.rivalPlayers()))zona = -14;
	...
	public List<Command> execute(GameSituations sp) {
		...
		if (sp.isStarts()) {
			...
			*/
			comandos.add(new CommandMoveTo(jugadormascercano, new Position(sp
					.ballPosition().getX(), sp.ballPosition().getY())));
			MasCercano(sp.myPlayers(), Player,-1);
			comandos.add(new CommandMoveTo(jugadormascercano2, new Position(sp.ballPosition().getX()-2,sp.ballPosition().getY()+2)));

		}
		if(sp.isRivalStarts()){portero(sp,comandos);
