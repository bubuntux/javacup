...
public class Helper {
	...
	private static double	distanciaMinimaMarca	= 0.98;
	...
	public static final Position	PUNTO_DISPARO_IZQ		= new Position(Constants.posteIzqArcoSup.getX() + 2, Constants.posteIzqArcoSup.getY());
	public static final Position	PUNTO_DISPARO_DER		= new Position(Constants.posteDerArcoSup.getX() - 2, Constants.posteDerArcoSup.getY());

	private static Position[]		rectaIntersecArquero	= new Position[] {
			new Position(Constants.centroArcoInf.getX() + (Constants.LARGO_AREA_CHICA / 2), Constants.centroArcoInf.getY() + Constants.ANCHO_AREA_CHICA),
			new Position(Constants.centroArcoInf.getX() - (Constants.LARGO_AREA_CHICA / 2), Constants.centroArcoInf.getY() + Constants.ANCHO_AREA_CHICA) };
	...
	public static CommandMoveTo marcar(int idxMiJugador, int idxRival, int rivalMasCercano, boolean balonAMiArco, HistorialMarcas historialMarcas, GameSituations sp, Util23 util23, int rivalMasPeligro) {
		Position pMarca;
		...
		if (idxRival != rivalMasPeligro
				&& (idxRival != rivalMasCercano || (pMio.distance(sp.ballPosition()) < pRival.distance(sp.ballPosition()) && sp.ballAltitude() <= Constants.ALTURA_CONTROL_BALON))) {
		...
		CommandMoveTo marcar = new CommandMoveTo(idxMiJugador, pMarca.setInsideGameField());
		return marcar;
	}

	public static Position getPosicionArquero(Position balonPos, Position pRivalMasCercano, Position pMiArquero, double alturaBalon, Position... balonposAntA) {
		if (pMiArquero.distance(balonPos) < pRivalMasCercano.distance(balonPos) && alturaBalon < Constants.ALTO_ARCO) {
			return balonPos;
		}
		Position balonposAnt = (balonposAntA == null || balonposAntA.length < 1) ? Constants.centroArcoInf : balonposAntA[0];
		Position[] rectaIntersecArquero = getLineaInterseccionArquero(balonPos);
		Position posIdeal = Position.Intersection(rectaIntersecArquero[0], rectaIntersecArquero[1], balonPos, balonposAnt);
		...
		if ((posIdeal.getX() > rectaIntersecArquero[0].getX() || posIdeal.getX() < rectaIntersecArquero[1].getX())) {
			posIdeal = Position.Intersection(rectaIntersecArquero[0], rectaIntersecArquero[1], balonPos, Constants.centroArcoInf);
			if (posIdeal == null) {
				posIdeal = Constants.cornerInfIzq;
			}
			if ((posIdeal.getX() > rectaIntersecArquero[0].getX() || posIdeal.getX() < rectaIntersecArquero[1].getX())) {
				...
				switch (getCuadranteBalon(balonPos)) {
					case I:
						posIdeal = new Position(Constants.posteIzqArcoInf.getX(), Constants.posteIzqArcoInf.getY() + (Constants.ANCHO_AREA_CHICA / 3));
						break;
					case IV:
						posIdeal = new Position(Constants.posteDerArcoInf.getX(), Constants.posteDerArcoInf.getY() + (Constants.ANCHO_AREA_CHICA / 3));
						break;
					default:
						posIdeal = Constants.centroArcoInf;
						break;
	...
	public static Position[] getLineaInterseccionArquero(Position balonPos) {
		...
		Position[] ret = new Position[] { new Position(rectaIntersecArqueroDefault[0].getX(), Constants.centroArcoInf.getY() + (Constants.ANCHO_AREA_CHICA * nuevoY)),
				new Position(rectaIntersecArqueroDefault[1].getX(), Constants.centroArcoInf.getY() + (Constants.ANCHO_AREA_CHICA * nuevoY)), };
		return ret;
	}

	private static Position[]	rectaIntersecArqueroDefault	= new Position[] {
			new Position(Constants.centroArcoInf.getX() + (Constants.LARGO_ARCO / 2), Constants.centroArcoInf.getY() + Constants.ANCHO_AREA_CHICA),
			new Position(Constants.centroArcoInf.getX() - (Constants.LARGO_ARCO / 2), Constants.centroArcoInf.getY() + Constants.ANCHO_AREA_CHICA) };
	...
	public static Position[] getRectaArqueroByPosBalon(Position posBalon) {
		if (posBalon.getY() > 0) {
			...
			Position res[] = new Position[] {
					new Position(Constants.centroArcoInf.getX() + (Constants.LARGO_AREA_CHICA / 2), Constants.centroArcoInf.getY() + (Constants.ANCHO_AREA_CHICA * multiplicador)),
					new Position(Constants.centroArcoInf.getX() - (Constants.LARGO_AREA_CHICA / 2), Constants.centroArcoInf.getY() + (Constants.ANCHO_AREA_CHICA) * multiplicador) };
			return res;
		}
	}
